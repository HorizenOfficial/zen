...
class CertMempoolCleanupSplit(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        try:
            cert_net_first_part = self.nodes[2].sc_send_certificate(scid, epoch_number, quality,
        ...
        mark_logs("Check cert {} is in mempool".format(cert_net_first_part), self.nodes, DEBUG_MODE)
        assert_true(cert_net_first_part in self.nodes[0].getrawmempool())
        ...
        try:
            csw_net_second_part = self.nodes[3].sendrawtransaction(sigRawtx['hex'])
            self.nodes[3].getrawtransaction(csw_net_second_part, 1)
        except JSONRPCException as e:
            errorString = e.error['message']
            mark_logs("Send csw failed with reason {}".format(errorString), self.nodes, DEBUG_MODE)
            assert(False)

        mark_logs("Check the CSW tx {} is in mempool...".format(csw_net_second_part), self.nodes, DEBUG_MODE)
        assert_true(csw_net_second_part in self.nodes[3].getrawmempool())
        ...
        mark_logs("Check CSW tx {} is no more in mempool, since we crossed the epoch safeguard".format(csw_net_second_part), self.nodes, DEBUG_MODE)
        assert_false(csw_net_second_part in self.nodes[3].getrawmempool()) 
