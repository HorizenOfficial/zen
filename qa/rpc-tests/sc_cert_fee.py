...
class sc_cert_base(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        try:
            cert_good = self.nodes[1].send_certificate(scid, epoch_number, quality, epoch_block_hash, proof, amounts, FT_SC_FEE, MBTR_SC_FEE, CERT_FEE)
        ...
        try:
            cert = self.nodes[3].send_certificate(scid, epoch_number, quality, epoch_block_hash, proof, amounts, FT_SC_FEE, MBTR_SC_FEE, CERT_FEE)
            assert(len(cert) > 0)
