...
class headers(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        print("------------------------------")
        self.mark_logs("\nAll nodes invalidate the most recent common block (height=%d).."%self.nodes[0].getblock(pre_sc_block_2, True)["height"])
        ...
        print("Checkikng network chain tips")
        for i in range(0, NUMB_OF_NODES):
            self.dump_ordered_tips(self.nodes[i].getchaintips())
            print "---"
