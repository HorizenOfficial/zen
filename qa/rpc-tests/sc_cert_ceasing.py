...
class sc_cert_ceasing(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        creation_amount = Decimal("12.0")
        bwt_amount = Decimal("5.0")
        ...
        mark_logs("Node0 generating 5 more blocks to achieve end of withdrawal epoch", self.nodes, DEBUG_MODE)
        self.nodes[0].generate(5)
        self.sync_all()
        epoch_block_hash, epoch_number = self.getEpochData(scid);

        # node0 create a cert_ep0 for funding node1 
        ...
        print
        mark_logs("Node0 generates 2 blocks to achieve cert maturity", self.nodes, DEBUG_MODE)
        self.nodes[0].generate(2)
        self.sync_all()
        bal1 = self.nodes[1].getbalance()
        print "Balance Node1 = {}\n".format(bal1)
        assert_equal(bal1, bwt_amount)

        mark_logs("Node 1 invalidates last block", self.nodes, DEBUG_MODE)
        self.nodes[1].invalidateblock(self.nodes[0].getbestblockhash())
        time.sleep(1)

        bal1 = self.nodes[1].getbalance()
        print "Balance Node1 = {}\n".format(bal1)
        assert_equal(bal1, Decimal("0.0"))

        mark_logs("Node1 generates 1 blocks to re-achieve cert maturity", self.nodes, DEBUG_MODE)
        ...
        mark_logs("Node1 generates 1 blocks to achieve sc ceasing", self.nodes, DEBUG_MODE)
        self.nodes[1].generate(1)
        time.sleep(1)
