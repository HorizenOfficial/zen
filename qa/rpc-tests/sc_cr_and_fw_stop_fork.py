...
from test_framework.util import initialize_chain_clean, \
    start_nodes, sync_blocks, sync_mempools, connect_nodes_bi, mark_logs, assert_true, assert_false
...
class sc_cr_fw_stop(BitcoinTestFramework):
    ...
    def setup_network(self, split=False):
        ...
        self.nodes = start_nodes(NUMB_OF_NODES, self.options.tmpdir, extra_args=
            [['-debug=py', '-debug=sc', '-logtimemicros=1']] * NUMB_OF_NODES)
    ...
    def run_test(self):
        ...
        - Join the network
           ...
           . The SC info is not retrievable anymore an no SC creation tx can be found in the mempool
        ...
        cmdInput = {
            ...
            "wCertVk": mcTest.generate_params("sc1"),
        ...
        delta = ForkHeights['STOP_SC_CR_AND_FWDT'] - currentHeight - 2
        ...
        self.sync_all()
        currentHeight = self.nodes[0].getblockcount()
        mark_logs(("active chain height = %d" % currentHeight), self.nodes, DEBUG_MODE)
        ...
        #----------------------Network Part 1
        mark_logs("Node 1 generates 2 nodes, crossing the fork point and reaching the longest chain", self.nodes, DEBUG_MODE)
