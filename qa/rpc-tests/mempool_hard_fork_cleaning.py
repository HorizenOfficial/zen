...
class Test (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        # ####################################################################################################
        # ####################################################################################################
        # Fork 12 - Shielded pool removal hard fork
        # ####################################################################################################
        for fork in range(11, 12 + 1):
            if (fork == 11):
                ...
                fork_descr = "Shielded Pool Removal"
                fork_height = ForkHeights['SHIELDED_POOL_REMOVAL']
                tx_type_descr = "shielded pool transaction"
            ...
            if (fork == 11):
                ...
                opid_z2z = self.nodes[0].z_mergetoaddress(["ANY_ZADDR"], self.nodes[0].z_getnewaddress(), Z_FEE, 0, 2)["opid"]
                wait_and_assert_operationid_status(self.nodes[0], opid_z2z)
                # Give time to the wallet to be notified and update the spent notes
                wait_until(lambda: self.nodes[0].getmempoolinfo()['fullyNotified'] == True, 20)
                opid_z2t = self.nodes[0].z_mergetoaddress(["ANY_ZADDR"], self.nodes[0].getnewaddress(), Z_FEE, 0, 2)["opid"]
                wait_and_assert_operationid_status(self.nodes[0], opid_z2t)
                expected_txs = 2
            ...
            assert_equal(self.nodes[0].getmempoolinfo()["size"], expected_txs)
