...
int64_t get_nsec_cpu_time() {
    ...
    // If we expected this to work, don't silently ignore failures, because that would hide the problem and incur an
    // unnecessarily system-call overhead. So if we ever observe this exception, we should probably add a suitable #ifdef .
    // TODO: clock_gettime(CLOCK_PROCESS_CPUTIME_ID) is not supported by native Windows. What about Cygwin? Should we #ifdef on
    // CLOCK_PROCESS_CPUTIME_ID or on __linux__?
...
static std::map<std::pair<std::string, std::string>, int64_t>
...
void print_cumulative_op_counts(const bool only_fq) {
    ...
    for (auto& msg : invocation_counts) {
        ...
        for (auto& data_point : op_data_points) {
            ...
            printf("%-5s = %7.0f (%3zu)", data_point.first.c_str(),
                   1. * cumulative_op_counts[std::make_pair(msg.first, data_point.first)] / msg.second, msg.second);
            first = false;
        }
        printf("\n");
    }
...
static void print_times_from_last_and_start(int64_t now, int64_t last, int64_t cpu_now, int64_t cpu_last) {
