...
void CCoins::From(const CScCertificate& cert, int nHeightIn, int bwtMaturityHeight, bool isBlockTopQualityCert) {
    ...
    {
        for (unsigned int bwtPos = nFirstBwtPos; bwtPos < vout.size(); ++bwtPos) Spend(bwtPos);
    }
...
void CCoins::Cleanup() {
    while (vout.size() > 0 && vout.back().IsNull()) vout.pop_back();
...
bool operator==(const CCoins& a, const CCoins& b) {
    ...
    return a.fCoinBase == b.fCoinBase && a.nHeight == b.nHeight && a.nVersion == b.nVersion && a.vout == b.vout &&
           a.nFirstBwtPos == b.nFirstBwtPos && a.nBwtMaturityHeight == b.nBwtMaturityHeight;
}

bool operator!=(const CCoins& a, const CCoins& b) { return !(a == b); }

bool CCoins::IsCoinBase() const { return fCoinBase; }
...
bool CCoins::isOutputMature(unsigned int outPos, int nSpendingHeigh) const {
    if (!IsCoinBase() && !IsFromCert()) return true;

    if (IsCoinBase()) return nSpendingHeigh >= (nHeight + COINBASE_MATURITY);
...
bool CCoins::IsAvailable(unsigned int nPos) const { return (nPos < vout.size() && !vout[nPos].IsNull()); }
...
void CCoinsView::GetScIds(std::set<uint256>& scIdsList) const {
    scIdsList.clear();
    return;
}
...
bool CCoinsView::HaveCswNullifier(const uint256& scId, const CFieldElement& nullifier) const { return false; }

bool CCoinsView::BatchWrite(CCoinsMap& mapCoins, const uint256& hashBlock, const uint256& hashAnchor, CAnchorsMap& mapAnchors,
...
bool CCoinsViewBacked::GetAnchorAt(const uint256& rt, ZCIncrementalMerkleTree& tree) const {
    return base->GetAnchorAt(rt, tree);
}
...
bool CCoinsViewBacked::GetSidechainEvents(int height, CSidechainEvents& scEvents) const {
    return base->GetSidechainEvents(height, scEvents);
}
...
bool CCoinsViewBacked::HaveCswNullifier(const uint256& scId, const CFieldElement& nullifier) const {
    return base->HaveCswNullifier(scId, nullifier);
}

void CCoinsViewBacked::SetBackend(CCoinsView& viewIn) { base = &viewIn; }
bool CCoinsViewBacked::BatchWrite(CCoinsMap& mapCoins, const uint256& hashBlock, const uint256& hashAnchor,
                                  CAnchorsMap& mapAnchors, CNullifiersMap& mapNullifiers, CSidechainsMap& mapSidechains,
                                  CSidechainEventsMap& mapSidechainEvents, CCswNullifiersMap& cswNullifiers) {
    return base->BatchWrite(mapCoins, hashBlock, hashAnchor, mapAnchors, mapNullifiers, mapSidechains, mapSidechainEvents,
                            cswNullifiers);
}
...
size_t CCoinsViewCache::DynamicMemoryUsage() const {
    return memusage::DynamicUsage(cacheCoins) + memusage::DynamicUsage(cacheAnchors) + memusage::DynamicUsage(cacheNullifiers) +
           memusage::DynamicUsage(cacheSidechains) + memusage::DynamicUsage(cacheSidechainEvents) +
           memusage::DynamicUsage(cacheCswNullifiers) + cachedCoinsUsage;
}
...
CSidechainsMap::const_iterator CCoinsViewCache::FetchSidechains(const uint256& scId) const {
    ...
    if (candidateIt != cacheSidechains.end()) return candidateIt;

    CSidechain tmp;
    if (!base->GetSidechain(scId, tmp)) return cacheSidechains.end();
...
CSidechainsMap::iterator CCoinsViewCache::ModifySidechain(const uint256& scId) {
    ...
    if (base->GetSidechain(scId, tmp))
        ret = cacheSidechains.insert(std::make_pair(scId, CSidechainsCacheEntry(tmp, CSidechainsCacheEntry::Flags::DEFAULT)))
                  .first;
    else
        ret =
            cacheSidechains.insert(std::make_pair(scId, CSidechainsCacheEntry(tmp, CSidechainsCacheEntry::Flags::FRESH))).first;
...
CSidechainEventsMap::const_iterator CCoinsViewCache::FetchSidechainEvents(int height) const {
    ...
    if (candidateIt != cacheSidechainEvents.end()) return candidateIt;

    CSidechainEvents tmp;
    if (!base->GetSidechainEvents(height, tmp)) return cacheSidechainEvents.end();
    ...
    CSidechainEventsMap::iterator ret =
        cacheSidechainEvents
            .insert(std::make_pair(height, CSidechainEventsCacheEntry(tmp, CSidechainEventsCacheEntry::Flags::DEFAULT)))
            .first;
...
CSidechainEventsMap::iterator CCoinsViewCache::ModifySidechainEvents(int height) {
    ...
    if (!base->GetSidechainEvents(height, tmp))
        ret = cacheSidechainEvents
                  .insert(std::make_pair(height, CSidechainEventsCacheEntry(tmp, CSidechainEventsCacheEntry::Flags::FRESH)))
                  .first;
    else
        ret = cacheSidechainEvents
                  .insert(std::make_pair(height, CSidechainEventsCacheEntry(tmp, CSidechainEventsCacheEntry::Flags::DEFAULT)))
...
bool CCoinsViewCache::GetNullifier(const uint256& nullifier) const {
    ...
    if (it != cacheNullifiers.end()) return it->second.entered;
...
uint256 CCoinsViewCache::GetBestAnchor() const {
    if (hashAnchor.IsNull()) hashAnchor = base->GetBestAnchor();
    return hashAnchor;
}

void CCoinsViewCache::SetBestBlock(const uint256& hashBlockIn) { hashBlock = hashBlockIn; }
...
bool CCoinsViewCache::HaveCswNullifier(const uint256& scId, const CFieldElement& nullifier) const {
    ...
    if (it != cacheCswNullifiers.end()) return (it->second.flag != CCswNullifiersCacheEntry::Flags::ERASED);

    if (!base->HaveCswNullifier(scId, nullifier)) return false;
...
bool CCoinsViewCache::AddCswNullifier(const uint256& scId, const CFieldElement& nullifier) {
    if (HaveCswNullifier(scId, nullifier)) return false;
...
size_t CCoinsViewCache::WriteCoins(const uint256& key, CCoinsCacheEntry& value) {
    size_t res = 0;
    if (value.flags & CCoinsCacheEntry::DIRTY) {  // Ignore non-dirty entries (optimization).
...
bool CCoinsViewCache::BatchWrite(CCoinsMap& mapCoins, const uint256& hashBlockIn, const uint256& hashAnchorIn,
                                 CAnchorsMap& mapAnchors, CNullifiersMap& mapNullifiers, CSidechainsMap& mapSidechains,
                                 CSidechainEventsMap& mapSidechainEvents, CCswNullifiersMap& cswNullifiers) {
    assert(!hasModifier);
    ...
    for (auto& entryToWrite : mapSidechains) WriteMutableEntry(entryToWrite.first, entryToWrite.second, cacheSidechains);

    for (auto& entryToWrite : mapSidechainEvents)
        WriteMutableEntry(entryToWrite.first, entryToWrite.second, cacheSidechainEvents);

    for (auto& entryToWrite : cswNullifiers) WriteImmutableEntry(entryToWrite.first, entryToWrite.second, cacheCswNullifiers);
...
bool CCoinsViewCache::CheckQuality(const CScCertificate& cert) const {
    ...
    if (GetSidechain(cert.GetScId(), info)) {
        if (info.lastTopQualityCertHash != cert.GetHash() && info.lastTopQualityCertReferencedEpoch == cert.epochNumber &&
...
bool CCoinsViewCache::UpdateSidechain(const CTransaction& tx, const CBlock& block, int blockHeight) {
    ...
    for (const CTxCeasedSidechainWithdrawalInput& csw : tx.GetVcswCcIn()) {
        ...
        LogPrint("sc", "%s():%d - sidechain balance decreased by CSW in scView csw_amount=%s scId=%s\n", __func__, __LINE__,
                 FormatMoney(csw.nValue), csw.scId.ToString());
    }
    ...
    for (const auto& cr : tx.GetVscCcOut()) {
        ...
        scIt->second.sidechain.lastTopQualityCertView.mainchainBackwardTransferRequestScFee =
            cr.mainchainBackwardTransferRequestScFee;
        ...
        scIt->second.sidechain.fixedParams.mainchainBackwardTransferRequestDataLength =
            cr.mainchainBackwardTransferRequestDataLength;
        ...
        LogPrint("sc", "%s():%d - immature balance added in scView (h=%d, amount=%s) %s\n", __func__, __LINE__, maturityHeight,
                 FormatMoney(cr.nValue), scId.ToString());
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: scCreation next maturing height [%d]\n", __func__, __LINE__,
                 cr.GetScId().ToString(), maturityHeight);
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: scCreation next ceasing height [%d]\n", __func__, __LINE__,
                 cr.GetScId().ToString(), nextCeasingHeight);
    }
    ...
    for (auto& ft : tx.GetVftCcOut()) {
        ...
        if (scIt->second.flag != CSidechainsCacheEntry::Flags::FRESH) scIt->second.flag = CSidechainsCacheEntry::Flags::DIRTY;

        LogPrint("sc", "%s():%d - immature balance added in scView (h=%d, amount=%s) %s\n", __func__, __LINE__, maturityHeight,
                 FormatMoney(ft.GetScValue()), ft.scId.ToString());
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: fwd Transfer next maturing height [%d]\n", __func__, __LINE__,
                 ft.GetScId().ToString(), maturityHeight);
    }
    ...
    for (auto& mbtr : tx.GetVBwtRequestOut()) {
        ...
        LogPrint("sc", "%s():%d - immature balance added in scView (h=%d, amount=%s) %s\n", __func__, __LINE__, maturityHeight,
                 FormatMoney(mbtr.GetScValue()), mbtr.scId.ToString());
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: mbtr scFees next maturing height [%d]\n", __func__, __LINE__,
                 mbtr.scId.ToString(), maturityHeight);
    }
...
bool CCoinsViewCache::RevertTxOutputs(const CTransaction& tx, int nHeight) {
    ...
    for (const auto& entry : tx.GetVBwtRequestOut()) {
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s] cancelled maturing height [%s] for fwd amount.\n", __func__,
                 __LINE__, entry.scId.ToString(), maturityHeight);
    }
    ...
    for (const auto& entry : tx.GetVscCcOut()) {
        ...
        if (scIt->second.sidechain.balance > 0) {
            ...
            LogPrintf("ERROR %s():%d - scId=%s balance not null: %s\n", __func__, __LINE__, scId.ToString(),
                      FormatMoney(scIt->second.sidechain.balance));
            return false;
        }
        ...
        if (HaveSidechainEvents(maturityHeight)) {
            ...
            LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s] deleted maturing height [%d] for creation amount.\n", __func__,
                     __LINE__, entry.GetScId().ToString(), maturityHeight);
        } else
            LogPrint(
                "sc",
                "%s():%d - SIDECHAIN-EVENT: scId[%s] nothing to do for scCreation amount maturing canceling at height [%d].\n",
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: undo of creation removes currentCeasingHeight [%d]\n", __func__,
                 __LINE__, entry.GetScId().ToString(), ceasingHeightToErase);
    }
...
CValidationState::Code CCoinsViewCache::IsScTxApplicableToState(const CTransaction& tx,
                                                                Sidechain::ScFeeCheckFlag scFeeCheckType,
                                                                bool* banSenderNode) const {
    return CValidationState::Code::OK;
}
...
void CCoinsViewCache::HandleMaturityHeightIndexSidechainEvents(
    int height, CBlockTreeDB* pblocktree,
    std::vector<std::pair<CMaturityHeightKey, CMaturityHeightValue>>& maturityHeightIndex) {
    return;
}

void CCoinsViewCache::RevertMaturityHeightIndexSidechainEvents(
    int height, CBlockUndo& blockUndo, CBlockTreeDB* pblocktree,
    std::vector<std::pair<CMaturityHeightKey, CMaturityHeightValue>>& maturityHeightIndex) {
    return;
}

#ifdef ENABLE_ADDRESS_INDEXING
void CCoinsViewCache::HandleIndexesSidechainEvents(
    int height, CBlockTreeDB* pblocktree, std::vector<std::pair<CAddressIndexKey, CAddressIndexValue>>& addressIndex,
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>& addressUnspentIndex) {
    return;
}

void CCoinsViewCache::RevertIndexesSidechainEvents(
    int height, CBlockUndo& blockUndo, CBlockTreeDB* pblocktree,
...
bool CCoinsViewCache::CheckCertTiming(const uint256& scId, int certEpoch) const {
    ...
    if (certEpoch != sidechain.lastTopQualityCertReferencedEpoch &&
        ...
        return error(
            "%s():%d - ERROR: certificate cannot be accepted, wrong epoch. Certificate Epoch %d (expected: %d or %d)\n",
            __func__, __LINE__, certEpoch, sidechain.lastTopQualityCertReferencedEpoch,
...
CValidationState::Code CCoinsViewCache::IsCertApplicableToState(const CScCertificate& cert, bool* banSenderNode) const {
    ...
    if (!Sidechain::checkCertCustomFields(sidechain, cert)) {
        ...
        if (banSenderNode) *banSenderNode = true;
        return CValidationState::Code::INVALID;
    }
    ...
    if (bwtTotalAmount > scBalance) {
        LogPrintf("%s():%d - ERROR: insufficent balance in scId[%s]: balance[%s], cert amount[%s]\n", __func__, __LINE__,
                  cert.GetScId().ToString(), FormatMoney(scBalance), FormatMoney(bwtTotalAmount));
    ...
    if (proof_plus_vk_size > Sidechain::MAX_PROOF_PLUS_VK_SIZE) {
        LogPrintf("%s():%d - ERROR: Cert [%s]\n proof plus vk size (%d) exceeded the limit %d\n", __func__, __LINE__,
                  certHash.ToString(), proof_plus_vk_size, Sidechain::MAX_PROOF_PLUS_VK_SIZE);
        if (banSenderNode) *banSenderNode = true;
        return CValidationState::Code::INVALID;
    }
...
bool CCoinsViewCache::CheckMinimumFtScFee(const CTxForwardTransferOut& ftOutput, CAmount* minValPtr) const {
    if (minValPtr) *minValPtr = MAX_MONEY;

    const CSidechain* const pSidechain = this->AccessSidechain(ftOutput.scId);

    if (pSidechain == nullptr) return false;
    if (this->GetSidechainState(ftOutput.scId) != CSidechain::State::ALIVE) return false;

    CAmount minVal = pSidechain->GetMinFtScFee();

    if (minValPtr) *minValPtr = minVal;
...
bool CCoinsViewCache::CheckMinimumMbtrScFee(const CBwtRequestOut& mbtrOutput, CAmount* minValPtr) const {
    ...
    if (pSidechain == nullptr) return false;
    if (this->GetSidechainState(mbtrOutput.scId) != CSidechain::State::ALIVE) return false;
        ...
        if (!IsFtScFeeApplicable(ft)) {
            if (scFeeCheckType == Sidechain::ScFeeCheckFlag::MINIMUM_IN_A_RANGE) {
                ...
                if (!CheckMinimumFtScFee(ft, &minScFee)) {
                    LogPrintf(
                        "%s():%d - ERROR: Invalid tx[%s] to scId[%s]: FT amount [%s] must be greater than minimum SC FT fee "
                        "[%s]\n",
                        __func__, __LINE__, txHash.ToString(), scId.ToString(), FormatMoney(ft.nValue), FormatMoney(minScFee));
                    return CValidationState::Code::INVALID;
                }
                LogPrintf(
                    "%s():%d - Warning: tx[%s] to scId[%s]: FT amount [%s] is not greater than act cert fee [%s], but is "
                    "greater than minimum SC FT fee [%s]\n",
                ...
                LogPrintf("%s():%d - ERROR: Invalid tx[%s] to scId[%s]: FT amount [%s] must be greater than SC FT fee [%s]\n",
                          __func__, __LINE__, txHash.ToString(), scId.ToString(), FormatMoney(ft.nValue),
                          FormatMoney(GetActiveCertView(scId).forwardTransferScFee));
                return CValidationState::Code::INVALID;
            }
        }

        LogPrint("sc", "%s():%d - OK: tx[%s] is sending [%s] to scId[%s]\n", __func__, __LINE__, txHash.ToString(),
                 FormatMoney(ft.nValue), scId.ToString());
    }
    ...
    for (size_t idx = 0; idx < tx.GetVBwtRequestOut().size(); ++idx) {
        ...
        if (!GetSidechain(scId, sidechain)) {
            LogPrintf("%s():%d - ERROR: tx[%s] MBTR output [%s] refers to unknown scId[%s]\n", __func__, __LINE__,
                      tx.ToString(), mbtr.ToString(), scId.ToString());
            return CValidationState::Code::INVALID;
        }
        ...
        if (mbtr.vScRequestData.size() != sidechain.fixedParams.mainchainBackwardTransferRequestDataLength) {
            LogPrintf(
                "%s():%d - ERROR: Invalid tx[%s] : MBTR request data size [%d] must be equal to the size specified "
                "during sidechain creation [%d] for scId[%s]\n",
                __func__, __LINE__, txHash.ToString(), mbtr.vScRequestData.size(),
                sidechain.fixedParams.mainchainBackwardTransferRequestDataLength, scId.ToString());
            if (banSenderNode) *banSenderNode = true;
            return CValidationState::Code::INVALID;
        }
        ...
        if (!IsMbtrScFeeApplicable(mbtr)) {
            if (scFeeCheckType == Sidechain::ScFeeCheckFlag::MINIMUM_IN_A_RANGE) {
                ...
                if (!CheckMinimumMbtrScFee(mbtr, &minScFee)) {
                    LogPrintf(
                        "%s():%d - ERROR: Invalid tx[%s] to scId[%s]: MBTR fee [%s] can not be less than minimum SC MBTR fee "
                        "[%s]\n",
                        __func__, __LINE__, txHash.ToString(), scId.ToString(), FormatMoney(mbtr.scFee), FormatMoney(minScFee));
                    return CValidationState::Code::INVALID;
                }
                LogPrintf(
                    "%s():%d - Warning: tx[%s] to scId[%s]: MBTR fee [%s] is lesser than act cert fee [%s], but is not lesser "
                    "than minimum SC MBTR fee [%s]\n",
    ...
    for (const CTxCeasedSidechainWithdrawalInput& csw : tx.GetVcswCcIn()) {
        ...
        if (s != CSidechain::State::CEASED) {
            LogPrintf("%s():%d - ERROR: Tx[%s] CSW input [%s]\n cannot be accepted, sidechain is not ceased\n", __func__,
                      __LINE__, tx.ToString(), csw.ToString());
            return CValidationState::Code::INVALID;
        }

        if (!sidechain.fixedParams.wCeasedVk.is_initialized()) {
            LogPrintf("%s():%d - ERROR: Tx[%s] CSW input [%s]\n refers to SC without CSW support\n", __func__, __LINE__,
                      tx.GetHash().ToString(), csw.ToString());
            if (banSenderNode) *banSenderNode = true;
            return CValidationState::Code::INVALID;
        }

        size_t proof_plus_vk_size =
            sidechain.fixedParams.wCeasedVk.get().GetByteArray().size() + csw.scProof.GetByteArray().size();
        if (proof_plus_vk_size > Sidechain::MAX_PROOF_PLUS_VK_SIZE) {
            LogPrintf("%s():%d - ERROR: Tx[%s] CSW input [%s]\n proof plus vk size (%d) exceeded the limit %d\n", __func__,
                      __LINE__, tx.GetHash().ToString(), csw.ToString(), proof_plus_vk_size, Sidechain::MAX_PROOF_PLUS_VK_SIZE);
            if (banSenderNode) *banSenderNode = true;
            return CValidationState::Code::INVALID;
        }
        ...
        if (cswTotalBalances[csw.scId] > sidechain.balance) {
            LogPrintf("%s():%d - ERROR: Tx[%s] CSW inputs total amount[%s] is greater than sc[%s] total balance[%s]\n",
                      __func__, __LINE__, tx.ToString(), FormatMoney(cswTotalBalances[csw.scId]), csw.scId.ToString(),
                      FormatMoney(sidechain.balance));
            return CValidationState::Code::INSUFFICIENT_SCID_FUNDS;
        }
...
void CCoinsViewCache::HandleTxIndexSidechainEvents(int height, CBlockTreeDB* pblocktree,
                                                   std::vector<std::pair<uint256, CTxIndexValue>>& txIndex) {
    if (!HaveSidechainEvents(height)) return;
...
void CCoinsViewCache::RevertTxIndexSidechainEvents(int height, CBlockUndo& blockUndo, CBlockTreeDB* pblocktree,
    ...
    for (auto it = blockUndo.scUndoDatabyScId.begin(); it != blockUndo.scUndoDatabyScId.end(); ++it) {
        if ((it->second.contentBitMask & CSidechainUndoData::AvailableSections::CEASED_CERT_DATA) == 0) continue;
...
void CCoinsViewCache::RevertIndexesSidechainEvents(
    ...
    for (auto it = blockUndo.scUndoDatabyScId.begin(); it != blockUndo.scUndoDatabyScId.end(); ++it) {
        ...
        if (pSidechain->lastTopQualityCertReferencedEpoch != CScCertificate::EPOCH_NULL) {
            ...
            UpdateBackwardTransferIndexes(pSidechain->lastTopQualityCertHash, txIndexVal.txIndex, addressIndex,
                                          addressUnspentIndex, CCoinsViewCache::flagIndexesUpdateType::RESTORE_CERTIFICATE);
        }
    }
}
#endif  // ENABLE_ADDRESS_INDEXING
...
bool CCoinsViewCache::UpdateSidechain(const CScCertificate& cert, CBlockUndo& blockUndo) {
    ...
    if (cert.epochNumber == (currentSc.lastTopQualityCertReferencedEpoch + 1)) {
        ...
        {
            return error("%s():%d - ERROR: cert quality %d not greater than last seen %d", __func__, __LINE__, cert.quality,
                         currentSc.lastTopQualityCertQuality);
        }

        currentSc.balance += currentSc.lastTopQualityCertBwtAmount;
    } else
        return error("%s():%d - ERROR: bad epoch value: %d (should be %d)\n", __func__, __LINE__, cert.epochNumber,
                     currentSc.lastTopQualityCertReferencedEpoch + 1);
    ...
    if (currentSc.balance < bwtTotalAmount) {
        return error("%s():%d - ERROR: Can not update balance %s with amount[%s] for scId=%s, would be negative\n", __func__,
                     __LINE__, FormatMoney(currentSc.balance), FormatMoney(bwtTotalAmount), scId.ToString());
    }
    ...
    if (cert.epochNumber != scUndoData.prevTopCommittedCertReferencedEpoch) {
        ...
        if (!HaveSidechainEvents(prevCeasingHeight)) {
            return error(
                "%s():%d - ERROR-SIDECHAIN-EVENT: scId[%s]: Could not find scheduling for current ceasing height [%d] nor next "
                "ceasing height [%d]\n",
        ...
        LogPrint("sc",
                 "%s():%d - SIDECHAIN-EVENT: scId[%s]: cert [%s] removes prevCeasingHeight [%d] (certEp=%d, currentEp=%d)\n",
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: cert [%s] sets nextCeasingHeight to [%d]\n", __func__, __LINE__,
                 cert.GetScId().ToString(), cert.GetHash().ToString(), nextCeasingHeight);
    }
...
#ifdef ENABLE_ADDRESS_INDEXING
void CCoinsViewCache::UpdateBackwardTransferIndexes(
    const uint256& certHash, int certIndex, std::vector<std::pair<CAddressIndexKey, CAddressIndexValue>>& addressIndex,
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>& addressUnspentIndex, flagIndexesUpdateType updateType) {
    LogPrint("cert", "%s():%d - called for cert %s\n", __func__, __LINE__, certHash.ToString());
    if (certHash.IsNull()) return;
    ...
    for (int pos = coins->nFirstBwtPos; pos < coins->vout.size(); ++pos) {
        ...
        if (scriptType != CScript::UNKNOWN) {
            ...
            addressIndex.push_back(
                std::make_pair(CAddressIndexKey(scriptType, addrHash, coins->nHeight, certIndex, certHash, pos, false),
            ...
            addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(scriptType, addrHash, certHash, pos),
                                                         CAddressUnspentValue(btOut.nValue, btOut.scriptPubKey, coins->nHeight,
                                                                              coins->nBwtMaturityHeight * multiplier)));
        }
    }
}
#endif  // ENABLE_ADDRESS_INDEXING
...
bool CCoinsViewCache::RestoreSidechain(const CScCertificate& certToRevert, const CSidechainUndoData& sidechainUndo) {
    ...
    if (certToRevert.epochNumber != currentSc.lastTopQualityCertReferencedEpoch) {
        ...
        if (!HaveSidechainEvents(ceasingHeightToErase)) {
            return error(
                "%s():%d - ERROR-SIDECHAIN-EVENT: scId[%s]: Could not find scheduling for current ceasing height [%d] nor "
                "previous ceasing height [%d]\n",
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT:: scId[%s]: undo of cert [%s] removes currentCeasingHeight [%d]\n", __func__,
                 __LINE__, certToRevert.GetScId().ToString(), certToRevert.GetHash().ToString(), ceasingHeightToErase);
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId[%s]: undo of cert [%s] set nextCeasingHeight to [%d]\n", __func__,
                 __LINE__, certToRevert.GetScId().ToString(), certToRevert.GetHash().ToString(), ceasingHeightToRestore);
    }
...
bool CCoinsViewCache::HandleSidechainEvents(int height, CBlockUndo& blockUndo,
    ...
    if (!HaveSidechainEvents(height)) return true;
    ...
    for (const uint256& maturingScId : scEvents.maturingScs) {
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: scId=%s balance updated to: %s\n", __func__, __LINE__,
                 maturingScId.ToString(), FormatMoney(scMaturingIt->second.sidechain.balance));
        ...
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: adding immature amount %s for scId=%s in blockundo\n", __func__, __LINE__,
                 FormatMoney(scMaturingIt->second.sidechain.mImmatureAmounts[height]), maturingScId.ToString());
    ...
    for (const uint256& ceasingScId : scEvents.ceasingScs) {
        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: about to handle scId[%s] and ceasingHeight [%d]\n", __func__, __LINE__,
                 ceasingScId.ToString(), height);

        CSidechain sidechain;
        assert(GetSidechain(ceasingScId, sidechain));

        LogPrint("sc", "%s():%d - SIDECHAIN-EVENT: lastCertEpoch [%d], lastCertHash [%s]\n", __func__, __LINE__,
                 sidechain.lastTopQualityCertReferencedEpoch, sidechain.lastTopQualityCertHash.ToString());

        LogPrint("sc", "%s():%d - set voidedCertHash[%s], ceasingScId = %s\n", __func__, __LINE__,
                 sidechain.lastTopQualityCertHash.ToString(), ceasingScId.ToString());
        ...
        if (pCertsStateInfo != nullptr)
            pCertsStateInfo->push_back(CScCertificateStatusUpdateInfo(
                ceasingScId, sidechain.lastTopQualityCertHash, sidechain.lastTopQualityCertReferencedEpoch,
...
bool CCoinsViewCache::RevertSidechainEvents(const CBlockUndo& blockUndo, int height,
    ...
    if (HaveSidechainEvents(height)) {
        LogPrint("sc",
                 "%s():%d - SIDECHAIN-EVENT:: attempt to recreate sidechain event at height [%d], but there is one already\n",
    ...
    for (auto it = blockUndo.scUndoDatabyScId.begin(); it != blockUndo.scUndoDatabyScId.end(); ++it) {
        if ((it->second.contentBitMask & CSidechainUndoData::AvailableSections::MATURED_AMOUNTS) == 0) continue;
        ...
        if (amountToRestore > 0) {
            LogPrint("sc", "%s():%d - adding immature amount %s into sc view for scId=%s\n", __func__, __LINE__,
                     FormatMoney(amountToRestore), scIdString);
            ...
            if (scIt->second.sidechain.balance < amountToRestore) {
                LogPrint("sc", "%s():%d - Can not update balance with amount[%s] for scId=%s, would be negative\n", __func__,
                         __LINE__, FormatMoney(amountToRestore), scId.ToString());
                return false;
            }
    ...
    for (auto it = blockUndo.scUndoDatabyScId.begin(); it != blockUndo.scUndoDatabyScId.end(); ++it) {
        ...
        if (pSidechain->lastTopQualityCertReferencedEpoch != CScCertificate::EPOCH_NULL) {
            ...
            if (pCertsStateInfo != nullptr)
                pCertsStateInfo->push_back(CScCertificateStatusUpdateInfo(
                    scId, pSidechain->lastTopQualityCertHash, pSidechain->lastTopQualityCertReferencedEpoch,
                    pSidechain->lastTopQualityCertQuality, CScCertificateStatusUpdateInfo::BwtState::BWT_ON));
        }
...
CSidechain::State CCoinsViewCache::GetSidechainState(const uint256& scId) const {
    CSidechain sidechain;
    if (!GetSidechain(scId, sidechain)) return CSidechain::State::NOT_APPLICABLE;

    if (!sidechain.isCreationConfirmed()) return CSidechain::State::UNCONFIRMED;
...
const CScCertificateView& CCoinsViewCache::GetActiveCertView(const uint256& scId) const {
    ...
    if (this->GetSidechainState(scId) == CSidechain::State::CEASED) return pSidechain->pastEpochTopQualityCertView;

    if (this->GetSidechainState(scId) == CSidechain::State::UNCONFIRMED) return pSidechain->lastTopQualityCertView;
...
CFieldElement CCoinsViewCache::GetCeasingCumTreeHash(const uint256& scId) const {
    ...
    if (pSidechain == nullptr) return CFieldElement{};

    CFieldElement ceasedBlockCum;
    if (!pSidechain->GetCeasingCumTreeHash(ceasedBlockCum)) return CFieldElement{};
...
bool CCoinsViewCache::Flush() {
    bool fOk = base->BatchWrite(cacheCoins, hashBlock, hashAnchor, cacheAnchors, cacheNullifiers, cacheSidechains,
                                cacheSidechainEvents, cacheCswNullifiers);
    cacheCoins.clear();
...
bool CCoinsViewCache::DecrementImmatureAmount(const uint256& scId, const CSidechainsMap::iterator& targetEntry, CAmount nValue,
                                              int maturityHeight) {
...
CAmount CCoinsViewCache::GetValueIn(const CTransactionBase& txBase) const {
    if (txBase.IsCoinBase()) return 0;

    CAmount nResult = 0;
    for (const CTxIn& in : txBase.GetVin()) nResult += GetOutputFor(in).nValue;
