...
CMutableTransaction GetValidReceiveTransaction(ZCJoinSplit& params, const libzcash::SpendingKey& sk, CAmount value,
                                               bool randomInputs, int32_t version /* = 2 */) {
    ...
    std::array<libzcash::JSOutput, 2> outputs = {libzcash::JSOutput(sk.address(), value),
                                                 libzcash::JSOutput(sk.address(), value)};

    // Prepare JoinSplits
    uint256 rt;
    JSDescription jsdesc{false, params, mtx.joinSplitPubKey, rt, inputs, outputs, 2 * value, 0, false};
...
CWalletTx GetValidReceive(ZCJoinSplit& params, const libzcash::SpendingKey& sk, CAmount value, bool randomInputs,
                          int32_t version /* = 2 */) {
    CMutableTransaction mtx = GetValidReceiveTransaction(params, sk, value, randomInputs, version);
    CTransaction tx{mtx};
...
libzcash::Note GetNote(ZCJoinSplit& params, const libzcash::SpendingKey& sk, const CTransaction& tx, size_t js, size_t n) {
    ...
    auto note_pt = libzcash::NotePlaintext::decrypt(decryptor, tx.GetVjoinsplit()[js].ciphertexts[n],
                                                    tx.GetVjoinsplit()[js].ephemeralKey, hSig, (unsigned char)n);
...
CWalletTx GetValidSpend(ZCJoinSplit& params, const libzcash::SpendingKey& sk, const libzcash::Note& note, CAmount value) {
    ...
    std::array<libzcash::JSInput, 2> inputs = {libzcash::JSInput(tree.witness(), note, sk), dummyin};
    ...
    JSDescription jsdesc{false, params, mtx.joinSplitPubKey, rt, inputs, outputs, 0, value, false};
    mtx.vjoinsplit.push_back(jsdesc);
