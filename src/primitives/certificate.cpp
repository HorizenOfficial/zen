...
CScCertificate::CScCertificate(int versionIn)
    : CTransactionBase(versionIn),
      scId(),
      ...
      nFirstBwtPos(0),
      forwardTransferScFee(INT_NULL),
      mainchainBackwardTransferRequestScFee(INT_NULL) {}

CScCertificate::CScCertificate(const CScCertificate& cert)
    : CTransactionBase(cert),
      scId(cert.scId),
      epochNumber(cert.epochNumber),
      quality(cert.quality),
      endEpochCumScTxCommTreeRoot(cert.endEpochCumScTxCommTreeRoot),
      scProof(cert.scProof),
      ...
      forwardTransferScFee(cert.forwardTransferScFee),
...
CScCertificate& CScCertificate::operator=(const CScCertificate& cert) {
    ...
    *const_cast<std::vector<FieldElementCertificateField>*>(&vFieldElementCertificateField) =
        cert.vFieldElementCertificateField;
...
void CScCertificate::UpdateHash() const { *const_cast<uint256*>(&hash) = SerializeHash(*this); }
...
bool CScCertificate::IsVersionStandard(int nHeight) const { return true; }
...
bool CScCertificate::CheckInputsOutputsNonEmpty(CValidationState& state) const {
    ...
    if (GetVin().empty()) {
        ...
        return state.DoS(10, error("%s(): vin empty", __func__), CValidationState::Code::INVALID, "bad-cert-vin-empty");
    }
...
bool CScCertificate::CheckAmounts(CValidationState& state) const {
    ...
    for (unsigned int pos = 0; pos < vout.size(); ++pos) {
        ...
        if (pos >= nFirstBwtPos && txout.nValue == 0)
            return state.DoS(100, error("CheckAmounts(): backward transfer has zero amount"), CValidationState::Code::INVALID,
                             "bad-txns-bwd-vout-zero");
    ...
    if (!MoneyRange(GetValueOfBackwardTransfers())) {
        LogPrint("sc", "%s():%d - Invalid cert[%s] : certificate amount is outside range\n", __func__, __LINE__,
                 GetHash().ToString());
        return state.DoS(100, error("%s: certificate amount is outside range", __func__), CValidationState::Code::INVALID,
                         "bwd-transfer-amount-outside-range");
    }
...
bool CScCertificate::CheckFeeAmount(const CAmount& totalVinAmount, CValidationState& state) const {
    if (!MoneyRange(totalVinAmount))
        return state.DoS(100, error("CheckFeeAmount(): total input amount out of range"), CValidationState::Code::INVALID,
                         "bad-cert-inputvalues-outofrange");
    ...
    if (totalVinAmount < GetValueOfChange())
        return state.DoS(100,
                         error("CheckInputs(): %s value in (%s) < value out (%s)", GetHash().ToString(),
    ...
    if (nCertFee < 0)
        return state.DoS(100, error("CheckFeeAmount(): %s nCertFee < 0", GetHash().ToString()), CValidationState::Code::INVALID,
                         "bad-cert-fee-negative");

    if (!MoneyRange(nCertFee))
        return state.DoS(100, error("CheckFeeAmount(): nCertFee out of range"), CValidationState::Code::INVALID,
                         "bad-cert-fee-outofrange");
...
CAmount CScCertificate::GetFeeAmount(const CAmount& valueIn) const { return (valueIn - GetValueOfChange()); }
#ifdef BITCOIN_TX
std::string CScCertificate::EncodeHex() const { return std::string{}; }
#else
std::string CScCertificate::EncodeHex() const { return EncodeHexCert(*this); }
#endif
...
std::string CScCertificate::ToString() const {
    ...
    std::string str;
    str += strprintf(
        "CScCertificate(hash=%s, ver=%d, vin.size()=%s, vout.size=%u, nFirstBwtPos=%d, ftFee=%d, mbtrFee=%d "
        "totAmount=%d.%08d\n)",
        GetHash().ToString().substr(0, 10), nVersion, vin.size(), vout.size(), nFirstBwtPos, forwardTransferScFee,
        mainchainBackwardTransferRequestScFee, total / COIN, total % COIN);
    ...
    if (limit > 0) {
        ...
        if (n > limit) {
            LogPrint("mempool", "%s():%d - Dropping cert %s : too many transparent inputs %zu > limit %zu\n", __func__,
                     __LINE__, GetHash().ToString(), n, limit);
            return false;
        }
    }
    return true;
...
bool CScCertificate::VerifyScript(const CScript& scriptPubKey, unsigned int nFlags, unsigned int nIn, const CChain* chain,
...
std::shared_ptr<const CTransactionBase> CScCertificate::MakeShared() const { return std::shared_ptr<const CTransactionBase>(); }
CFieldElement CScCertificate::GetDataHash(const Sidechain::ScFixedParameters& scFixedParams) const {
    static const CFieldElement dummy;
    return dummy;
}
#else
...
bool CScCertificate::VerifyScript(const CScript& scriptPubKey, unsigned int nFlags, unsigned int nIn, const CChain* chain,
                                  bool cacheStore, ScriptError* serror) const {
    if (nIn >= GetVin().size())
        return ::error("%s:%d can not verify Signature: nIn too large for vin size %d", GetHash().ToString(), nIn,
                       GetVin().size());

    const CScript& scriptSig = GetVin()[nIn].scriptSig;

    if (!::VerifyScript(scriptSig, scriptPubKey, nFlags, CachingCertificateSignatureChecker(this, nIn, chain, cacheStore),
                        serror)) {
...
void CScCertificate::AddJoinSplitToJSON(UniValue& entry) const { entry.pushKV("vjoinsplit", UniValue{UniValue::VARR}); }
...
CFieldElement CScCertificate::GetDataHash(const Sidechain::ScFixedParameters& scFixedParams) const {
    ...
    field_t* certDataHash =
        zendoo_get_cert_data_hash(scidFe, input.epochNumber, input.quality, bt_list_ptr, bt_list_len, custom_fields.get(),
                                  custom_fields_len, input.endEpochCumScTxCommTreeRoot.GetFieldElement().get(),
                                  input.mainchainBackwardTransferRequestScFee, input.forwardTransferScFee, &errorCode);
...
CAmount CScCertificate::GetValueOfBackwardTransfers() const {
    CAmount nValueOut = 0;
    for (int pos = nFirstBwtPos; pos < vout.size(); ++pos) nValueOut += vout[pos].nValue;
...
CAmount CScCertificate::GetValueOfChange() const {
    CAmount nValueOut = 0;
    for (unsigned int pos = 0; pos < nFirstBwtPos; ++pos) nValueOut += vout[pos].nValue;
    return nValueOut;
}
...
CMutableScCertificate::CMutableScCertificate()
    : CMutableTransactionBase(),
      scId(),
      epochNumber(CScCertificate::EPOCH_NULL),
      quality(CScCertificate::QUALITY_NULL),
      endEpochCumScTxCommTreeRoot(),
      scProof(),
      vFieldElementCertificateField(),
      vBitVectorCertificateField(),
...
uint256 CMutableScCertificate::GetHash() const { return SerializeHash(*this); }

void CMutableScCertificate::insertAtPos(unsigned int pos, const CTxOut& out) {
    if (pos < nFirstBwtPos) ++(*const_cast<int*>(&nFirstBwtPos));
...
void CMutableScCertificate::resizeOut(unsigned int newSize) {
    if (newSize > nFirstBwtPos) vout.insert(vout.begin() + nFirstBwtPos, newSize - nFirstBwtPos, CTxOut());

    if (newSize < nFirstBwtPos) vout.erase(vout.begin() + newSize, vout.begin() + nFirstBwtPos);

    (*const_cast<int*>(&nFirstBwtPos)) = newSize;
}

void CMutableScCertificate::resizeBwt(unsigned int newSize) { vout.resize(nFirstBwtPos + newSize); }
...
std::string CMutableScCertificate::ToString() const {
    ...
    str += strprintf("CMutableScCertificate(hash=%s, ver=%d, ftFee=%d, mbtrFee=%d, vin.size()=%s, vout.size=%u\n)",
                     GetHash().ToString().substr(0, 10), nVersion, forwardTransferScFee, mainchainBackwardTransferRequestScFee,
                     vin.size(), vout.size());
