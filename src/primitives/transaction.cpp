...
JSDescription::JSDescription(bool makeGrothProof, ZCJoinSplit& params, const uint256& joinSplitPubKey, const uint256& anchor,
                             const std::array<libzcash::JSInput, ZC_NUM_JS_INPUTS>& inputs,
                             const std::array<libzcash::JSOutput, ZC_NUM_JS_OUTPUTS>& outputs, CAmount vpub_old,
                             CAmount vpub_new, bool computeProof,
                             uint256* esk  // payment disclosure
                             )
    : vpub_old(vpub_old), vpub_new(vpub_new), anchor(anchor) {
    std::array<libzcash::Note, ZC_NUM_JS_OUTPUTS> notes;

    proof = params.prove(makeGrothProof, inputs, outputs, notes, ciphertexts, ephemeralKey, joinSplitPubKey, randomSeed, macs,
                         nullifiers, commitments, vpub_old, vpub_new, anchor, computeProof,
                         esk  // payment disclosure
    );
}

JSDescription JSDescription::Randomized(bool makeGrothProof, ZCJoinSplit& params, const uint256& joinSplitPubKey,
                                        const uint256& anchor, std::array<libzcash::JSInput, ZC_NUM_JS_INPUTS>& inputs,
    ...
    return JSDescription(makeGrothProof, params, joinSplitPubKey, anchor, inputs, outputs, vpub_old, vpub_new, computeProof,
                         esk  // payment disclosure
    );
}
...
class SproutProofVerifier : public boost::static_visitor<bool> {
  ...
  public:
    SproutProofVerifier(ZCJoinSplit& params, libzcash::ProofVerifier& verifier, const uint256& joinSplitPubKey,
                        const JSDescription& jsdesc)
    ...
    bool operator()(const libzcash::PHGRProof& proof) const {
        return params.verify(proof, verifier, joinSplitPubKey, jsdesc.randomSeed, jsdesc.macs, jsdesc.nullifiers,
                             jsdesc.commitments, jsdesc.vpub_old, jsdesc.vpub_new, jsdesc.anchor);
    }
    ...
    bool operator()(const libzcash::GrothProof& proof) const {
        ...
        return librustzcash_sprout_verify(proof.begin(), jsdesc.anchor.begin(), h_sig.begin(), jsdesc.macs[0].begin(),
                                          jsdesc.macs[1].begin(), jsdesc.nullifiers[0].begin(), jsdesc.nullifiers[1].begin(),
                                          jsdesc.commitments[0].begin(), jsdesc.commitments[1].begin(), jsdesc.vpub_old,
                                          jsdesc.vpub_new);
    }
};

bool JSDescription::Verify(ZCJoinSplit& params, libzcash::ProofVerifier& verifier, const uint256& joinSplitPubKey) const {
...
CTxCeasedSidechainWithdrawalInput::CTxCeasedSidechainWithdrawalInput(const CAmount& nValueIn, const uint256& scIdIn,
                                                                     const CFieldElement& nullifierIn,
                                                                     const uint160& pubKeyHashIn, const CScProof& scProofIn,
                                                                     const CFieldElement& actCertDataHashIn,
                                                                     const CFieldElement& ceasingCumScTxCommTreeIn,
...
std::string CTxCeasedSidechainWithdrawalInput::ToString() const {
    return strprintf(
        ...
        nValue / COIN, nValue % COIN, scId.ToString(), nullifier.GetHexRepr().substr(0, 10), pubKeyHash.ToString(),
        scProof.GetHexRepr().substr(0, 10), actCertDataHash.GetHexRepr().substr(0, 10),
...
//----------------------------------------------------------------------------
uint256 CTxForwardTransferOut::GetHash() const { return SerializeHash(*this); }

std::string CTxForwardTransferOut::ToString() const {
    return strprintf("CTxForwardTransferOut(nValue=%d.%08d, address=%s, scId=%s, mcReturnAddress=%s)", nValue / COIN,
                     nValue % COIN, HexStr(address).substr(0, 30), scId.ToString(), mcReturnAddress.ToString());
}
...
CTxScCreationOut::CTxScCreationOut(const CAmount& nValueIn, const uint256& addressIn, const CAmount& ftScFee,
                                   const CAmount& mbtrScFee, const Sidechain::ScFixedParameters& paramsIn)
    : CTxCrosschainOut(nValueIn, addressIn),
      generatedScId(),
      version(paramsIn.version),
      withdrawalEpochLength(paramsIn.withdrawalEpochLength),
      customData(paramsIn.customData),
      constant(paramsIn.constant),
      wCertVk(paramsIn.wCertVk),
      wCeasedVk(paramsIn.wCeasedVk),
...
uint256 CTxScCreationOut::GetHash() const { return SerializeHash(*this); }

std::string CTxScCreationOut::ToString() const {
    return strprintf(
        "CTxScCreationOut(scId=%s, version=%d, withdrawalEpochLength=%d, "
...
void CTxScCreationOut::GenerateScId(const uint256& txHash, unsigned int pos) const {
    ...
    const std::vector<unsigned char> tmp((uint8_t*)serialized_buffer,
                                         (uint8_t*)serialized_buffer + Sidechain::SC_FE_SIZE_IN_BYTES);
    uint256 scid(tmp);
...
CBwtRequestOut::CBwtRequestOut(const uint256& scIdIn, const uint160& pkhIn, const Sidechain::ScBwtRequestParameters& paramsIn)
    : scId(scIdIn), vScRequestData(paramsIn.vScRequestData), mcDestinationAddress(pkhIn), scFee(paramsIn.scFee) {}

std::string CBwtRequestOut::ToString() const {
    std::string requestDataStr;

    for (auto fe : vScRequestData) requestDataStr += strprintf("\n  [%s]\n", fe.GetHexRepr());

    return strprintf("CBwtRequestOut(scId=%s, vScRequestData=%s, pkh=%s, scFee=%d.%08d", scId.ToString(), requestDataStr,
...
CMutableTransactionBase::CMutableTransactionBase() : nVersion(TRANSPARENT_TX_VERSION), vin(), vout() {}
...
CMutableTransaction::CMutableTransaction(const CTransaction& tx)
    : CMutableTransactionBase(),
      vcsw_ccin(tx.GetVcswCcIn()),
      vsc_ccout(tx.GetVscCcOut()),
      vft_ccout(tx.GetVftCcOut()),
      vmbtr_out(tx.GetVBwtRequestOut()),
      nLockTime(tx.GetLockTime()),
      vjoinsplit(tx.GetVjoinsplit()),
...
bool CMutableTransaction::addOut(const CTxOut& out) {
    vout.push_back(out);
    return true;
}
...
bool CMutableTransaction::add(const CBwtRequestOut& out) {
    vmbtr_out.push_back(out);
    return true;
}

//--------------------------------------------------------------------------------------------------------
CTransactionBase::CTransactionBase(int nVersionIn) : nVersion(nVersionIn), vin(), vout(), hash() {}
...
int CTransactionBase::GetComplexity() const { return vin.size() * vin.size(); }
...
bool CTransaction::CheckSerializedSize(CValidationState& state) const {
    ...
    if (size > MAX_TX_SIZE) {
        LogPrintf("%s():%d - Tx id = %s, size = %d, limit = %d, tx = %s\n", __func__, __LINE__, GetHash().ToString(), size,
                  MAX_TX_SIZE, ToString());
        return state.DoS(100, error("checkSerializedSizeLimits(): size limits failed"), CValidationState::Code::INVALID,
                         "bad-txns-oversize");
    }
...
bool CTransaction::CheckAmounts(CValidationState& state) const {
    ...
    for (const CTxOut& txout : vout) {
        if (txout.nValue < 0)
            return state.DoS(100, error("CheckAmounts(): txout.nValue negative"), CValidationState::Code::INVALID,
                             "bad-txns-vout-negative");
        if (txout.nValue > MAX_MONEY)
            return state.DoS(100, error("CheckAmounts(): txout.nValue too high"), CValidationState::Code::INVALID,
                             "bad-txns-vout-toolarge");
        nCumulatedValueOut += txout.nValue;
        if (!MoneyRange(nCumulatedValueOut))
            return state.DoS(100, error("CheckAmounts(): txout total out of range"), CValidationState::Code::INVALID,
    ...
    for (const JSDescription& joinsplit : vjoinsplit) {
        if (joinsplit.vpub_old < 0) {
            return state.DoS(100, error("CheckAmounts(): joinsplit.vpub_old negative"), CValidationState::Code::INVALID,
                             "bad-txns-vpub_old-negative");
        }

        if (joinsplit.vpub_new < 0) {
            return state.DoS(100, error("CheckAmounts(): joinsplit.vpub_new negative"), CValidationState::Code::INVALID,
                             "bad-txns-vpub_new-negative");
        }

        if (joinsplit.vpub_old > MAX_MONEY) {
            return state.DoS(100, error("CheckAmounts(): joinsplit.vpub_old too high"), CValidationState::Code::INVALID,
                             "bad-txns-vpub_old-toolarge");
        }

        if (joinsplit.vpub_new > MAX_MONEY) {
            return state.DoS(100, error("CheckAmounts(): joinsplit.vpub_new too high"), CValidationState::Code::INVALID,
    ...
    for (const CTxScCreationOut& scOut : vsc_ccout) {
        if (!scOut.CheckAmountRange(nCumulatedValueOut))
            return state.DoS(100, error("%s(): ccout total out of range", __func__), CValidationState::Code::INVALID,
                             "bad-txns-ccout-range");
    }
    ...
    for (const JSDescription& joinsplit : GetVjoinsplit()) {
        ...
        if (!MoneyRange(joinsplit.vpub_new) || !MoneyRange(nCumulatedValueIn)) {
            return state.DoS(100, error("CheckAmounts(): txin total out of range"), CValidationState::Code::INVALID,
...
bool CTransactionBase::CheckInputsDuplication(CValidationState& state) const {
    ...
    for (const CTxIn& txin : GetVin()) {
        if (vInOutPoints.count(txin.prevout))
            return state.DoS(100, error("CheckInputsDuplications(): duplicate inputs"), CValidationState::Code::INVALID,
    ...
    for (const JSDescription& joinsplit : GetVjoinsplit()) {
        for (const uint256& nf : joinsplit.nullifiers) {
            if (vJoinSplitNullifiers.count(nf))
                return state.DoS(100, error("CheckInputsDuplications(): duplicate nullifiers"), CValidationState::Code::INVALID,
...
bool CTransaction::CheckInputsInteraction(CValidationState& state) const {
    if (IsCoinBase()) {
        ...
        if (vjoinsplit.size() > 0)
            return state.DoS(100, error("CheckInputsInteraction(): coinbase has joinsplits"), CValidationState::Code::INVALID,
                             "bad-cb-has-joinsplits");

        if (vin[0].scriptSig.size() < 2 || vin[0].scriptSig.size() > 100)
            return state.DoS(100, error("CheckInputsInteraction(): coinbase script size"), CValidationState::Code::INVALID,
                             "bad-cb-length");
        ...
        if (vcsw_ccin.size() != 0)
            return state.DoS(100, error("CheckInputsInteraction(): coinbase has CSW inputs"), CValidationState::Code::INVALID,
                             "bad-cb-has-cswinputs");
    } else {
        for (const CTxIn& txin : vin)
            if (txin.prevout.IsNull())
                return state.DoS(10, error("CheckInputsInteraction(): prevout is null"), CValidationState::Code::INVALID,
...
CTransaction::CTransaction(const CTransaction& tx)
    : CTransactionBase(tx),
      vjoinsplit(tx.vjoinsplit),
      nLockTime(tx.nLockTime),
      vcsw_ccin(tx.vcsw_ccin),
      vsc_ccout(tx.vsc_ccout),
      vft_ccout(tx.vft_ccout),
      vmbtr_out(tx.vmbtr_out),
      joinSplitPubKey(tx.joinSplitPubKey),
      joinSplitSig(tx.joinSplitSig) {}
...
void CTransaction::UpdateHash() const {
    *const_cast<uint256*>(&hash) = SerializeHash(*this);
    ...
    for (unsigned int pos = 0; pos < vsc_ccout.size(); pos++) vsc_ccout[pos].GenerateScId(hash, pos);
}
...
const uint256& CTransaction::GetScIdFromScCcOut(int pos) const {
    ...
    LogPrint("sc", "%s():%d - tx[%s] has scid[%s] for ccout[%s] (pos[%d])\n", __func__, __LINE__, GetHash().ToString(),
             scid.ToString(), GetVscCcOut().at(pos).GetHash().ToString(), pos);
...
bool CTransaction::IsValidVersion(CValidationState& state) const {
    ...
    if (nVersion < MIN_OLD_TX_VERSION && nVersion != GROTH_TX_VERSION && !IsScVersion()) {
        return state.DoS(100, error("BasicVersionCheck(): version too low"), CValidationState::Code::INVALID,
...
bool CTransaction::CheckInputsOutputsNonEmpty(CValidationState& state) const {
    ...
    if (GetVin().empty() && GetVjoinsplit().empty() && GetVcswCcIn().empty()) {
        ...
        return state.DoS(10, error("CheckNonEmpty(): vin empty"), CValidationState::Code::INVALID, "bad-txns-vin-empty");
    }
    ...
    if (GetVout().empty() && GetVjoinsplit().empty() && ccOutIsNull()) {
        return state.DoS(10, error("CheckNonEmpty(): vout empty"), CValidationState::Code::INVALID, "bad-txns-vout-empty");
    }
...
bool CTransaction::CheckFeeAmount(const CAmount& totalVinAmount, CValidationState& state) const {
    if (!MoneyRange(totalVinAmount))
        return state.DoS(100, error("%s(): total input amount out of range", __func__), CValidationState::Code::INVALID,
                         "bad-txns-inputvalues-outofrange");

    if (!CheckAmounts(state)) return false;

    if (totalVinAmount < GetValueOut())
        return state.DoS(100,
                         error("%s(): %s value in (%s) < value out (%s)", __func__, GetHash().ToString(),
    ...
    if (nTxFee < 0)
        return state.DoS(100, error("%s(): %s nTxFee < 0", __func__, GetHash().ToString()), CValidationState::Code::INVALID,
                         "bad-txns-fee-negative");

    if (!MoneyRange(nTxFee))
        return state.DoS(100, error("%s(): nTxFee out of range", __func__), CValidationState::Code::INVALID,
...
CAmount CTransaction::GetValueOut() const {
    ...
    if (!MoneyRange(nValueOut)) throw std::runtime_error("CTransaction::GetValueOut(): value out of range");
...
std::string CTransaction::ToString() const {
    ...
    if (IsScVersion()) {
        str += strprintf(
            "CTransaction(hash=%s, ver=%d, vin.size=%u, vout.size=%u, vcsw_ccin.size=%u, vsc_ccout.size=%u, vft_ccout.size=%u, "
            "nLockTime=%u)\n",
            GetHash().ToString().substr(0, 10), nVersion, vin.size(), vout.size(), vcsw_ccin.size(), vsc_ccout.size(),
            vft_ccout.size(), nLockTime);

        for (unsigned int i = 0; i < vin.size(); i++) str += "    " + vin[i].ToString() + "\n";
        for (unsigned int i = 0; i < vout.size(); i++) str += "    " + vout[i].ToString() + "\n";
        for (unsigned int i = 0; i < vcsw_ccin.size(); i++) str += "    " + vcsw_ccin[i].ToString() + "\n";
        for (unsigned int i = 0; i < vsc_ccout.size(); i++) str += "    " + vsc_ccout[i].ToString() + "\n";
        for (unsigned int i = 0; i < vft_ccout.size(); i++) str += "    " + vft_ccout[i].ToString() + "\n";
    } else {
...
bool CTransaction::VerifyScript(const CScript& scriptPubKey, unsigned int nFlags, unsigned int nIn, const CChain* chain,
                                bool cacheStore, ScriptError* serror) const {
    return true;
}
std::string CTransaction::EncodeHex() const { return ""; }
void CTransaction::Relay() const {}
std::shared_ptr<const CTransactionBase> CTransaction::MakeShared() const { return std::shared_ptr<const CTransactionBase>(); }
...
bool CTransactionBase::CheckBlockAtHeight(CValidationState& state, int nHeight, int dosLevel) const {
    ...
    for (int pos = 0; pos < GetVout().size(); ++pos) {
        ...
        if (IsBackwardTransfer(pos)) continue;
...
bool CTransaction::ContextualCheck(CValidationState& state, int nHeight, int dosLevel) const {
    if (!CheckBlockAtHeight(state, nHeight, dosLevel)) return false;
    ...
    if (isGROTHActive) {
        ...
        if (nVersion == TRANSPARENT_TX_VERSION || (areSidechainsSupported && (nVersion == sidechainTxVersion))) {
            ...
            for (const CTxScCreationOut& scCreationOutput : vsc_ccout) {
                if (scCreationOutput.version > maxSidechainVersionAllowed) {
                    return state.DoS(
                        dosLevel,
                        error("ContextualCheck(): transparent or sc tx but sc creation output has wrong sidechain version"),
        ...
        if (nVersion != GROTH_TX_VERSION) {
            ...
            return state.DoS(dosLevel, error("ContextualCheck(): unexpected tx version"), CValidationState::Code::INVALID,
                             "bad-tx-version-unexpected");
        }
        return true;
    } else {
        // sidechain fork is after groth one
        ...
        if (nVersion < TRANSPARENT_TX_VERSION) {
            LogPrintf(
                "%s():%d - rejecting (ver=%d) transaction at block height %d - groth_active[%d] (shieldedTxVersion=%d), "
                "sidechain_active[%d]\n",
                __func__, __LINE__, nVersion, nHeight, (int)isGROTHActive, shieldedTxVersion, (int)areSidechainsSupported);
            return state.DoS(0, error("ContextualCheck(): unexpected tx version"), CValidationState::Code::INVALID,
                             "bad-tx-version-unexpected");
        }
        return true;
    }
...
void CTransaction::AddJoinSplitToJSON(UniValue& entry) const { entry.pushKV("vjoinsplit", TxJoinSplitToJSON(*this)); }
...
void CTransaction::AddSidechainOutsToJSON(UniValue& entry) const { Sidechain::AddSidechainOutsToJSON(*this, entry); }
...
bool CTransaction::VerifyScript(const CScript& scriptPubKey, unsigned int nFlags, unsigned int nIn, const CChain* chain,
    ...
    if (!::VerifyScript(scriptSig, scriptPubKey, nFlags, CachingTransactionSignatureChecker(this, nIn, chain, cacheStore),
                        serror)) {
...
std::string CTransaction::EncodeHex() const { return EncodeHexTx(*this); }
