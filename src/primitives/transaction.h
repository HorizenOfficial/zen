...
static_assert(GROTH_TX_VERSION < MIN_OLD_TX_VERSION, "Groth tx version must be lower than minimum");

static_assert(PHGR_TX_VERSION >= MIN_OLD_TX_VERSION, "PHGR tx version must not be lower than minimum");

static_assert(TRANSPARENT_TX_VERSION >= MIN_OLD_TX_VERSION, "TRANSPARENT tx version must not be lower than minimum");

// Many static casts to int * of Tx nVersion (int32_t *) are performed. Verify at compile time that they are equivalent.
static_assert(sizeof(int32_t) == sizeof(int),
              "int size differs from 4 bytes. This may lead to unexpected behaviors on static casts");
...
class SproutProofSerializer : public boost::static_visitor<> {
  ...
  public:
    SproutProofSerializer(Stream& s, bool useGroth, int nType, int nVersion)
        : s(s), useGroth(useGroth), nType(nType), nVersion(nVersion) {}
...
inline void SerReadWriteSproutProof(Stream& s, const T& proof, bool useGroth, CSerActionSerialize ser_action, int nType,
                                    int nVersion) {
...
template <typename Stream, typename T>
inline void SerReadWriteSproutProof(Stream& s, T& proof, bool useGroth, CSerActionUnserialize ser_action, int nType,
                                    int nVersion) {
    if (useGroth) {
...
class JSDescription {
  public:
    ...
    JSDescription(bool makeGrothProof, ZCJoinSplit& params, const uint256& joinSplitPubKey, const uint256& rt,
                  const std::array<libzcash::JSInput, ZC_NUM_JS_INPUTS>& inputs,
                  const std::array<libzcash::JSOutput, ZC_NUM_JS_OUTPUTS>& outputs, CAmount vpub_old, CAmount vpub_new,
                  bool computeProof = true,  // Set to false in some tests
                  uint256* esk = nullptr     // payment disclosure
    );

    static JSDescription Randomized(bool makeGrothProof, ZCJoinSplit& params, const uint256& joinSplitPubKey, const uint256& rt,
    ...
    bool Verify(ZCJoinSplit& params, libzcash::ProofVerifier& verifier, const uint256& joinSplitPubKey) const;
    ...
    friend bool operator==(const JSDescription& a, const JSDescription& b) {
        return (a.vpub_old == b.vpub_old && a.vpub_new == b.vpub_new && a.anchor == b.anchor && a.nullifiers == b.nullifiers &&
                a.commitments == b.commitments && a.ephemeralKey == b.ephemeralKey && a.ciphertexts == b.ciphertexts &&
                a.randomSeed == b.randomSeed && a.macs == b.macs && a.proof == b.proof);
    }

    friend bool operator!=(const JSDescription& a, const JSDescription& b) { return !(a == b); }
};
...
class BaseOutPoint {
  public:
    ...
    friend bool operator==(const BaseOutPoint& a, const BaseOutPoint& b) { return (a.hash == b.hash && a.n == b.n); }

    friend bool operator!=(const BaseOutPoint& a, const BaseOutPoint& b) { return !(a == b); }
};
...
class CTxIn {
  public:
    ...
    explicit CTxIn(const COutPoint& prevoutIn, const CScript& scriptSigIn = CScript(),
                   uint32_t nSequenceIn = std::numeric_limits<unsigned int>::max());
    CTxIn(const uint256& hashPrevTx, const uint32_t& nOut, const CScript& scriptSigIn = CScript(),
...
class CTxCeasedSidechainWithdrawalInput {
  public:
    ...
    explicit CTxCeasedSidechainWithdrawalInput(const CAmount& nValueIn, const uint256& scIdIn, const CFieldElement& nullifierIn,
    ...
    friend bool operator==(const CTxCeasedSidechainWithdrawalInput& a, const CTxCeasedSidechainWithdrawalInput& b) {
        return (a.nValue == b.nValue && a.scId == b.scId && a.nullifier == b.nullifier && a.pubKeyHash == b.pubKeyHash &&
                a.scProof == b.scProof && a.actCertDataHash == b.actCertDataHash &&
                a.ceasingCumScTxCommTree == b.ceasingCumScTxCommTree && a.redeemScript == b.redeemScript);
    }
...
class CTxOut {
  public:
    ...
    CTxOut(const CAmount& nValueIn, CScript scriptPubKeyIn) : nValue(nValueIn), scriptPubKey(scriptPubKeyIn) {}
...
class CTxCrosschainOut : public CTxCrosschainOutBase {
  public:
    ...
    CTxCrosschainOut(const CAmount& nValueIn, const uint256& addressIn) : nValue(nValueIn), address(addressIn) {}
...
class CTxForwardTransferOut : public CTxCrosschainOut {
  public:
    ...
    CTxForwardTransferOut(const uint256& scIdIn, const CAmount& nValueIn, const uint256& addressIn,
                          const uint160& mcReturnAddressIn)
    ...
    friend bool operator!=(const CTxForwardTransferOut& a, const CTxForwardTransferOut& b) { return !(a == b); }
};
...
class CTxScCreationOut : public CTxCrosschainOut {
  ...
  public:
     ...
     * Due to the constraints of the consensus logic, we are sure that the most significant byte of the WithdrawalEpochLength
     * was always zero before hard fork 9.
     *
    ...
    CTxScCreationOut(const CAmount& nValueIn, const uint256& addressIn, const CAmount& ftScFee, const CAmount& mbtrScFee,
    ...
    friend bool operator==(const CTxScCreationOut& a, const CTxScCreationOut& b) {
        return isBaseEqual(a, b) && a.generatedScId == b.generatedScId && a.withdrawalEpochLength == b.withdrawalEpochLength &&
               a.customData == b.customData && a.constant == b.constant && a.wCertVk == b.wCertVk &&
               a.wCeasedVk == b.wCeasedVk && a.vFieldElementCertificateFieldConfig == b.vFieldElementCertificateFieldConfig &&
    ...
    friend bool operator!=(const CTxScCreationOut& a, const CTxScCreationOut& b) { return !(a == b); }
};
...
class CBwtRequestOut : public CTxCrosschainOutBase {
  public:
    ...
    friend bool operator==(const CBwtRequestOut& a, const CBwtRequestOut& b) {
        return (a.scId == b.scId && a.vScRequestData == b.vScRequestData && a.mcDestinationAddress == b.mcDestinationAddress &&
                a.scFee == b.scFee);
    }

    friend bool operator!=(const CBwtRequestOut& a, const CBwtRequestOut& b) { return !(a == b); }
...
class CTransactionBase {
  ...
  public:
    ...
    friend bool operator==(const CTransactionBase& a, const CTransactionBase& b) { return a.hash == b.hash; }

    friend bool operator!=(const CTransactionBase& a, const CTransactionBase& b) { return !(a == b); }
    ...
    virtual bool VerifyScript(const CScript& scriptPubKey, unsigned int flags, unsigned int nIn, const CChain* chain,
    ...
    static bool IsCertificate(int nVersion) { return (nVersion == SC_CERT_VERSION); }

    static bool IsTransaction(int nVersion) { return !IsCertificate(nVersion); }
};
...
class CTransaction : public CTransactionBase {
  ...
  public:
    ...
    static_assert(MIN_OLD_VERSION >= MIN_OLD_TX_VERSION, "standard rule for tx version should be consistent with network rule");
  ...
  public:
    ...
    bool IsNull() const override {
        ...
        if (IsScVersion()) ret = ret && ccIsNull();

        return ret;
    }

    bool ccIsNull() const { return vcsw_ccin.empty() && ccOutIsNull(); }

    bool ccOutIsNull() const { return vsc_ccout.empty() && vft_ccout.empty() && vmbtr_out.empty(); }
  ...
  public:
    ...
    bool VerifyScript(const CScript& scriptPubKey, unsigned int flags, unsigned int nIn, const CChain* chain, bool cacheStore,
