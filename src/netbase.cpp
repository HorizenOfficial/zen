...
void SplitHostPort(std::string in, int& portOut, std::string& hostOut) {
    ...
    bool fBracketed =
        fHaveColon &&
        (in[0] == '[' && in[colon - 1] == ']');  // if there is a colon, and in[0]=='[', colon is not 0, so in[colon-1] is safe
...
bool LookupNumeric(const char* pszName, CService& addr, int portDefault) { return Lookup(pszName, addr, portDefault, false); }
...
static bool Socks5(const std::string& strDest, int port, const ProxyCredentials* auth, SOCKET& hSocket) {
    ...
    if (pchRet1[1] == 0x02 && auth) {
        ...
        if (auth->username.size() > 255 || auth->password.size() > 255) return error("Proxy username or password too long");
...
static bool ConnectThroughProxy(const proxyType& proxy, const std::string& strDest, int port, SOCKET& hSocketRet, int nTimeout,
...
void CNetAddr::Init() { memset(ip, 0, sizeof(ip)); }

void CNetAddr::SetIP(const CNetAddr& ipIn) { memcpy(ip, ipIn.ip, sizeof(ip)); }
...
CNetAddr::CNetAddr(const struct in_addr& ipv4Addr) { SetRaw(NET_IPV4, (const uint8_t*)&ipv4Addr); }

CNetAddr::CNetAddr(const struct in6_addr& ipv6Addr) { SetRaw(NET_IPV6, (const uint8_t*)&ipv6Addr); }
...
bool CNetAddr::IsIPv6() const { return (!IsIPv4() && !IsTor()); }
...
bool CNetAddr::IsTor() const { return (memcmp(ip, pchOnionCat, sizeof(pchOnionCat)) == 0); }
...
std::vector<unsigned char> CNetAddr::GetGroup() const {
    ...
    if (nBits > 0) vchRet.push_back(GetByte(15 - nStartByte) | ((1 << (8 - nBits)) - 1));
        ...
        if (slash != strSubnet.npos) {
            ...
            if (ParseInt32(strNetmask, &n))  // If valid number, assume /24 symtex
            {
                if (n >= 0 && n <= (128 - astartofs * 8))  // Only valid if in range of bits of address
                {
                    n += astartofs * 8;
                    // Clear bits [n..127]
                    for (; n < 128; ++n) netmask[n >> 3] &= ~(1 << (7 - (n & 7)));
                } else {
                    valid = false;
                }
            } else  // If not a valid number, try full netmask syntax
            {
                if (LookupHost(strNetmask.c_str(), vIP, 1, false))  // Never allow lookup for netmask
                {
                    ...
                    for (int x = astartofs; x < 16; ++x) netmask[x] = vIP[0].ip[x];
                } else {
                    valid = false;
                }
            }
...
std::string CSubNet::ToString() const {
    ...
    if (network.IsIPv4())
        ...
        strNetmask = strprintf("%x:%x:%x:%x:%x:%x:%x:%x", netmask[0] << 8 | netmask[1], netmask[2] << 8 | netmask[3],
...
std::string NetworkErrorString(int err) {
    char buf[256];
    buf[0] = 0;
    if (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, NULL, err,
