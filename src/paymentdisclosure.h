...
struct PaymentDisclosureInfo {
    ...
    PaymentDisclosureInfo(uint8_t v, uint256 esk, uint256 key, libzcash::PaymentAddress zaddr)
        : version(v), esk(esk), joinSplitPrivKey(key), zaddr(zaddr) {}
    ...
    friend bool operator!=(const PaymentDisclosureInfo& a, const PaymentDisclosureInfo& b) { return !(a == b); }
};
...
struct PaymentDisclosurePayload {
    ...
    friend bool operator==(const PaymentDisclosurePayload& a, const PaymentDisclosurePayload& b) {
        return (a.version == b.version && a.esk == b.esk && a.txid == b.txid && a.js == b.js && a.n == b.n &&
                a.zaddr == b.zaddr && a.message == b.message);
    }

    friend bool operator!=(const PaymentDisclosurePayload& a, const PaymentDisclosurePayload& b) { return !(a == b); }
};
...
struct PaymentDisclosure {
    ...
    PaymentDisclosure(){};
    PaymentDisclosure(const PaymentDisclosurePayload payload, const std::array<unsigned char, 64> sig)
        : payload(payload), payloadSig(sig){};
    PaymentDisclosure(const uint256& joinSplitPubKey, const PaymentDisclosureKey& key, const PaymentDisclosureInfo& info,
    ...
    friend bool operator!=(const PaymentDisclosure& a, const PaymentDisclosure& b) { return !(a == b); }
};
