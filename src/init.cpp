...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    // Do not translate _(...) -help-debug options, Many technical terms, and only a very small audience, so is unnecessary
    // stress to translators
    ...
    strUsage += HelpMessageOpt("-checklevel=<n>",
                               strprintf(_("How thorough the block verification of -checkblocks is (0-4, default: %u)"), 3));
    ...
    strUsage += HelpMessageOpt(
        "-disabledeprecation=<version>",
        strprintf(_("Disable block-height node deprecation and automatic shutdown (example: -disabledeprecation=%s)"),
                  FormatVersion(CLIENT_VERSION)));
    strUsage += HelpMessageOpt("-exportdir=<dir>", _("Specify directory to be used when exporting data"));
    strUsage += HelpMessageOpt("-dbcache=<n>", strprintf(_("Set database cache size in megabytes (%d to %d, default: %d)"),
    ...
    strUsage += HelpMessageOpt(
        "-maxorphantx=<n>",
        strprintf(_("Keep at most <n> unconnectable transactions in memory (default: %u)"), DEFAULT_MAX_ORPHAN_TRANSACTIONS));
    strUsage += HelpMessageOpt("-mempooltxinputlimit=<n>", _("Set the maximum number of transparent inputs in a transaction "
                                                             "that the mempool will accept (default: 0 = no limit applied)"));
    strUsage += HelpMessageOpt("-par=<n>", strprintf(_("Set the number of script verification threads (%u to %d, 0 = auto, <0 "
...
#endif
    strUsage += HelpMessageOpt(
        "-prune=<n>", strprintf(_("Reduce storage requirements by pruning (deleting) old blocks. This mode disables wallet "
                                  "support and is incompatible with -txindex. "
                                  "Warning: Reverting this setting requires re-downloading the entire blockchain. "
                                  "(default: 0 = disable pruning blocks, >%u = target size in MiB to use for block files)"),
                                MIN_DISK_SPACE_FOR_BLOCK_FILES / 1024 / 1024));
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files on startup"));
    strUsage +=
        HelpMessageOpt("-reindexfast", _("Rebuild block chain index from current blk000??.dat files on startup, skipping "
                                         "expensive checks for blocks below checkpoints. It is incompatible with reindex"));
#if !defined(WIN32)
    strUsage += HelpMessageOpt("-sysperms", _("Create new files with system default permissions, instead of umask 077 (only "
                                              "effective with disabled wallet functionality)"));
#endif
    strUsage += HelpMessageOpt(
        "-txindex", strprintf(_("Maintain a full transaction index, used by the getrawtransaction rpc call (default: %u)"), 0));
    strUsage +=
        HelpMessageOpt("-maturityheightindex",
                       strprintf(_("Maintain a maturity height index that stores for every height the cerficates that became "
                                   "mature, used by the getblockexpanded rpc call. It requires -txindex (default: %u)"),
                                 0));

#ifdef ENABLE_ADDRESS_INDEXING
    strUsage += HelpMessageOpt("-addressindex", strprintf(_("Maintain a full address index, used to query for the balance, "
                                                            "txids and unspent outputs for addresses (default: %u)"),
                                                          DEFAULT_ADDRESSINDEX));
    strUsage += HelpMessageOpt("-timestampindex", strprintf(_("Maintain a timestamp index for block hashes, used to query "
                                                              "blocks hashes by a range of timestamps (default: %u)"),
                                                            DEFAULT_TIMESTAMPINDEX));
    strUsage += HelpMessageOpt(
        "-spentindex",
        strprintf(
            _("Maintain a full spent index, used to query the spending txid and input index for an outpoint (default: %u)"),
            DEFAULT_SPENTINDEX));

    strUsage += HelpMessageOpt(
        "-blocktreedbmaxopenfiles",
        strprintf(_("Maximum number of open files for the Block Tree LevelDB (default: %u)"), DEFAULT_DB_MAX_OPEN_FILES));
    strUsage +=
        HelpMessageOpt("-blocktreedbcompression",
                       strprintf(_("Enable compression for the Block Tree LevelDB (default: %u)"), DEFAULT_DB_COMPRESSION));
    ...
    strUsage += HelpMessageOpt(
        "-bantime=<n>", strprintf(_("Number of seconds to keep misbehaving peers from reconnecting (default: %u)"), 86400));
    strUsage +=
    ...
    strUsage += HelpMessageOpt("-listenonion",
                               strprintf(_("Automatically create Tor hidden service (default: %d)"), DEFAULT_LISTEN_ONION));
    strUsage += HelpMessageOpt("-maxconnections=<n>", strprintf(_("Maintain at most <n> connections to peers (default: %u)"),
    ...
    strUsage += HelpMessageOpt("-port=<port>",
                               strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 9033, 19033));
    strUsage += HelpMessageOpt("-proxy=<ip:port>", _("Connect through SOCKS5 proxy"));
    strUsage += HelpMessageOpt(
        "-proxyrandomize",
        strprintf(_("Randomize credentials for every proxy connection. This enables Tor stream isolation (default: %u)"), 1));
    strUsage += HelpMessageOpt("-seednode=<ip>", _("Connect to a node to retrieve peer addresses, and disconnect"));
    strUsage += HelpMessageOpt(
        "-timeout=<n>",
        strprintf(_("Specify connection timeout in milliseconds (minimum: 1, default: %d)"), DEFAULT_CONNECT_TIMEOUT));
    strUsage +=
        HelpMessageOpt("-torcontrol=<ip>:<port>",
                       strprintf(_("Tor control port to use if onion listening enabled (default: %s)"), DEFAULT_TOR_CONTROL));
    strUsage += HelpMessageOpt("-torpassword=<pass>", _("Tor control port password (default: empty)"));
    strUsage +=
        HelpMessageOpt("-tlsfallbacknontls=<0 or 1>", _("If a TLS connection fails, the next connection attempt of the same "
                                                        "peer (based on IP address) takes place without TLS (default: 1)"));
    ...
    strUsage += HelpMessageOpt(
        "-tlskeypwd=<password>",
        _("Password for a private key encryption (default: not set, i.e. private key will be stored unencrypted)"));
    ...
    strUsage += HelpMessageOpt("-websocket=<0 or 1>",
                               _("If set to 1 opens a websocket channel listening for client connections (default: 0)"));
    strUsage += HelpMessageOpt("-wsaddress=<ip address>",
                               _("If websocket=1, listen for ws connections at this ip address (default: 127.0.0.1)"));
    strUsage += HelpMessageOpt("-wsport=<port>",
                               _("If websocket=1, listen for ws connections at <wsaddress>:<wsport> (default: 8888)"));
#ifdef USE_UPNP
#if USE_UPNP
    ...
    strUsage +=
        HelpMessageOpt("-txconfirmtarget=<n>", strprintf(_("If paytxfee is not set, include enough fee so transactions begin "
                                                           "confirmation on average within n blocks (default: %u)"),
                                                         DEFAULT_TX_CONFIRM_TARGET));
    strUsage +=
        HelpMessageOpt("-maxtxfee=<amt>", strprintf(_("Maximum total fees (in %s) to use in a single wallet transaction; "
    ...
    strUsage += HelpMessageOpt("-walletbroadcast",
                               _("Make the wallet broadcast transactions") + " " + strprintf(_("(default: %u)"), true));
    ...
    if (showDebug) {
        strUsage += HelpMessageOpt("-checkpoints",
                                   strprintf("Disable expensive verification for known chain history (default: %u)", 1));
        strUsage += HelpMessageOpt(
            "-dblogsize=<n>",
            strprintf("Flush database activity from memory pool to disk log every <n> megabytes (default: %u)", 100));
        strUsage +=
        ...
        strUsage += HelpMessageOpt("-stopafterblockimport",
                                   strprintf("Stop running after importing blocks from disk (default: %u)", 0));
    }
    string debugCategories =
        "addrman, alert, bench, cert, coindb, db, estimatefee, fork, http, libevent, lock, mempool, net, partitioncheck, pow, "
        "proxy, prune, "
        "rand, reindex, rpc, sc, selectcoins, tor, ws, zendoo_mc_cryptolib, zmq, zrpc, zrpcunsafe (implies zrpc)";  // Don't
                                                                                                                    // translate
                                                                                                                    // these
    strUsage +=
    ...
    strUsage += HelpMessageOpt(
        "-logtimemicros",
        strprintf(_("Meaningful if -logtimestamps=1. In debug output timestamp reports microseconds (default: %u)"), 0));
    if (showDebug) {
        strUsage += HelpMessageOpt(
            "-limitfreerelay=<n>",
            strprintf("Continuously rate-limit free transactions to <n>*1000 bytes per minute (default: %u)", 15));
        strUsage += HelpMessageOpt(
            "-relaypriority", strprintf("Require high priority for relaying free or low-fee transactions (default: %u)", 0));
        strUsage += HelpMessageOpt("-maxsigcachesize=<n>",
                                   strprintf("Limit size of signature cache to <n> entries (default: %u)", 50000));
    }
    strUsage +=
        HelpMessageOpt("-minrelaytxfee=<amt>",
                       strprintf(_("Fees (in %s/kB) smaller than this are considered zero fee for relaying (default: %s)"),
                                 CURRENCY_UNIT, FormatMoney(::minRelayTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-printtoconsole", _("Send trace/debug info to console instead of debug.log file"));
    if (showDebug) {
        strUsage += HelpMessageOpt("-printpriority",
                                   strprintf("Log transaction priority and fee per kB when mining blocks (default: %u)", 0));
        strUsage +=
            HelpMessageOpt("-privdb", strprintf("Sets the DB_PRIVATE flag in the wallet db environment (default: %u)", 1));
        strUsage +=
            HelpMessageOpt("-regtest",
    ...
    strUsage += HelpMessageOpt(
        "-datacarriersize",
        strprintf(_("Maximum size of data in data carrier transactions we relay and mine (default: %u)"), MAX_OP_RETURN_RELAY));
    ...
    strUsage += HelpMessageOpt("-blocktxpartitionmaxsize=<n> (regtest only)",
                               strprintf(_("Set maximum partition block size for transcations in bytes (default: %u)"),
                                         DEFAULT_BLOCK_TX_PART_MAX_SIZE));

    strUsage += HelpMessageOpt(
        "-blockprioritysize=<n>",
        strprintf(_("Set maximum size of high-priority/low-fee transactions/certificates in bytes (default: %d)"),
    ...
    strUsage += HelpMessageOpt("-blockmaxcomplexity=<n>",
                               strprintf(_("Limit transactions to be included into blocks based on block complexity. "
                                           " Block complexity is the sum of transaction complexity per block. Transaction "
                                           "complexity is the number of inputs of a transaction squared. "
                                           " Like -mempooltxinputlimit this switch is intended as a last resort when unable to "
                                           "build blocks fast enough because of poor GBT performance. "
                                           " 0  or negative values means no limit is applied. (default: %d)"),
                                         DEFAULT_BLOCK_MAX_COMPLEXITY_SIZE));
    strUsage +=
        HelpMessageOpt("-deprecatedgetblocktemplate",
                       (_("Disable block complexity calculation and use the previous GetBlockTemplate implementation")));

    strUsage += HelpMessageOpt(
        "-scproofverificationdelay=<time>",
        strprintf(_("The maximum delay in milliseconds between sc proof batch verification requests. (default: %d)"),
                  CScAsyncProofVerifier::BATCH_VERIFICATION_MAX_DELAY));

    strUsage += HelpMessageOpt(
        "-scproofqueuesize=<size>",
        strprintf(_("The threshold size of the sc proof queue that triggers a call to the batch verification. (default: %d)"),
                  CScAsyncProofVerifier::BATCH_VERIFICATION_MAX_SIZE));

    strUsage += HelpMessageOpt("-cbhsafedepth=<n>",
                               "regtest only - Set safe depth for skipping checkblockatheight in txout scripts (default "
                               "depends on regtest/testnet params)");

    strUsage += HelpMessageOpt("-cbhminage=<n>",
                               "regtest only - Set the minimum legal age of the referenced block for checkblockatheight in "
                               "txout scripts (default depends on regtest/testnet params)");
    ...
    strUsage +=
        HelpMessageOpt("-allowdustoutput",
                       "regtest only - when checking a tx to be standard, regtest allows by default a tx to have a null or "
                       "dust output. Setting this option to 0 will prevent that (default: 1 = allow dust output)");

    strUsage += HelpMessageOpt("-subsidyhalvinginterval=<n>",
                               "regtest only - Set halving interval for testing purposes (default=2000; must be > 100)");

    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-blockversion=<n>", "Override block version to test forking scenarios");

    strUsage += HelpMessageOpt("-sccoinsmaturity=<n>",
                               "regtest only - Set the maturity of sc funds as number of blocks to be mined before they are "
                               "computed in the sc balance (default depends on regtest/testnet params)");

    strUsage += HelpMessageOpt(
        "-blocksforscfeecheck=<n>",
        "regtest only - Set the number of blocks used for computing the number of epoch to go back for getting the active data "
        "cert sc fee value and check on it for removing a FT or mbtr tx from mempool (default depends on regtest params)");

    strUsage += HelpMessageOpt("-skipscproof",
                               "regtest only - Skip the proof verification for sidechain certificates or CSW transactions (by "
                               "default it is never skipped)");

    strUsage += HelpMessageOpt(
        "-forcelocalban", "regtest only - Override the default behavior that prevents the ban of a misbehaving local node");
    ...
    strUsage +=
        HelpMessageOpt("-equihashsolver=<name>", _("Specify the Equihash solver to be used if enabled (default: \"default\")"));
    strUsage += HelpMessageOpt("-mineraddress=<addr>", _("Send mined coins to a specific single address"));
    strUsage += HelpMessageOpt(
        "-minetolocalwallet", strprintf(_("Require that mined blocks use a coinbase address in the local wallet (default: %u)"),
#ifdef ENABLE_WALLET
                                        1
#else
                                        0
    ...
    strUsage += HelpMessageOpt("-rpcbind=<addr>",
                               _("Bind to given address to listen for JSON-RPC connections. Use [host]:port notation for IPv6. "
                                 "This option can be specified multiple times (default: bind to all interfaces)"));
    strUsage += HelpMessageOpt("-rpcuser=<user>", _("Username for JSON-RPC connections"));
    strUsage += HelpMessageOpt("-rpcpassword=<pw>", _("Password for JSON-RPC connections"));
    strUsage += HelpMessageOpt(
        "-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8231, 18231));
    strUsage += HelpMessageOpt(
        "-rpcallowip=<ip>",
        _("Allow JSON-RPC connections from specified source. Valid for <ip> are a single IP (e.g. 1.2.3.4), a network/netmask "
          "(e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24). This option can be specified multiple times"));
    strUsage += HelpMessageOpt(
        "-rpcthreads=<n>", strprintf(_("Set the number of threads to service RPC calls (default: %d)"), DEFAULT_HTTP_THREADS));
    if (showDebug) {
        strUsage += HelpMessageOpt(
            "-rpcworkqueue=<n>",
    ...
    // strUsage += HelpMessageOpt("-rpcasyncthreads=<n>", strprintf(_("Set the number of threads to service Async RPC calls
    // (default: %d)"), 1));

    if (mode == HMM_BITCOIND) {
        strUsage += HelpMessageGroup(_("Metrics Options (only if -daemon and -printtoconsole are not set):"));
        strUsage +=
            HelpMessageOpt("-showmetrics", _("Show metrics on stdout (default: 1 if running in a console, 0 otherwise)"));
        strUsage += HelpMessageOpt("-metricsui", _("Set to 1 for a persistent metrics screen, 0 for sequential metrics output "
                                                   "(default: 1 if running in a console, 0 otherwise)"));
        strUsage += HelpMessageOpt(
            "-metricsrefreshtime",
            strprintf(_("Number of seconds between metrics refreshes (default: %u if running in a console, %u otherwise)"), 1,
                      600));
    }
...
void CleanupBlockRevFiles() {
    ...
    for (const auto& [first, second] : mapBlockFiles) {
        if (atoi(first) == nContigCounter) {
            nContigCounter++;
            continue;
        }
        remove(second);
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles) {
    ...
    if (fReindex || fReindexFast) {
        ...
        if (fReindexFast) LogPrintf("Headers-only reindexing finished. Going on with blocks\n");
...
void ThreadNotifyRecentlyAdded() {
    while (true) {
        ...
        auto nextFire = std::chrono::duration_cast<std::chrono::seconds>(now + std::chrono::seconds(1));
        std::this_thread::sleep_until(std::chrono::time_point<std::chrono::steady_clock>(nextFire));
...
static void ZC_LoadParams() {
    ...
    if (!(boost::filesystem::exists(pk_path) && boost::filesystem::exists(vk_path) &&
          boost::filesystem::exists(sapling_spend) && boost::filesystem::exists(sapling_output) &&
          boost::filesystem::exists(sprout_groth16))) {
        uiInterface.ThreadSafeMessageBox(
            strprintf(_("Cannot find the Horizen network parameters in the following directory:\n"
                        "%s\n"
...
bool AppInitServers() {
    ...
    if (GetBoolArg("-websocket", false) && !StartWsServer()) return false;
    return true;
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-bind")) {
        if (SoftSetBoolArg("-listen", true)) LogPrintf("%s: parameter interaction: -bind set -> setting -listen=1\n", __func__);
    }
    ...
    if (GetArg("-prune", 0)) {
        if (GetBoolArg("-txindex", false)) return InitError(_("Prune mode is incompatible with -txindex."));
#ifdef ENABLE_WALLET
        if (!GetBoolArg("-disablewallet", false)) {
    ...
    if ((Params().NetworkIDString() == "regtest") && mapArgs.count("-subsidyhalvinginterval")) {
        ...
        if (val != Params().GetConsensus().nSubsidyHalvingInterval) {
            ...
            if (val <= targetFork.getHeight(CBaseChainParams::REGTEST)) assert(!"subsidyhalvinginterval too low");

            Params(CBaseChainParams::REGTEST).SetSubsidyHalvingInterval(val);
            LogPrintf("%s():%d - %s: set nSubsidyHalvingInterval = %d) \n", __func__, __LINE__, Params().NetworkIDString(),
                      Params().GetConsensus().nSubsidyHalvingInterval);
        }
    }
    ...
    // Check for -debugnet
    if (GetBoolArg("-debugnet", false)) InitWarning(_("Warning: Unsupported argument -debugnet ignored, use -debug=net."));
    // Check for -socks - as this is a privacy risk to continue, exit here
    if (mapArgs.count("-socks"))
        return InitError(
            _("Error: Unsupported argument -socks found. Setting SOCKS version isn't possible anymore, only SOCKS5 proxies are "
              "supported."));
    // Check for -tor - as this is a privacy risk to continue, exit here
    if (GetBoolArg("-tor", false)) return InitError(_("Error: Unsupported argument -tor found, use -onion."));

    if (GetBoolArg("-benchmark", false)) InitWarning(_("Warning: Unsupported argument -benchmark ignored, use -debug=bench."));
    ...
    if (nPruneTarget) {
        if (nPruneTarget < MIN_DISK_SPACE_FOR_BLOCK_FILES) {
            return InitError(strprintf(_("Prune configured below the minimum of %d MB.  Please use a higher number."),
                                       MIN_DISK_SPACE_FOR_BLOCK_FILES / 1024 / 1024));
        }
    ...
    if (mapArgs.count("-maxtxfee")) {
        ...
        if (CFeeRate(maxTxFee, 1000) < ::minRelayTxFee) {
            return InitError(strprintf(_("Invalid amount for -maxtxfee=<amount>: '%s' (must be at least the minrelay fee of %s "
    ...
    if (mapArgs.count("-mineraddress")) {
        ...
        if (!addr.SetString(mapArgs["-mineraddress"])) {
            return InitError(strprintf(_("Invalid address for -mineraddress=<addr>: '%s' (must be a transparent address)"),
    ...
    // Sanity check
    if (!InitSanityCheck()) return InitError(_("Initialization sanity check failed. Zen is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Zen is probably already running."),
                                       GetDataDir().string()));

    } catch (const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Zen is probably already running.") + " %s.",
                                   GetDataDir().string(), e.what()));
    }

    // Initialize sidechains folder, hosting keys for sidechains validations
    if (!Sidechain::InitSidechainsFolder()) return InitError(strprintf(_("Cannot create or access sidechains folder.")));

    // Initialize DLog keys
    if (!Sidechain::InitDLogKeys()) return InitError(strprintf(_("Cannot initialize DLog keys in sidechains folder.")));
    ...
    // This print might not be correct, since at this point the options in the the zen.conf have already been read, and if a
    // custom datadir has ben specified, the config file would be erroneously printed to be there
    ...
    if ((chainparams.NetworkIDString() != "regtest") && GetBoolArg("-showmetrics", isatty(STDOUT_FILENO)) && !fPrintToConsole &&
    ...
    if (!fDisableWallet) {
        ...
        if (!CWallet::Verify(strWalletFile, warningString, errorString)) return false;

        if (!warningString.empty()) InitWarning(warningString);
        if (!errorString.empty()) return InitError(warningString);
    ...
    if (proxyArg != "" && proxyArg != "0") {
        ...
        if (!addrProxy.IsValid()) return InitError(strprintf(_("Invalid -proxy address: '%s'"), proxyArg));
    ...
    if (onionArg != "") {
        if (onionArg == "0") {    // Handle -noonion/-onion=0
            ...
            if (!addrOnion.IsValid()) return InitError(strprintf(_("Invalid -onion address: '%s'"), onionArg));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                pblocktree =
                    new CBlockTreeDB(nBlockTreeDBCache, false, fReindex || fReindexFast, dbCompression, dbMaxOpenFiles);
                ...
                if (fHavePruned && !fPruneMode) {
                    strLoadError =
                        _("You need to rebuild the database using -reindex or -reindexfast to go back to unpruned mode.  This "
        ...
        if (!fLoaded) {
            ...
            if (!fReset) {
                bool fRet =
                    uiInterface.ThreadSafeQuestion(strLoadError + ".\n\n" + _("Do you want to rebuild the block database now?"),
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                string msg(
                    _("error reading wallet.dat! All keys read correctly, but transaction data"
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(
            _("Horizen was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Horizen "
              "with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(
            _("Horizen was not built with wallet support. Set -mineraddress, or rebuild Horizen with wallet support."));
    }
#endif  // !ENABLE_WALLET
    ...
    if (mapArgs.count("-mineraddress")) {
        ...
        if (GetBoolArg("-minetolocalwallet", true) && !minerAddressInLocalWallet) {
            return InitError(
                _("-mineraddress is not in the local wallet. Either use a local address, or set -minetolocalwallet=0"));
        }
#endif  // ENABLE_WALLET
    }
#endif  // ENABLE_MINING
    ...
    if (mapArgs.count("-loadblock")) {
        const auto& source{mapMultiArgs["-loadblock"]};
        for (const std::string& strFile : source) {
    ...
    if (GetBoolArg("-listenonion", DEFAULT_LISTEN_ONION)) StartTorControl(threadGroup, scheduler);

    StartNode(threadGroup, scheduler);

    // Monitor the chain, and alert if we get blocks much quicker or slower than expected
    int64_t nPowTargetSpacing = Params().GetConsensus().nPowTargetSpacing;
    CScheduler::Function f = boost::bind(&PartitionCheck, &IsInitialBlockDownload, boost::ref(cs_main),
                                         boost::cref(pindexBestHeader), nPowTargetSpacing);
    ...
    threadGroup.create_thread(
        boost::bind(&CScAsyncProofVerifier::RunPeriodicVerification, &CScAsyncProofVerifier::GetInstance()));
