...
CMerkleBlock::CMerkleBlock(const CBlock& block, const std::set<uint256>& txids) : header(block.GetBlockHeader()) {
    vector<bool> vMatch;
    vector<uint256> vHashes;
...
void CPartialMerkleTree::TraverseAndBuild(int height, unsigned int pos, const std::vector<uint256>& vTxid,
    ...
    for (unsigned int p = pos << height; p < (pos + 1) << height && p < nTransactions; p++) fParentOfMatch |= vMatch[p];
    // store as flag bit
    ...
    if (height == 0 || !fParentOfMatch) {
        ...
        if (pos * 2 + 1 < CalcTreeWidth(height - 1)) TraverseAndBuild(height - 1, pos * 2 + 1, vTxid, vMatch);
    }
}

uint256 CPartialMerkleTree::TraverseAndExtract(int height, unsigned int pos, unsigned int& nBitsUsed, unsigned int& nHashUsed,
