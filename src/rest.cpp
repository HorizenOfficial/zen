...
static bool rest_headers(HTTPRequest* req, const std::string& strURIPart) {
    ...
    uint256 hash;
    if (!ParseHashStr(hashStr, hash)) return RESTERR(req, HTTP_BAD_REQUEST, "Invalid hash: " + hashStr);
    ...
    BOOST_FOREACH (const CBlockIndex* pindex, headers) { ssHeader << pindex->GetBlockHeader(); }
    ...
    switch (rf) {
        ...
        case RF_JSON: {
            ...
            BOOST_FOREACH (const CBlockIndex* pindex, headers) { jsonHeaders.push_back(blockheaderToJSON(pindex)); }
...
static bool rest_block(HTTPRequest* req, const std::string& strURIPart, bool showTxDetails) {
    ...
    {
        LOCK(cs_main);
        if (mapBlockIndex.count(hash) == 0) return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");

        pblockindex = mapBlockIndex[hash];
        if (fHavePruned && !(pblockindex->nStatus & BLOCK_HAVE_DATA) && pblockindex->nTx > 0)
            return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not available (pruned data)");

        if (!ReadBlockFromDisk(block, pblockindex)) return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");
    }
...
static bool rest_block_extended(HTTPRequest* req, const std::string& strURIPart) { return rest_block(req, strURIPart, true); }
...
static bool rest_tx(HTTPRequest* req, const std::string& strURIPart) {
    ...
    CTransaction tx;
    uint256 hashBlock{};
    if (!GetTransaction(hash, tx, hashBlock, true)) return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");
...
static bool rest_getutxos(HTTPRequest* req, const std::string& strURIPart) {
    ...
    if (uriParts.size() > 0) {
        ...
        if (uriParts.size() > 0 && uriParts[0] == "checkmempool") fCheckMemPool = true;
    ...
    switch (rf) {
        ...
        case RF_JSON: {
            if (!fInputParsed) return RESTERR(req, HTTP_INTERNAL_SERVER_ERROR, "Error: empty request");
            break;
        }
        default: {
            return RESTERR(req, HTTP_NOT_FOUND, "output format not found (available: " + AvailableDataFormatsString() + ")");
    ...
    if (vOutPoints.size() > MAX_GETUTXOS_OUTPOINTS)
        return RESTERR(
            req, HTTP_INTERNAL_SERVER_ERROR,
