...
std::string JSOutPoint::ToString() const { return strprintf("JSOutPoint(%s, %d, %d)", hash.ToString().substr(0, 10), js, n); }

std::string COutput::ToString() const {
    return strprintf("COutput(%s, %d, %d) [%s]", tx->getTxBase()->GetHash().ToString(), pos, nDepth,
                     FormatMoney(tx->getTxBase()->GetVout()[pos].nValue));
}
...
CZCPaymentAddress CWallet::GenerateNewZKey() {
    ...
    if (CCryptoKeyStore::HaveSpendingKey(addr)) throw std::runtime_error("CWallet::GenerateNewZKey(): Collision detected");
    ...
    if (!AddZKey(k)) throw std::runtime_error("CWallet::GenerateNewZKey(): AddZKey failed");
    return pubaddr;
}
...
bool CWallet::AddZKey(const libzcash::SpendingKey& key) {
    ...
    if (!CCryptoKeyStore::AddSpendingKey(key)) return false;

    // check if we need to remove from viewing keys
    if (HaveViewingKey(addr)) RemoveViewingKey(key.viewing_key());
...
bool CWallet::AddCryptedSpendingKey(const libzcash::PaymentAddress& address, const libzcash::ReceivingKey& rk,
                                    const std::vector<unsigned char>& vchCryptedSecret) {
    if (!CCryptoKeyStore::AddCryptedSpendingKey(address, rk, vchCryptedSecret)) return false;
    if (!fFileBacked) return true;
    {
        LOCK(cs_wallet);
        if (pwalletdbEncryption) {
            return pwalletdbEncryption->WriteCryptedZKey(address, rk, vchCryptedSecret, mapZKeyMetadata[address]);
        } else {
            return CWalletDB(strWalletFile).WriteCryptedZKey(address, rk, vchCryptedSecret, mapZKeyMetadata[address]);
        }
    }
    return false;
}
...
bool CWallet::LoadCryptedZKey(const libzcash::PaymentAddress& addr, const libzcash::ReceivingKey& rk,
                              const std::vector<unsigned char>& vchCryptedSecret) {
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libzcash::SpendingKey& key) { return CCryptoKeyStore::AddSpendingKey(key); }
...
bool CWallet::LoadViewingKey(const libzcash::ViewingKey& vk) { return CCryptoKeyStore::AddViewingKey(vk); }
...
bool CWallet::RemoveWatchOnly(const CScript& dest) {
    ...
    if (fFileBacked)
        if (!CWalletDB(strWalletFile).EraseWatchOnly(dest)) return false;
...
void CWallet::ChainTip(const CBlockIndex* pindex, const CBlock* pblock, ZCIncrementalMerkleTree tree, bool added) {
    if (added) {
...
bool CWallet::SetMinVersion(enum WalletFeature nVersion, CWalletDB* pwalletdbIn, bool fExplicit) {
    ...
    if (fExplicit && nVersion > nWalletMaxVersion) nVersion = FEATURE_LATEST;
    ...
    if (fFileBacked) {
        ...
        if (nWalletVersion > 40000) pwalletdb->WriteMinVersion(nWalletVersion);
...
bool CWallet::SetMaxVersion(int nVersion) {
    ...
    if (nWalletVersion > nVersion) return false;
...
bool CWallet::Verify(const string& walletFile, string& warningString, string& errorString) {
    ...
    if (boost::filesystem::exists(GetDataDir() / walletFile)) {
        ...
        if (r == CDBEnv::RECOVER_FAIL) errorString += _("wallet.dat corrupt, salvage failed");
    }
    return true;
}
    ...
    for (TxSpends::const_iterator it = range.first; it != range.second; ++it) {
        ...
        if (mit != mapWallet.end()) {
            if (mit->second->GetDepthInMainChain() >= 0) {
                ...
                LogPrint("cert", "%s():%d - obj[%s] has depth %d\n", __func__, __LINE__, wtxid.ToString(),
                         mit->second->GetDepthInMainChain());
            }
        }
    }
    return false;
...
void CWallet::IncrementNoteWitnesses(const CBlockIndex* pindex, const CBlock* pblockIn, ZCIncrementalMerkleTree& tree) {
    {
        LOCK(cs_wallet);
        for (auto& wtxItem : mapWallet) {
            for (mapNoteData_t::value_type& item : wtxItem.second->mapNoteData) {
                ...
                if (nd->witnessHeight < pindex->nHeight) {
                    ...
                    assert((nd->witnessHeight == -1) || (nd->witnessHeight == pindex->nHeight - 1));
            ...
            for (size_t i = 0; i < tx.GetVjoinsplit().size(); i++) {
                ...
                for (uint8_t j = 0; j < jsdesc.commitments.size(); j++) {
                    ...
                    for (auto& wtxItem : mapWallet) {
                        for (mapNoteData_t::value_type& item : wtxItem.second->mapNoteData) {
                            CNoteData* nd = &(item.second);
                            if (nd->witnessHeight < pindex->nHeight && nd->witnesses.size() > 0) {
                                // Check the validity of the cache
                    ...
                    if (txIsOurs) {
                        ...
                        if (mapWallet[hash]->mapNoteData.count(jsoutpt) &&
                            ...
                            if (nd->witnesses.size() > 0) {
                                ...
                                LogPrintf(
                                    "Inconsistent witness cache state found for %s\n- Cache size: %d\n- Top (height %d): %s\n- "
                                    "New (height %d): %s\n",
                                    jsoutpt.ToString(), nd->witnesses.size(), nd->witnessHeight,
                                    nd->witnesses.front().root().GetHex(), pindex->nHeight, tree.witness().root().GetHex());
                                nd->witnesses.clear();
                            }
        ...
        for (auto& wtxItem : mapWallet) {
            for (mapNoteData_t::value_type& item : wtxItem.second->mapNoteData) {
                ...
                if (nd->witnessHeight <= pindex->nHeight) {
                    ...
                    assert((nd->witnessHeight == -1) || (nd->witnessHeight == pindex->nHeight));
                    if (nd->witnesses.size() > 0) {
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase) {
    ...
    kMasterKey.nDeriveIterations =
        (kMasterKey.nDeriveIterations + kMasterKey.nDeriveIterations * 100 / ((double)(GetTimeMillis() - nStartTime))) / 2;
...
vTxWithInputs CWallet::OrderedTxWithInputs(const std::string& address) const {
    ...
    for (auto it : wtxOrdered) {
        ...
        LogPrintf("%s():%d - processing ordered tx: nOrderPos[%d]: tx[%s]\n", __func__, __LINE__, orderPos,
                  wtx->getTxBase()->GetHash().ToString());
            ...
            for (mapNoteData_t::value_type& item : wtxItem.second->mapNoteData) {
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        item.second.nullifier = GetNoteNullifier(wtxItem.second->getTxBase()->GetVjoinsplit()[i],
                                                                 item.second.address, dec, hSig, item.first.n);
                    }
                }
            }
...
bool CWallet::AddToWallet(const CWalletTransactionBase& wtxIn, bool fFromLoadWallet, CWalletDB* pwalletdb) {
    ...
    if (fFromLoadWallet) {
        ...
        //// debug print
        LogPrintf("AddToWallet %s  %s%s\n", wtxIn.getTxBase()->GetHash().ToString(), (fInsertedNew ? "new" : ""),
                  (fUpdated ? "update" : ""));

        // Write to disk
        if (fInsertedNew || fUpdated)
            if (!wtx.WriteToDisk(pwalletdb)) return false;
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionBase& obj, const CBlock* pblock, int bwtMaturityDepth, bool fUpdate) {
    {
        ...
        try {
            if (fExisted || IsMine(obj) || IsFromMe(obj) || noteData.size() > 0) {
                ...
                if (pblock) sobj->SetMerkleBranch(*pblock);
...
void CWallet::SyncTransaction(const CTransaction& tx, const CBlock* pblock) {
    LOCK(cs_wallet);
    if (!AddToWalletIfInvolvingMe(tx, pblock, -1, true)) return;  // Not one of ours
...
void CWallet::SyncCertificate(const CScCertificate& cert, const CBlock* pblock, int bwtMaturityHeight) {
    LOCK(cs_wallet);
    if (!AddToWalletIfInvolvingMe(cert, pblock, bwtMaturityHeight, true)) return;  // Not one of ours
...
void CWallet::SyncCertStatusInfo(const CScCertificateStatusUpdateInfo& certStatusInfo) {
    ...
    LogPrint("cert", "%s():%d - Called for cert[%s], bwtAreStripped[%d]\n", __func__, __LINE__,
             certStatusInfo.certHash.ToString(), certStatusInfo.bwtState == CScCertificateStatusUpdateInfo::BwtState::BWT_OFF);
    ...
    // Write to disk
    if (!itCert->second->WriteToDisk(&walletdb)) LogPrintf("%s():%d - ERROR in writing to db\n", __func__, __LINE__);
}
...
void CWallet::MarkAffectedTransactionsDirty(const CTransactionBase& tx) {
    ...
    for (const JSDescription& jsdesc : tx.GetVjoinsplit()) {
        for (const uint256& nullifier : jsdesc.nullifiers) {
            if (mapNullifiersToNotes.count(nullifier) && mapWallet.count(mapNullifiersToNotes[nullifier].hash)) {
...
void CWallet::EraseFromWallet(const uint256& hash) {
    if (!fFileBacked) return;
    {
        ...
        if (mapWallet.erase(hash)) CWalletDB(strWalletFile).EraseWalletTxBase(hash);
    }
    return;
}
 ...
 */
boost::optional<uint256> CWallet::GetNoteNullifier(const JSDescription& jsdesc, const libzcash::PaymentAddress& address,
        ...
        for (uint8_t j = 0; j < tx.GetVjoinsplit()[i].ciphertexts.size(); j++) {
            for (const NoteDecryptorMap::value_type& item : mapNoteDecryptors) {
                try {
                    ...
                    JSOutPoint jsoutpt{hash, i, j};
                    auto nullifier = GetNoteNullifier(tx.GetVjoinsplit()[i], address, item.second, hSig, j);
                    if (nullifier) {
...
bool CWallet::IsFromMe(const uint256& nullifier) const {
    {
        LOCK(cs_wallet);
        if (mapNullifiersToNotes.count(nullifier) && mapWallet.count(mapNullifiersToNotes.at(nullifier).hash)) {
            return true;
        }
    }
    return false;
...
void CWallet::GetNoteWitnesses(std::vector<JSOutPoint> notes, std::vector<boost::optional<ZCIncrementalWitness>>& witnesses,
                               uint256& final_anchor) {
    {
        ...
        for (JSOutPoint note : notes) {
            if (mapWallet.count(note.hash) && mapWallet[note.hash]->mapNoteData.count(note) &&
...
isminetype CWallet::IsMine(const CTxIn& txin) const {
    {
        ...
        if (mi != mapWallet.end()) {
            ...
            if (txin.prevout.n < prev.getTxBase()->GetVout().size()) return IsMine(prev.getTxBase()->GetVout()[txin.prevout.n]);
        }
    }
    return ISMINE_NO;
}
...
bool CWallet::BwtIsMine(const CScCertificate& cert) const {
    for (unsigned int i = 0; i < cert.GetVout().size(); i++) {
        if (cert.IsBackwardTransfer(i) && IsMine(cert.GetVout()[i])) return true;
    }
    return false;
}
...
CAmount CWallet::GetCredit(const CWalletTransactionBase& txWalletBase, const isminefilter& filter, bool& fCanBeCached,
                           bool keepImmatureVoutsOnly) const {
...
int CWalletTx::GetIndexInBlock(const CBlock& block) {
    ...
    for (nIndex = 0; nIndex < (int)block.vtx.size(); nIndex++)
        if (block.vtx[nIndex] == wrappedTx) break;
...
CWalletTx::CWalletTx(const CWallet* pwalletIn, const CTransaction& txIn) : CWalletTransactionBase(pwalletIn), wrappedTx(txIn) {}

CWalletTx::CWalletTx(const CWalletTx& rhs) : CWalletTransactionBase(rhs), wrappedTx(rhs.wrappedTx) {}
...
void CWalletTx::GetAmounts(list<COutputEntry>& listReceived, list<COutputEntry>& listSent, CAmount& nFee,
    ...
    if (isFromMyTaddr) {
        ...
        if (myVpubOld > myVpubNew) {
            COutputEntry output = {CNoDestination(), myVpubOld - myVpubNew, CCoins::outputMaturity::MATURE,
                                   (int)wrappedTx.GetVout().size(), /*isBackwardTransfer*/ false};
            listSent.push_back(output);
        } else if (myVpubNew > myVpubOld) {
            COutputEntry output = {CNoDestination(), myVpubNew - myVpubOld, CCoins::outputMaturity::MATURE,
                                   (int)wrappedTx.GetVout().size(), /*isBackwardTransfer*/ false};
    ...
    for (unsigned int pos = 0; pos < wrappedTx.GetVout().size(); ++pos) {
        ...
        if (!ExtractDestination(txout.scriptPubKey, address)) {
            LogPrintf("%s():%d - can not get address for txout %d of tx %s (script %s)\n", __func__, __LINE__, pos,
                      wrappedTx.GetHash().ToString(), txout.scriptPubKey.ToString());
            address = CNoDestination();
        }

        COutputEntry output = {address, txout.nValue, IsOutputMature(pos), (int)pos, /*isBackwardTransfer*/ false};
    ...
    if (wrappedTx.IsScVersion()) {
        if (nDebit > 0) {
            ...
            for (const auto& out : wrappedTx.GetVscCcOut()) totalScOut += out.nValue;
            for (const auto& out : wrappedTx.GetVftCcOut()) totalScOut += out.nValue;
            for (const auto& out : wrappedTx.GetVBwtRequestOut()) totalScOut += out.scFee;

            COutputEntry output = {CNoDestination(), totalScOut, CCoins::outputMaturity::MATURE,
                                   (int)wrappedTx.GetVout().size(), /*isBackwardTransfer*/ false};
            listSent.push_back(output);
        }
    }
...
void CWalletTransactionBase::GetMatureAmountsForAccount(const string& strAccount, CAmount& nReceived, CAmount& nSent,
...
void CWallet::WitnessNoteCommitment(std::vector<uint256> commitments,
                                    std::vector<boost::optional<ZCIncrementalWitness>>& witnesses, uint256& final_anchor) {
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            if (pindex->nHeight % 100 == 0 && dProgressTip - dProgressStart > 0.0)
                ShowProgress(_("Rescanning..."), std::max(1, std::min(99, (int)((Checkpoints::GuessVerificationProgress(
            ...
            for (const CTransaction& tx : block.vtx) {
                if (AddToWalletIfInvolvingMe(tx, &block, -1, fUpdate)) ret++;
            }
            ...
            for (auto itCert = block.vcert.rbegin(); itCert != block.vcert.rend(); ++itCert) {
                ...
                if (AddToWalletIfInvolvingMe(*itCert, &block, bwtMaxDepth, fUpdate)) {
                    ...
                    if (fUpdate) {
                        ...
                        SyncCertStatusInfo(CScCertificateStatusUpdateInfo(
                            itCert->GetScId(), itCert->GetHash(), itCert->epochNumber, itCert->quality,
                        ...
                        if (prevScDataAvailable) {
                            if (bTopQualityCert && (prevScData.certEpoch == itCert->epochNumber) &&
                                (prevScData.certQuality < itCert->quality)) {
                                SyncCertStatusInfo(CScCertificateStatusUpdateInfo(
                                    prevScData.scId, prevScData.certHash, prevScData.certEpoch, prevScData.certQuality,
                                    CScCertificateStatusUpdateInfo::BwtState::BWT_OFF));
                            }
                        }
                    }
        ...
        for (const auto& scId : allScIds) {
            if (pcoinsTip->GetSidechainState(scId) != CSidechain::State::ALIVE) {
                ...
                if (fUpdate)
                    SyncCertStatusInfo(CScCertificateStatusUpdateInfo(
                        scId, sidechain.lastTopQualityCertHash, sidechain.lastTopQualityCertReferencedEpoch,
...
CAmount CWalletTransactionBase::GetDebit(const isminefilter& filter) const {
    if (getTxBase()->GetVin().empty()) return 0;
...
CCoins::outputMaturity CWalletTransactionBase::IsOutputMature(unsigned int vOutPos) const {
    ...
    if (nDepth < 0) return CCoins::outputMaturity::NOT_APPLICABLE;

    if (nDepth == 0) {
        if (!getTxBase()->IsCoinBase() && !getTxBase()->IsCertificate()) return CCoins::outputMaturity::MATURE;
    ...
    if (!getTxBase()->IsBackwardTransfer(vOutPos)) return CCoins::outputMaturity::MATURE;

    if (getTxBase()->IsBackwardTransfer(vOutPos) && bwtAreStripped) return CCoins::outputMaturity::NOT_APPLICABLE;
...
CAmount CWalletTransactionBase::GetCredit(const isminefilter& filter) const {
    ...
    if (filter & ISMINE_WATCH_ONLY) {
        ...
        if (!fWatchCreditCached)
            nWatchCreditCached =
                pwallet->GetCredit(*this, ISMINE_WATCH_ONLY, fWatchCreditCached, /*keepImmatureVoutsOnly*/ false);
...
CAmount CWalletTransactionBase::GetImmatureCredit(bool fUseCache) const {
    if ((this->getTxBase()->IsCoinBase() || this->getTxBase()->IsCertificate()) && this->HasImmatureOutputs()) {
        ...
        nImmatureCreditCached =
            pwallet->GetCredit(*this, ISMINE_SPENDABLE, fImmatureCreditCached, /*keepImmatureVoutsOnly*/ true);
...
CAmount CWalletTransactionBase::GetImmatureWatchOnlyCredit(const bool& fUseCache) const {
    if ((this->getTxBase()->IsCoinBase() || this->getTxBase()->IsCertificate()) && this->HasImmatureOutputs()) {
        ...
        nImmatureWatchCreditCached =
            pwallet->GetCredit(*this, ISMINE_WATCH_ONLY, fImmatureWatchCreditCached, /*keepImmatureVoutsOnly*/ true);
...
CAmount CWalletTransactionBase::GetAvailableCredit(bool fUseCache) const {
    ...
    if (fAvailableCreditCached) nAvailableCreditCached = nCredit;
...
CAmount CWalletTransactionBase::GetAvailableWatchOnlyCredit(const bool& fUseCache) const {
    ...
    if (fAvailableWatchCreditCached) nAvailableWatchCreditCached = nCredit;
...
bool CWalletTransactionBase::IsTrusted(bool canSpendZeroConfChange) const {
    ...
    if (!CheckFinalTx(*getTxBase())) return false;
...
std::vector<uint256> CWallet::ResendWalletTransactionsBefore(int64_t nTime) {
    ...
    for (auto& item : mapWallet) {
        ...
        if (wtx.nTimeReceived > nTime) continue;
        mapSorted.insert(make_pair(wtx.nTimeReceived, &wtx));
    }
    for (auto& item : mapSorted) {
        CWalletTransactionBase& wtx = *(item.second);
        if (wtx.RelayWalletTransaction()) result.push_back(wtx.getTxBase()->GetHash());
    }
    return result;
}
...
void CWallet::ResendWalletTransactions(int64_t nBestBlockTime) {
    ...
    if (!relayed.empty()) LogPrintf("%s: rebroadcast %u unconfirmed transactions\n", __func__, relayed.size());
}
...
void CWallet::GetUnconfirmedData(const std::string& address, int& numbOfUnconfirmedTx, CAmount& unconfInput,
                                 CAmount& unconfOutput, CAmount& bwtImmatureOutput, eZeroConfChangeUsage zconfchangeusage,
                                 bool fIncludeNonFinal) const {
    unconfOutput = 0;
    unconfInput = 0;
    bwtImmatureOutput = 0;
    numbOfUnconfirmedTx = 0;

    LogPrint("sc", "%s():%d - called zconfchangeusage[%d], fIncludeNonFinal[%d]\n", __func__, __LINE__, (int)zconfchangeusage,
             fIncludeNonFinal);
...

    {
        ...
        for (vTxWithInputs::reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
            ...
            LogPrint("sc", "%s():%d - isFinal[%d], tx[%s]\n", __func__, __LINE__, (int)isFinal,
                     pcoin->getTxBase()->GetHash().ToString());

            if (!fIncludeNonFinal && !isFinal) continue;
            ...
            if (!isFinal || (!trusted && pcoin->GetDepthInMainChain() == 0)) {
                ...
                for (const auto& txout : pcoin->getTxBase()->GetVout()) {
                    ...
                    auto res = std::search(txout.scriptPubKey.begin(), txout.scriptPubKey.end(), scriptToMatch.begin(),
                                           scriptToMatch.end());
                    if (res == txout.scriptPubKey.begin()) {
                        ...
                        if (!IsSpent(pcoin->getTxBase()->GetHash(), vout_idx)) {
                            ...
                            LogPrint("cert", "%s():%d - found matching tx[%s] but out[%d] is spent: %s\n", __func__, __LINE__,
                                     pcoin->getTxBase()->GetHash().ToString(), vout_idx, pcoin->getTxBase()->ToString());
                        }
                    }
                    vout_idx++;
                }
            ...
            if (cert != nullptr) {
                for (unsigned int i = 0; i < cert->GetVout().size(); i++) {
                    if (cert->IsBackwardTransfer(i) && pcoin->IsOutputMature(i) == CCoins::outputMaturity::IMMATURE) {
                        ...
                        if (res == txout.scriptPubKey.begin()) {
                            ...
                            LogPrint("cert", "%s():%d - found bwtout of matching cert[%s] with immature credit\n", __func__,
                                     __LINE__, cert->GetHash().ToString());
                        }
                    }
                }
            }
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl* coinControl,
                             bool fIncludeZeroValue, bool fIncludeCoinBase, bool fIncludeCommunityFund) const {
    vCoins.clear();

    {
        ...
        for (MAP_WALLET_CONST_IT it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            const CWalletTransactionBase* pcoin = (*it).second.get();
            if (!CheckFinalTx(*pcoin->getTxBase())) continue;

            if (fOnlyConfirmed && !pcoin->IsTrusted()) continue;

            if (pcoin->getTxBase()->IsCoinBase() && !fIncludeCoinBase && !fIncludeCommunityFund) continue;

            if (!pcoin->HasMatureOutputs()) continue;
            ...
            for (unsigned int voutPos = 0; voutPos < pcoin->getTxBase()->GetVout().size(); voutPos++) {
                ...
                if (!IsSpent(wtxid, voutPos) && mine != ISMINE_NO && !IsLockedCoin((*it).first, voutPos) &&
                    ...
                    (!coinControl || !coinControl->HasSelected() || coinControl->fAllowOtherInputs ||
                     coinControl->IsSelected((*it).first, voutPos))) {
                    if (pcoin->getTxBase()->IsCoinBase()) {
                        const CCoins* coins = pcoinsTip->AccessCoins(wtxid);
                        assert(coins);

                        if (IsCommunityFund(coins, voutPos)) {
                            if (!fIncludeCommunityFund) continue;
                        } else {
                            if (!fIncludeCoinBase) continue;
                        }
                    } else if (pcoin->getTxBase()->IsCertificate()) {
                        if (pcoin->IsOutputMature(voutPos) != CCoins::outputMaturity::MATURE) continue;

                        LogPrint("cert", "%s():%d - cert[%s] out[%d], amount=%s, spendable[%s]\n", __func__, __LINE__,
                                 pcoin->getTxBase()->GetHash().ToString(), voutPos,
                                 FormatMoney(pcoin->getTxBase()->GetVout()[voutPos].nValue),
                                 ((mine & ISMINE_SPENDABLE) != ISMINE_NO) ? "Y" : "N");
                    }
...
static void ApproximateBestSubset(vector<pair<CAmount, pair<const CWalletTransactionBase*, unsigned int>>> vValue,
                                  const CAmount& nTotalLower, const CAmount& nTargetValue, vector<char>& vfBest, CAmount& nBest,
                                  int iterations = 1000) {
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins,
                                 set<pair<const CWalletTransactionBase*, unsigned int>>& setCoinsRet,
    ...
    if (coinLowestLarger.second.first &&
        ...
        for (unsigned int i = 0; i < vValue.size(); i++)
            if (vfBest[i]) LogPrint("selectcoins", "%s ", FormatMoney(vValue[i].first));
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTransactionBase*, unsigned int>>& setCoinsRet,
    ...
    for (const COutPoint& outpoint : vPresetInputs) {
        ...
        if (it != mapWallet.end()) {
            ...
            if (pcoin->getTxBase()->GetVout().size() <= outpoint.n) return false;
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosRet, std::string& strFailReason) {
    ...
    for (const CTxIn& txin : tx.vin) coinControl.Select(txin.prevout);
    ...
    CWalletTx wtx;
    if (!CreateTransaction(vecSend, vecScSend, vecFtSend, vecBwtRequest, wtx, reservekey, nFeeRet, nChangePosRet, strFailReason,
                           &coinControl, false, tx.vcsw_ccin))
        return false;

    if (nChangePosRet != -1) tx.insertAtPos(nChangePosRet, wtx.getWrappedTx().GetVout()[nChangePosRet]);
    ...
    for (const CTxIn& txin : wtx.getWrappedTx().GetVin()) {
        ...
        if (!found) tx.vin.push_back(txin);
    }
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, const std::vector<CRecipientScCreation>& vecScSend,
                                const std::vector<CRecipientForwardTransfer>& vecFtSend,
                                const std::vector<CRecipientBwtRequest>& vecBwtRequest, CWalletTx& wtxNew,
    ...
    if (!checkAndAddCcOut(vecScSend, totalOutputValue, strFailReason)) return false;
    if (!checkAndAddCcOut(vecFtSend, totalOutputValue, strFailReason)) return false;
    if (!checkAndAddCcOut(vecBwtRequest, totalOutputValue, strFailReason)) return false;
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nSubtractFeeFromAmount == 0) nTotalValue += nFeeRet;
                double dPriority = 0;
                // vouts to the payees
                for (const CRecipient& recipient : vecSend) {
                ...
                if (nTotalValue > 0) {
                    if (!SelectCoins(nTotalValue, setCoins, nValueIn, fOnlyCoinbaseCoins, fNeedCoinbaseCoins, coinControl)) {
                        if (fOnlyCoinbaseCoins && Params().GetConsensus().fCoinbaseMustBeProtected) {
                            ...
                            strFailReason =
                                _("Insufficient funds, coinbase funds can only be spent after they have been sent to a zaddr");
                        } else {
                ...
                if (nSubtractFeeFromAmount == 0) nChange -= nFeeRet;
                ...
                if (!sign) {
                    for (CTxIn& vin : txNew.vin) vin.scriptSig = CScript();
                    for (auto& cswIn : txNew.vcsw_ccin) cswIn.redeemScript = CScript();
                }
...
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey) {
    {
        ...
        if (fBroadcastTransactions) {
            ...
            if (MempoolReturnValue::VALID != AcceptTxBaseToMemoryPool(mempool, stateDummy, *wtxNew.getTxBase(),
                                                                      LimitFreeFlag::OFF, RejectAbsurdFeeFlag::ON,
                                                                      MempoolProofVerificationFlag::SYNC)) {
...
bool CWallet::AddAccountingEntry(const CAccountingEntry& acentry, CWalletDB& pwalletdb) {
    if (!pwalletdb.WriteAccountingEntry_Backend(acentry)) return false;
...
bool CWallet::SetDefaultKey(const CPubKey& vchPubKey) {
    if (fFileBacked) {
        if (!CWalletDB(strWalletFile).WriteDefaultKey(vchPubKey)) return false;
    }
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances() {
...

    {
        ...
        for (auto& walletEntry : mapWallet) {
            ...
            if (!CheckFinalTx(*pcoin->getTxBase()) || !pcoin->IsTrusted()) continue;

            if (!pcoin->HasMatureOutputs()) continue;

            if (pcoin->GetDepthInMainChain() < (pcoin->IsFromMe(ISMINE_ALL) ? 0 : 1)) continue;

            for (unsigned int pos = 0; pos < pcoin->getTxBase()->GetVout().size(); pos++) {
                CTxDestination addr;
                if (!IsMine(pcoin->getTxBase()->GetVout()[pos])) continue;

                if (pcoin->getTxBase()->IsCertificate()) {
                    if (pcoin->IsOutputMature(pos) != CCoins::outputMaturity::MATURE) continue;
                }

                if (!ExtractDestination(pcoin->getTxBase()->GetVout()[pos].scriptPubKey, addr)) continue;
    ...
    for (auto& walletEntry : mapWallet) {
        ...
        if (pcoin->getTxBase()->GetVin().size() > 0) {
            ...
            for (const CTxIn& txin : pcoin->getTxBase()->GetVin()) {
                ...
                if (!IsMine(txin)) /* If this input isn't mine, ignore it */
                    continue;
                if (!ExtractDestination(mapWallet[txin.prevout.hash]->getTxBase()->GetVout()[txin.prevout.n].scriptPubKey,
                                        address))
                    continue;
            ...
            if (any_mine) {
                for (const CTxOut& txout : pcoin->getTxBase()->GetVout())
                    if (IsChange(txout)) {
        ...
        for (unsigned int i = 0; i < pcoin->getTxBase()->GetVout().size(); i++)
            if (IsMine(pcoin->getTxBase()->GetVout()[i])) {
                CTxDestination address;
                if (!ExtractDestination(pcoin->getTxBase()->GetVout()[i].scriptPubKey, address)) continue;
                grouping.insert(address);
                groupings.insert(grouping);
                grouping.clear();
            }
...
void CWallet::GetAllReserveKeys(set<CKeyID>& setAddress) const {
    ...
    for (const int64_t id : setKeyPool) {
        CKeyPool keypool;
        if (!walletdb.ReadPool(id, keypool)) throw runtime_error("GetAllReserveKeyHashes(): read failed");
        assert(keypool.vchPubKey.IsValid());
...
void CWallet::UpdatedTransaction(const uint256& hashTx) {
    {
        ...
        if (mi != mapWallet.end()) NotifyTransactionChanged(this, hashTx, CT_UPDATED);
    }
}
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t>& mapKeyBirth) const {
    ...
    CBlockIndex* pindexMax =
        chainActive[std::max(0, chainActive.Height() - 144)];  // the tip can be reorganised; use a 144-block safety margin
...
int CWalletTransactionBase::GetDepthInMainChainINTERNAL(const CBlockIndex*& pindexRet) const {
    ...
    if (!fMerkleVerified) {
        if (CBlock::CheckMerkleBranch(getTxBase()->GetHash(), vMerkleBranch, nIndex) != pindex->hashMerkleRoot) return 0;
...
int CWalletTransactionBase::GetDepthInMainChain(const CBlockIndex*& pindexRet) const {
    ...
    if (nResult == 0 && !mempool.exists(getTxBase()->GetHash())) return -1;  // Not in chain, not in mempool
...
int CWalletCert::GetIndexInBlock(const CBlock& block) {
    ...
    for (nIndex = 0; nIndex < (int)block.vcert.size(); nIndex++)
        if (block.vcert[nIndex] == wrappedCertificate) break;
...
CWalletCert::CWalletCert(const CWalletCert& rhs) : CWalletTransactionBase(rhs), wrappedCertificate(rhs.wrappedCertificate) {}
...
void CWalletCert::GetAmounts(std::list<COutputEntry>& listReceived, std::list<COutputEntry>& listSent, CAmount& nFee,
    ...
    for (unsigned int pos = 0; pos < wrappedCertificate.GetVout().size(); ++pos) {
        ...
        CCoins::outputMaturity outputMaturity = this->IsOutputMature(pos);
        if (outputMaturity == CCoins::outputMaturity::NOT_APPLICABLE) continue;

        COutputEntry output = {address, txout.nValue, outputMaturity, (int)pos, wrappedCertificate.IsBackwardTransfer(pos)};

        // If we are debited by the transaction, add the output as a "sent" entry
        // unless it is a backward transfer output
        if (nDebit > 0 && !output.isBackwardTransfer) listSent.push_back(output);
...
std::shared_ptr<CWalletTransactionBase> CWalletTransactionBase::MakeWalletObjectBase(const CTransactionBase& obj,
