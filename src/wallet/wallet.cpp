...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
...

    {
        ...
        while (pindex)
        {
            ...
            if (pindex->pprev) {
//TODO CHECK
                if (ForkManager::getInstance().isTransactionUpgradeActive(TransactionTypeActive::SAPLING_TX, pindex->pprev->nHeight)) {
...
//TODO CHECK
                {
                	LOCK(cs_main);
            		if (ForkManager::getInstance().isTransactionUpgradeActive(TransactionTypeActive::OVERWINTER_TX, chainActive.Height() + 1)) {
            			limit = 0;
            		}
                }
                if (limit > 0) {
