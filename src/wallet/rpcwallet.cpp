...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Zen address: ") + name_);
        }
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid horizen address: ") + input.get_str());
            }
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"horizenaddress\"    (string) The new shielded address.\n"
            "\nExamples:\n"
...
// #define V3_JS_DESCRIPTION_SIZE    (GetSerializeSize(JSDescription(), SER_NETWORK, (OVERWINTER_TX_VERSION)))
#define V3_JS_DESCRIPTION_SIZE    ((JSDescription::getNewInstance(true).GetSerializeSize(SER_NETWORK, PROTOCOL_VERSION, OVERWINTER_TX_VERSION)) - 1)
...
// #define Z_SENDMANY_MAX_ZADDR_OUTPUTS(TX_VER)    ((MAX_TX_SIZE_BEFORE_SAPLING / JSDescription::getNewInstance(TX_VER == GROTH_TX_VERSION).GetSerializeSize(SER_NETWORK, PROTOCOL_VERSION, TX_VER)) - 1)
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            + strprintf("%d bytes.", MAX_TX_SIZE_BEFORE_SAPLING)
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
//TODO CHECK
    bool overwinterActive = ForkManager::getInstance().isTransactionUpgradeActive(TransactionTypeActive::OVERWINTER_TX, nextBlockHeight);
    unsigned int max_tx_size = MAX_TX_SIZE_AFTER_SAPLING;
    if (!ForkManager::getInstance().isTransactionUpgradeActive(TransactionTypeActive::SAPLING_TX, nextBlockHeight)) {
        max_tx_size = MAX_TX_SIZE_BEFORE_SAPLING;
    }
...
//TODO CHECK
    if (contextualTx.nVersion == 1) {
    	contextualTx.nVersion = shieldedTxVersion; // Tx format should support vjoinsplits
    }
...
#define JOINSPLIT_SIZE (JSDescription::getNewInstance(true).GetSerializeSize(SER_NETWORK, PROTOCOL_VERSION, GROTH_TX_VERSION))
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    const bool overwinterActive = false; // NetworkUpgradeActive(nextBlockHeight, Params().GetConsensus(), Consensus::UPGRADE_OVERWINTER);
    const bool saplingActive = false; // NetworkUpgradeActive(nextBlockHeight, Params().GetConsensus(), Consensus::UPGRADE_SAPLING);
...
//TODO CHECK
    ...
    if (contextualTx.nVersion == 1 && isSproutShielded) {
        contextualTx.nVersion = GROTH_TX_VERSION; // Tx format should support vjoinsplit
    }
