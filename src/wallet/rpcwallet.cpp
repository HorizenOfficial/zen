...
void AddVinExpandedToJSON(const CWalletTransactionBase& tx, UniValue& entry) {
    if (!tx.getTxBase()->IsCertificate()) entry.pushKV("locktime", (int64_t)tx.getTxBase()->GetLockTime());
...
void TxExpandedToJSON(const CWalletTransactionBase& tx, UniValue& entry) {
    ...
    if (tx.getTxBase()->IsCertificate()) {
        ...
        if (conf >= 0) {
            ...
            bwtMaturityHeight =
                sidechain.GetCertMaturityHeight(dynamic_cast<const CScCertificate*>(tx.getTxBase())->epochNumber);
        }
    }
...
static int getCertMaturityHeight(const CWalletTransactionBase& wtx) {
    ...
    if (it == mapBlockIndex.end()) {
        throw JSONRPCError(
            RPC_TYPE_ERROR,
            strprintf("coluld not find cert maturity height since block %s is not in active chain", wtx.hashBlock.ToString()));
    }
...
static void WalletTxToJSON(const CWalletTransactionBase& wtx, UniValue& entry, isminefilter filter,
                           bool addCertMaturityInfo = false) {
    int confirms = wtx.GetDepthInMainChain();
    entry.pushKV("confirmations", confirms);
    if (wtx.getTxBase()->IsCoinBase()) entry.pushKV("generated", true);
    if (confirms > 0) {
        ...
        if (addCertMaturityInfo) {
            ...
            if (pindexMat == nullptr) {
                ...
                throw JSONRPCError(RPC_TYPE_ERROR,
                                   strprintf("coluld not find the block where the cert reached maturity height"));
            }
...
static void FillScCreationReturnObj(const CTransaction& tx, UniValue& ret) {
    ...
    if (tx.GetVscCcOut().size() != 1) {
        throw JSONRPCError(RPC_TYPE_ERROR, strprintf("creation vector output size %d is invalid", tx.GetVscCcOut().size()));
    }
...
string AccountFromValue(const UniValue& value) {
    ...
    if (strAccount != "") throw JSONRPCError(RPC_WALLET_ACCOUNTS_UNSUPPORTED, "Accounts are unsupported");
    return strAccount;
}
...
UniValue getnewaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. If provided, it MUST be set to the empty string \"\" to "
            "represent the default account. Passing any other string will result in an error.\n"
    ...
    if (account.vchPubKey.IsValid()) {
        ...
        for (auto it = pwalletMain->getMapWallet().begin();
            ...
            for (const CTxOut& txout : wtx.getTxBase()->GetVout()) {
                ...
                auto res =
                    std::search(txout.scriptPubKey.begin(), txout.scriptPubKey.end(), scriptPubKey.begin(), scriptPubKey.end());
                if (res == txout.scriptPubKey.begin()) bKeyUsed = true;
            }
        }
    }
...
UniValue getaccountaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"account\"       (string, required) MUST be set to the empty string \"\" to represent the default account. "
...
UniValue setaccount(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"account\"         (string, required) MUST be set to the empty string \"\" to represent the default account. "
            "Passing any other string will result in an error\n"
    ...
    if (!address.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Zen address");
...
UniValue getaccount(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("getaccount", "\"horizenaddress\"") + HelpExampleRpc("getaccount", "\"horizenaddress\""));
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"account\"  (string, required) MUST be set to the empty string \"\" to represent the default account. Passing "
...
UniValue listaddresses(const UniValue& params, bool fHelp) {
    ...
    for (const auto& item : pwalletMain->mapAddressBook) {
        const CBitcoinAddress& address = item.first;
        const string& strName = item.second.name;
        if (strName == "") ret.push_back(address.ToString());
    }
    return ret;
}
...
static void SendMoney(const CTxDestination& address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew) {
    ...
    if (nValue > curBalance) throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Insufficient funds");
    ...
    if (!pwalletMain->CreateTransaction(vecSend, vecScSend, vecFtSend, vecBwtRequest, wtxNew, reservekey, nFeeRequired,
                                        nChangePosRet, strError)) {
        if (!fSubtractFeeFromAmount && nValue + nFeeRequired > pwalletMain->GetBalance()) {
            unsigned int nBytes = ::GetSerializeSize(*wtxNew.getTxBase(), SER_NETWORK, PROTOCOL_VERSION);
            strError = strprintf(
                "Error: This transaction (sz=%d, vin.size=%d) requires a transaction fee of at least %s because of its amount, "
    ...
    if (!pwalletMain->CommitTransaction(wtxNew, reservekey))
        throw JSONRPCError(
            ...
            "such as if you used a copy of wallet.dat and coins were spent in the copy but not marked as spent here.");
}
...
UniValue sendtoaddress(const UniValue& params, bool fHelp) {
    ...
    // Wallet comments
    CWalletTx wtx;
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) wtx.mapValue["comment"] = params[2].get_str();
    if (params.size() > 3 && !params[3].isNull() && !params[3].get_str().empty()) wtx.mapValue["to"] = params[3].get_str();

    bool fSubtractFeeFromAmount = false;
    if (params.size() > 4) fSubtractFeeFromAmount = params[4].get_bool();
...
UniValue sc_create(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            " \"withdrawalEpochLength\": epoch  (numeric, optional, default=" +
            strprintf("%d", SC_RPC_OPERATION_DEFAULT_EPOCH_LENGTH) +
            ...
            "\n"
            ", the maximum (for any network type) is: " +
            strprintf("%d", Params().ScMaxWithdrawalEpochLength()) +
            "\n"
            " \"fromaddress\":taddr             (string, optional) The taddr to send the funds from. If omitted funds are "
            "taken from all available UTXO\n"
            " \"changeaddress\":taddr           (string, optional) The taddr to send the change to, if any. If not set, "
            "\"fromaddress\" is used. If the latter is not set too, a newly generated address will be used\n"
            " \"toaddress\":scaddr              (string, required) The receiver PublicKey25519Proposition in the SC\n"
            " \"amount\":amount                 (numeric, required) Funds to be sent to the newly created Sidechain. Value "
            "expressed in " +
            CURRENCY_UNIT +
            "\n"
            " \"minconf\":conf                  (numeric, optional, default=1) Only use funds confirmed at least this many "
            "times.\n"
            " \"fee\":fee                       (numeric, optional) The fee amount to attach to this transaction in " +
            CURRENCY_UNIT +
            ". If not specified it is automatically computed using a fixed fee rate (default is 1zat per byte)\n"
            ...
            strprintf("%d", MAX_SC_CUSTOM_DATA_LEN) +
            ...
            strprintf("%d", CFieldElement::ByteSize()) +
            " bytes\n"
            " \"vFieldElementCertificateFieldConfig\":array         (array, optional) An array whose entries are sizes (in "
            "bits). Any certificate should have as many custom FieldElements with the corresponding size.\n"
            " \"vBitVectorCertificateFieldConfig\":array            (array, optional) An array whose entries are "
            "bitVectorSizeBits and maxCompressedSizeBytes pairs. Any certificate should have as many custom "
            "BitVectorCertificateField with the corresponding sizes\n"
            " \"forwardTransferScFee\":fee                        (numeric, optional, default=0) The amount of fee in " +
            CURRENCY_UNIT +
            " due to sidechain actors when creating a FT\n"
            " \"mainchainBackwardTransferScFee\":fee              (numeric, optional, default=0) The amount of fee in " +
            CURRENCY_UNIT +
            " due to sidechain actors when creating a MBTR\n"
            " \"mainchainBackwardTransferRequestDataLength\":len (numeric, optional, default=0) The expected size (max=" +
            strprintf("%d", MAX_SC_MBTR_DATA_LEN) +
            ...
            "}\n"
            "\nExamples:\n" +
            HelpExampleCli("sc_create",
                           "'{\"toaddress\": \"8aaddc9671dc5c8d33a3494df262883411935f4f54002fe283745fb394be508a\" ,\"amount\": "
                           "5.0, \"wCertVk\": abcd..ef}'"));

    LOCK2(cs_main, pwalletMain->cs_wallet);

    // valid input keywords
    static const std::set<std::string> validKeyArgs = {"version",
    ...
    if (!inputObject.isObject()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, expected object");
    ...
    for (const string& s : inputObject.getKeys()) {
        ...
        if (!setKeyArgs.insert(s).second) throw JSONRPCError(RPC_INVALID_PARAMETER, string("Duplicate key in input: ") + s);
    }
    ...
    if (setKeyArgs.count("version")) {
        ...
        if (sidechainVersion < 0 ||
            sidechainVersion > zen::ForkManager::getInstance().getMaxSidechainVersion(chainActive.Height() + 1))
    ...
    if (setKeyArgs.count("changeaddress")) {
        ...
        if (!changeaddress.IsValid()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               string("Invalid parameter, unknown changeaddress format: ") + inputString);
        }
    ...
    if (setKeyArgs.count("amount")) {
        ...
        if (!MoneyRange(nAmount)) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, amount out of range");
        if (nAmount == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, amount can not be null");
    } else {
    ...
    if (setKeyArgs.count("minconf")) {
        ...
        if (nMinDepth < 0) throw JSONRPCError(RPC_TYPE_ERROR, "Invalid minconf: must be greater that 0");
    }
    ...
    if (nFee > nAmount)
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           strprintf("Fee %s is greater than output %s", FormatMoney(nFee), FormatMoney(nAmount)));
    ...
    if (setKeyArgs.count("wCertVk")) {
        ...
        if (!Sidechain::AddScData(inputString, wCertVkVec, CScVKey::MaxByteSize(), Sidechain::CheckSizeMode::CHECK_UPPER_LIMIT,
                                  error)) {
    ...
    if (setKeyArgs.count("customData")) {
        ...
        if (!Sidechain::AddScData(inputString, fixedParams.customData, MAX_SC_CUSTOM_DATA_LEN,
                                  Sidechain::CheckSizeMode::CHECK_UPPER_LIMIT, error)) {
    ...
    if (setKeyArgs.count("constant")) {
        ...
        if (!Sidechain::AddScData(inputString, scConstantByteArray, CFieldElement::ByteSize(),
                                  Sidechain::CheckSizeMode::CHECK_STRICT, error)) {
    ...
    if (setKeyArgs.count("wCeasedVk")) {
        ...
        if (!inputString.empty()) {
            ...
            if (!Sidechain::AddScData(inputString, wCeasedVkVec, CScVKey::MaxByteSize(),
                                      Sidechain::CheckSizeMode::CHECK_UPPER_LIMIT, error)) {
                throw JSONRPCError(RPC_TYPE_ERROR, string("wCeasedVk: ") + error);
            }

            fixedParams.wCeasedVk = CScVKey(wCeasedVkVec);
            if (!fixedParams.wCeasedVk->IsValid()) {
        ...
        if (!PairsArray.isNull()) {
            for (auto& pairEntry : PairsArray.getValues()) {
                ...
                fixedParams.vBitVectorCertificateFieldConfig.push_back(
                    BitVectorCertificateFieldConfig{pairEntry[0].get_int(), pairEntry[1].get_int()});
            }
        }
    }
    ...
    if (setKeyArgs.count("forwardTransferScFee")) {
        ...
        if (!uniFtScFee.isNull()) {
            ...
            if (!MoneyRange(ftScFee)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   strprintf("Invalid forwardTransferScFee, amount out of range [%d, %d]", 0, MAX_MONEY));
            }
        }
    }
        ...
        if (!uniMbtrScFee.isNull()) {
            ...
            if (!MoneyRange(mbtrScFee)) {
                throw JSONRPCError(
                    ...
                    strprintf("Invalid mainchainBackwardTransferScFee, amount out of range [%d, %d]", 0, MAX_MONEY));
            }
        }
    }
    ...
    if (setKeyArgs.count("mainchainBackwardTransferRequestDataLength")) {
        ...
        if (!uniMbtrDataLength.isNull()) {
            if (!uniMbtrDataLength.isNum()) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   "Invalid mainchainBackwardTransferRequestDataLength, expected integer");
            }
            ...
            if (mbtrDataLength < 0 || mbtrDataLength > MAX_SC_MBTR_DATA_LEN) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   strprintf("Invalid mainchainBackwardTransferRequestDataLength: out of range [%d, %d]", 0,
                                             MAX_SC_MBTR_DATA_LEN));
            }
        }
    }
    ...
    Sidechain::ScRpcCreationCmdTx cmd(tx_create, vOutputs, fromaddress, changeaddress, nMinDepth, nFee, ftScFee, mbtrScFee,
                                      fixedParams);
...
UniValue sc_send(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || (params.size() != 1 && params.size() != 2))
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"outputs\"                       (string, required) A json array of json objects representing the amounts to "
            "send.\n"
            "[{\n"
            "   \"scid\": id                      (string, required) The uint256 side chain ID\n"
            "   \"toaddress\":scaddr              (string, required) The receiver PublicKey25519Proposition in the SC\n"
            "   \"amount\":amount                 (numeric, required) Value expressed in " +
            CURRENCY_UNIT +
            "\n"
            "   \"mcReturnAddress\":\"address\"   (string, required) The Horizen mainchain address where to send the backward "
            "transfer in case Forward Transfer is rejected by the sidechain\n"
            "},...,]\n"
            ...
            HelpExampleCli(
                "sc_send",
                "'[{ \"toaddress\": \"abcd\", \"amount\": 3.0, \"scid\": "
                "\"13a3083bdcf42635c8ce5d46c2cae26cfed7dc889d9b4ac0b9939c6631a73bdc\", \"mcReturnAddress\": \"taddr\"}]'"));

    LOCK2(cs_main, pwalletMain->cs_wallet);
    RPCTypeCheck(params, boost::assign::list_of(UniValue::VARR)(UniValue::VOBJ));

    // valid keywords in optional params
    static const std::set<std::string> validKeyArgs = {"fromaddress", "changeaddress", "minconf", "fee"};

    // valid keywords in output array
    static const std::set<std::string> validKeyOutputArray = {"scid", "toaddress", "amount", "mcReturnAddress"};
    ...
    if (outputsArr.size() == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, output arrays is empty.");
    ...
    if (params.size() > 1 && !params[1].isNull()) {
        ...
        if (!cmdParams.isObject()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, expected object");
        ...
        if (setKeyArgs.count("fromaddress")) {
            ...
            if (!fromaddress.IsValid()) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   string("Invalid parameter, unknown fromaddress format: ") + inputString);
            }
        }
        ...
        if (setKeyArgs.count("changeaddress")) {
            ...
            if (!IsMine(*pwalletMain, GetScriptForDestination(changeaddress.Get())))
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   string("Invalid parameter, changeaddress is not mine: ") + inputString);
        }
        ...
        if (nFee > totalAmount)
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               strprintf("Fee %s is greater than output %s", FormatMoney(nFee), FormatMoney(totalAmount)));
    }
...
UniValue sc_request_transfer(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || (params.size() != 1 && params.size() != 2))
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"outputs\"                       (string, required) A json array of json objects representing the request to "
            "send.\n"
            "[{\n"
            "   \"scid\":side chain ID               (string, required) The uint256 side chain ID\n"
            "   \"vScRequestData\":                  (array, required) It is an arbitrary array of byte strings of even length "
            "expressed in\n"
            "                                           hexadecimal format representing a SC reference (for instance an Utxo "
            "ID) for which a backward transfer is being requested. The size of each string must be \n" +
            strprintf("%d", CFieldElement::ByteSize()) +
            " bytes\n"
            "   \"mcDestinationAddress\":\"address\" (string, required) The Horizen mainchain address where to send the "
            "backward transfer\n"
            "   \"scFee\":amount,                    (numeric, required) The amount in " +
            CURRENCY_UNIT +
            ...
            "\nExamples:\n" +
            HelpExampleCli("sc_request_transfer",
                           "'[{ \"mcDestinationAddress\": \"taddr\", \"vScRequestData\": "
                           "[\"06f75b4e1c1f49e6f329aa23f57e42bf305644b5b85c4d4ac60d7ef3b50679e8\"], \"scid\": "
                           "\"13a3083bdcf42635c8ce5d46c2cae26cfed7dc889d9b4ac0b9939c6631a73bdc\", \"scFee\": 19.0 }]'"));

    LOCK2(cs_main, pwalletMain->cs_wallet);
    RPCTypeCheck(params, boost::assign::list_of(UniValue::VARR)(UniValue::VOBJ));

    // valid keywords in cmd arguments
    static const std::set<std::string> validKeyOutputArray = {"scid", "vScRequestData", "mcDestinationAddress", "scFee"};
    ...
    if (argsArray.size() == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, args arrays is empty.");
    ...
    for (const UniValue& o : argsArray.getValues()) {
        ...
        if (setKeyOutputArray.count("scFee")) {
            ...
            if (!MoneyRange(scFee)) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, amount out of range");
        } else {
        ...
        if (setKeyOutputArray.count("vScRequestData")) {
            ...
            for (auto fe : reqDataValues) {
                ...
                if (!Sidechain::AddScData(vScRequestDataString, vScRequestDataByteArray, CFieldElement::ByteSize(),
                                          Sidechain::CheckSizeMode::CHECK_STRICT, error)) {
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns the total amount received by the given horizenaddress in transactions with at least minconf "
            "confirmations.\n"
    ...
    for (auto it = pwalletMain->getMapWallet().begin(); it != pwalletMain->getMapWallet().end(); ++it) {
        const CWalletTransactionBase& wtx = *((*it).second);
        if (wtx.getTxBase()->IsCoinBase() || !CheckFinalTx(*wtx.getTxBase())) continue;
        ...
        for (unsigned int pos = 0; pos < wtx.getTxBase()->GetVout().size(); ++pos) {
            ...
            if (wtx.getTxBase()->IsCertificate()) {
                if (wtx.IsOutputMature(pos) != CCoins::outputMaturity::MATURE) continue;
            }
...
UniValue getbalance(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"account\"      (string, optional) DEPRECATED. If provided, it MUST be set to the empty string \"\" or to the "
            "string \"*\", either of which will give the total available balance. Passing any other string will result in an "
            "error\n"
            "2. minconf          (numeric, optional, default=1) only include transactions confirmed at least this many times\n"
        ...
        for (auto it = pwalletMain->getMapWallet().begin(); it != pwalletMain->getMapWallet().end(); ++it) {
            ...
            if (wtx->GetDepthInMainChain() >= nMinDepth) {
                for (const COutputEntry& r : listReceived)
                    if (r.maturity == CCoins::outputMaturity::MATURE) nBalance += r.amount;
            }
...
UniValue getunconfirmedbalance(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "n       (numeric) the server's total unconfirmed balance in " +
            CURRENCY_UNIT +
            "\n"

            "\nExamples:\n" +
            HelpExampleCli("getunconfirmedbalance", "") + HelpExampleRpc("getunconfirmedbalance", ""));
...
UniValue sendfrom(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) MUST be set to the empty string \"\" to represent the default "
            "account. Passing any other string will result in an error\n"
            ...
            HelpExampleCli("sendfrom",
                           "\"tabby\" \"znnwwojWQJp1ARgbi1dqYtmnNMfihmg8m1b\" 0.01 6 \"donation\" \"ZenCash outpost\"") +
            "\nAs a json rpc call\n" +
    ...
    if (params.size() > 4 && !params[4].isNull() && !params[4].get_str().empty()) wtx.mapValue["comment"] = params[4].get_str();
    if (params.size() > 5 && !params[5].isNull() && !params[5].get_str().empty()) wtx.mapValue["to"] = params[5].get_str();
...
UniValue sendmany(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "                           Those recipients will receive less Zen than you enter in their corresponding amount "
            "field\n"
            ...
            HelpExampleCli(
                "sendmany",
                "\"\" "
                "\"{\\\"znnwwojWQJp1ARgbi1dqYtmnNMfihmg8m1b\\\":0.01,\\\"znYHqyumkLY3zVwgaHq3sbtHXuP8GxsNws3\\\":0.02}\"") +
            ...
            HelpExampleCli(
                "sendmany",
                "\"\" \"{\\\"znnwwojWQJp1ARgbi1dqYtmnNMfihmg8m1b\\\":0.01,\\\"znYHqyumkLY3zVwgaHq3sbtHXuP8GxsNws3\\\":0.02}\" "
                "1 \"\" \"[\\\"znnwwojWQJp1ARgbi1dqYtmnNMfihmg8m1b\\\",\\\"znYHqyumkLY3zVwgaHq3sbtHXuP8GxsNws3\\\"]\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("sendmany",
                           "\"\", "
                           "\"{\\\"znnwwojWQJp1ARgbi1dqYtmnNMfihmg8m1b\\\":0.01,\\\"znYHqyumkLY3zVwgaHq3sbtHXuP8GxsNws3\\\":0."
                           "02}\", 6, \"testing\""));
    ...
    CWalletTx wtx;
    wtx.strFromAccount = strAccount;
    if (params.size() > 3 && !params[3].isNull() && !params[3].get_str().empty()) wtx.mapValue["comment"] = params[3].get_str();

    UniValue subtractFeeFromAmount(UniValue::VARR);
    if (params.size() > 4) subtractFeeFromAmount = params[4].get_array();
    ...
    for (const string& name_ : keys) {
        ...
        if (!address.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Zen address: ") + name_);
        ...
        for (size_t idx = 0; idx < subtractFeeFromAmount.size(); idx++) {
            ...
            if (addr.get_str() == name_) fSubtractFeeFromAmount = true;
        }
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, dumVecScSend, dumVecFtSend, dumVecBwtRequest, wtx, keyChange,
                                                   nFeeRequired, nChangePosRet, strFailReason);
    if (!fCreated) throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, strFailReason);
    if (!pwalletMain->CommitTransaction(wtx, keyChange)) throw JSONRPCError(RPC_WALLET_ERROR, "Transaction commit failed");
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg =
            ...
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. If provided, MUST be set to the empty string \"\" to represent "
            ...
            HelpExampleCli("addmultisigaddress",
                           "2 \"[\\\"t16sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\",\\\"t171sgjn4YtPu27adkKGrdDwzRTxnRkBfKV\\\"]\"") +
            "\nAs json rpc call\n" +
...
UniValue ListReceived(const UniValue& params, bool fByAccounts) {
    ...
    if (fByAccounts) {
        for (map<string, tallyitem>::iterator it = mapAccountTally.begin(); it != mapAccountTally.end(); ++it) {
            ...
            UniValue obj(UniValue::VOBJ);
            if ((*it).second.fIsWatchonly) obj.pushKV("involvesWatchonly", true);
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. minconf                               (numeric, optional, default=1) the minimum number of confirmations "
            "before payments are included\n"
            "2. includeempty                          (numeric, optional, default=false) whether to include addresses that "
            ...
            "    \"account\": \"accountname\",        (string) DEPRECATED. The account of the receiving address. The default "
            "account is \"\"\n"
            "    \"amount\": xxxx,                    (numeric) the total amount in " +
            CURRENCY_UNIT +
...
UniValue listreceivedbyaccount(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. minconf                          (numeric, optional, default=1) the minimum number of confirmations before "
            "payments are included\n"
            "2. includeempty                     (boolean, optional, default=false) whether to include accounts that haven't "
...
static void MaybePushAddress(UniValue& entry, const CTxDestination& dest) {
    ...
    if (addr.Set(dest)) entry.pushKV("address", addr.ToString());
}

// the flags minedInRange and certMaturingInRange are passed along only when the listsinceblock rpc cmd is used
void ListTransactions(const CWalletTransactionBase& wtx, const string& strAccount, int nMinDepth, bool fLong,
                      UniValue& transactions, const isminefilter& filter, bool includeImmatureBTs, bool minedInRange = true,
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount)) {
        for (const COutputEntry& s : listSent) {
            ...
            if (s.vout != -1) entry.pushKV("vout", s.vout);
            entry.pushKV("fee", ValueFromAmount(-nFee));
            if (fLong) WalletTxToJSON(wtx, entry, filter);
        ...
        for (const COutputEntry& r : listReceived) {
            ...
            if (fAllAccounts || (account == strAccount)) {
                ...
                if (wtx.getTxBase()->IsCoinBase()) {
                    ...
                    if (r.isBackwardTransfer) entry.pushKV("isBackwardTransfer", r.isBackwardTransfer);
                }

                entry.pushKV("amount", ValueFromAmount(r.amount));
                if (r.vout != -1) entry.pushKV("vout", r.vout);
                if (fLong) {
...
UniValue listtransactions(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 6)
        throw runtime_error(
            ...
            "4. includeWatchonly                     (bool, optional, default=false) include transactions to watchonly "
            "addresses (see 'importaddress')\n"
            "5. address                              (string, optional) include only transactions involving this address\n"
            "6. includeImmatureBTs                   (bool, optional, default=false) Whether to include immature certificate "
            "Backward transfers\n"
            ...
            "    \"amount\": xxxx,                    (numeric) the amount in " +
            CURRENCY_UNIT +
            ...
            "    \"fee\": xxxx,                       (numeric) the amount of the fee in " +
            CURRENCY_UNIT +
            ...
            "    \"otheraccount\": \"accountname\",   (string) for the 'move' category of transactions, the account the funds "
            "came \n"
            "                                           from (for receiving funds, positive amounts), or went to (for sending "
            "funds,\n"
            "                                           negative amounts)\n"
    ...
    if (params.size() > 5)
        if (params[5].get_bool()) includeImmatureBTs = true;
...
UniValue getunconfirmedtxdata(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "                         spendability as specified, otherwise the value set by zend option "
            "\'spendzeroconfchange\' \n"
            "                         will be used instead\n"
            "3. includeNonFinalTxes  (boolean, optional, default=true) If true the command will consider also non final txes "
            "in the\n"
    ...
    if (!taddr.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Zen address");
...
UniValue listtxesbyaddress(const UniValue& params, bool fHelp) {
    ...
    bool reverse = false;
    if (params.size() > 3) reverse = params[3].get_bool();
    ...
    if (reverse) std::reverse(arrTmp.begin(), arrTmp.end());  // Return oldest to newest
...
UniValue listaccounts(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "  \"account\": xxxx,   (numeric) the property name is the account name, and the value is the total balance for "
            "the account\n"
            "  ...\n"
            "}\n"
    ...
    for (auto it = pwalletMain->getMapWallet().cbegin(), end = pwalletMain->getMapWallet().end(); it != end; ++it) {
        const CWalletTransactionBase& wtx = *(it->second);
        ...
        if ((wtx.getTxBase()->IsCoinBase() && !wtx.HasMatureOutputs()) || wtx.GetDepthInMainChain() < 0) continue;
        ...
        if (wtx.GetDepthInMainChain() >= nMinDepth) {
            for (const COutputEntry& r : listReceived) {
                if (r.maturity == CCoins::outputMaturity::IMMATURE) continue;
...
UniValue listsinceblock(const UniValue& params, bool fHelp) {
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "2. target-confirmations:               (numeric, optional, default=1) the confirmations required, must be 1 or "
            "more\n"
            "3. includeWatchonly:                   (bool, optional, default=false) include transactions to watchonly "
            "addresses (see 'importaddress')"
            "4. includeImmatureBTs:                 (bool, optional, default=false) Whether to include immature certificate "
            "Backward transfers\n"
            ...
            "    \"address\":\"horizenaddress\",    (string) the horizen address of the transaction. Not present for move "
            "transactions (category = move)\n"
            "    \"category\":\"send|receive\",     (string) the transaction category. 'send' has negative amounts, 'receive' "
            "has positive amounts\n"
            "    \"amount\": xxxx,                  (numeric) the amount in " +
            CURRENCY_UNIT +
            ". This is negative for the 'send' category, and for the 'move' category for moves\n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' "
    ...
    bool includeImmatureBTs = false;
    if (params.size() > 3)
        if (params[3].getBool()) includeImmatureBTs = true;
    ...
    for (auto it = pwalletMain->getMapWallet().begin(); it != pwalletMain->getMapWallet().end(); ++it) {
        ...
        if (tx.getTxBase()->IsCertificate() && depthInMainChain > 0) {
            ...
            LogPrint("cert",
                     "%s():%d - cert[%s]: depthInMc[%d], matHeight[%d], matDepth[%d], cmdDepth[%d], minedInRange[%s], "
                     "matInRange[%s]\n",
                     __func__, __LINE__, tx.getTxBase()->GetHash().ToString(), depthInMainChain, matHeight, matDepth, depth,
                     minedInRange ? "Y" : "N", certMaturingInRange ? "Y" : "N");
        }
...
UniValue gettransaction(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "2. \"includeWatchonly\"                             (bool, optional, default=false) whether to include watchonly "
            "addresses in balance calculation and details[]\n"
            "3. \"includeImmatureBTs\"                           (bool, optional, default=false) Whether to include immature "
            "certificate Backward transfersin balance calculation and details[]\n"

            "\nResult:\n"
            "{\n"
            "  \"version\": n,                                   (numeric) the transaction version \n"
            "  \"amount\": xxxx,                                 (numeric) the transaction amount in " +
            CURRENCY_UNIT +
            "\n"
            ...
            "  \"time\": ttt,                                    (numeric) the transaction time in seconds since epoch (1 Jan "
            "1970 GMT)\n"
            "  \"timereceived\": ttt,                            (numeric) the time received in seconds since epoch (1 Jan "
            "1970 GMT)\n"
            ...
            "     ,...\n"
            "  ],\n"
            "  \"vsc_ccout\" : [                                  (array of json objects) Sidechain creation crosschain "
            "outputs (only if version = -4)\n"
            "     {\n"
            "       \"scid\" : \"hex\",                           (string) The sidechain id\n"
            ...
            "certificate verification, allowed values:\n" +
            ...
            "     ,...\n"
            "  ],\n"
            "  \"vft_ccout\" : [                                  (array of json objects) Sidechain forward transfer "
            "crosschain outputs (only if version = -4)\n"
            "     {\n"
            "       \"scid\" : \"hex\",                           (string) The sidechain id\n"
            "       \"value\" : x.xxx,                            (numeric) The value in " +
            CURRENCY_UNIT +
            ...
            "     ,...\n"
            "  ],\n"
            "  \"vmbtr_out\" : [                                  (array of json objects) Sidechain backward transfer request "
            "outputs (only if version = -4)\n"
            "     {\n"
            "       \"scid\" : \"hex\",                           (string) The sidechain id\n"
            "       \"n\" : n,                                    (numeric) crosschain output index\n"
            ...
            "       }\n"
            "       \"scFee\" :                                   (numeric) The fee in " +
            CURRENCY_UNIT +
            " required to create a Backward Transfer Request to sidechain\n"
            ...
            "    {\n"
            "      \"account\": \"accountname\",                  (string) DEPRECATED. The account name involved in the "
            ...
            "      \"vpub_old\": xxxx                             (numeric) the amount removed from the transparent value "
            "pool\n"
    ...
    bool includeImmatureBTs = false;
    if (params.size() > 2)
        if (params[2].getBool()) includeImmatureBTs = true;
    ...
    if (wtx.IsFromMe(filter)) entry.pushKV("fee", ValueFromAmount(nFee));
...
UniValue backupwallet(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"destination\"   (string, required) the destination filename, saved in the directory set by -exportdir "
            "option\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("backupwallet", "\"destination\"") + HelpExampleRpc("backupwallet", "\"destination\""));
    ...
    if (clean.compare(unclean) != 0) {
        throw JSONRPCError(
            RPC_WALLET_ERROR,
            strprintf("Filename is invalid as only alphanumeric characters are allowed.  Try '%s' instead.", clean));
    }
...
UniValue encryptwallet(const UniValue& params, bool fHelp) {
    ...
    StartShutdown();
    return "wallet encrypted; Horizen server stopping, restart to run with encrypted wallet. The keypool has been flushed, you "
...
UniValue settxfee(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            CURRENCY_UNIT +
            "/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key "
            "in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the "
            "wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": xxxxx,          (numeric) the transaction fee configuration, set in " +
            CURRENCY_UNIT +
            "/kB\n"
            "}\n"
    ...
    if (pwalletMain->IsCrypted()) obj.pushKV("unlocked_until", nWalletUnlockTime);
...
UniValue listunspent(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            HelpExampleCli(
                "listunspent",
                "6 9999999 \"[\\\"t1PGFqEzfmQch1gKD3ra4k18PNj3tTUUSqg\\\",\\\"t1LtvqCaApEdUGFkpKMM4MstjcaL4dKg8SP\\\"]\"") +
            HelpExampleRpc(
                "listunspent",
                "6, 9999999 \"[\\\"t1PGFqEzfmQch1gKD3ra4k18PNj3tTUUSqg\\\",\\\"t1LtvqCaApEdUGFkpKMM4MstjcaL4dKg8SP\\\"]\""));
    ...
    for (const COutput& out : vecOutputs) {
        ...
        if (setAddress.size()) {
            CTxDestination address;
            if (!ExtractDestination(out.tx->getTxBase()->GetVout()[out.pos].scriptPubKey, address)) continue;
        ...
        if (ExtractDestination(out.tx->getTxBase()->GetVout()[out.pos].scriptPubKey, address)) {
            ...
            if (pwalletMain->mapAddressBook.count(address)) entry.pushKV("account", pwalletMain->mapAddressBook[address].name);
        }
...
UniValue z_listunspent(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "When minconf is 0, unspent notes with zero confirmations are returned, even though they are not immediately "
            "spendable.\n"
            ...
            "    \"confirmations\" : n       (numeric) the number of confirmations\n"
            "    \"spendable\" : true|false  (boolean) true if note can be spent by wallet, false if note has zero "
            "confirmations, false if address is watchonly\n"
    ...
    if (params.size() > 3) {
        ...
        if (addresses.size() == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, addresses array is empty.");
        ...
        for (const UniValue& o : addresses.getValues()) {
            ...
            try {
                ...
                if (!fIncludeWatchonly && !pwalletMain->HaveSpendingKey(addr)) {
                    throw JSONRPCError(
                        RPC_INVALID_PARAMETER,
                        string("Invalid parameter, spending key for address does not belong to wallet: ") + address);
                }
                zaddrs.insert(addr);
            } catch (const std::runtime_error&) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   string("Invalid parameter, address is not a valid zaddr: ") + address);
            }
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp) {
    ...
    CTransaction origTx;
    if (!DecodeHexTx(origTx, params[0].get_str())) throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed");
...
UniValue zc_sample_joinsplit(const UniValue& params, bool fHelp) {
    ...
    const int shieldedTxVersion = zen::ForkManager::getInstance().getShieldedTxVersion(chainActive.Height());
    ...
    JSDescription samplejoinsplit(isGroth, *pzcashParams, pubKeyHash, anchor, {JSInput(), JSInput()}, {JSOutput(), JSOutput()},
                                  0, 0);
...
UniValue zc_benchmark(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2) {
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("zcbenchmark", "\"benchmarktype\" 2") + HelpExampleRpc("zcbenchmark", "\"benchmarktype\", 2"));
    }
...
UniValue zc_raw_receive(const UniValue& params, bool fHelp) {
...

    {
        ...
        try {
            ...
            throw runtime_error("encrypted_note could not be decoded");
        }
    }

    ZCNoteDecryption decryptor(k.receiving_key());

    NotePlaintext npt = NotePlaintext::decrypt(decryptor, ct, epk, h_sig, nonce);
    ...
    pwalletMain->WitnessNoteCommitment({commitment}, witnesses, anchor);
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp) {
    ...
    if (params[3].get_real() != 0.0) vpub_old = AmountFromValue(params[3]);

    if (params[4].get_real() != 0.0) vpub_new = AmountFromValue(params[4]);
    ...
    JSDescription jsdesc(mtx.nVersion == GROTH_TX_VERSION, *pzcashParams, joinSplitPubKey, anchor, {vjsin[0], vjsin[1]},
                         {vjsout[0], vjsout[1]}, vpub_old, vpub_new);
    ...
    // Sanity check
    assert(crypto_sign_verify_detached(&mtx.joinSplitSig[0], dataToBeSigned.begin(), 32, mtx.joinSplitPubKey.begin()) == 0);
...
UniValue zc_raw_keygen(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 0) {
        throw runtime_error(
            ...
            + HelpExampleCli("zcrawkeygen", "") + HelpExampleRpc("zcrawkeygen", ""));
    }
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 0)
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("z_getnewaddress", "") + HelpExampleRpc("z_getnewaddress", ""));
...
UniValue z_gettotalbalance(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nCAUTION: If the wallet contains watch-only zaddrs, the returned private balance may be larger than the actual "
            "balance,"
            "\nbecause spends cannot be detected with incoming viewing keys.\n"

            "\nArguments:\n"
            "1. minconf                    (numeric, optional, default=1) only include private and transparent transactions "
            ...
            "'importaddress' and 'z_importviewingkey')\n"
...
UniValue z_getoperationstatus(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "      {\n"
            "           \"status\": \"xxxx\",                 (string) status, can be \"success\", \"failed\", \"cancelled\"\n"
            "           error: {                              (object, optional) if the status is \"failed\", the error object "
            "has key-value pairs (code-message)\n"
...
UniValue z_getoperationstatus_IMPL(const UniValue& params, bool fRemoveFinishedOperations = false) {
    ...
    for (auto id : ids) {
        if (useFilter && !filter.count(id)) continue;
...
#define Z_SENDMANY_MAX_ZADDR_OUTPUTS(TX_VER)                                                                               \
    ((MAX_TX_SIZE /                                                                                                        \
      JSDescription::getNewInstance(TX_VER == GROTH_TX_VERSION).GetSerializeSize(SER_NETWORK, PROTOCOL_VERSION, TX_VER)) - \
     1)
...
UniValue z_sendmany(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "\nWhen sending coinbase UTXOs to a zaddr, change is not allowed. The entire value of the UTXO(s) must be "
            "consumed." +
            ...
            CURRENCY_UNIT +
            " is the value\n"
            "      \"memo\":memo        (string, optional) if the address is a zaddr, raw data represented in hexadecimal "
            "string format\n"
            "    }, ... ]\n"
            "3. minconf                 (numeric, optional, default=1) only use funds confirmed at least this many times\n"
            "4. fee                     (numeric, optional, default=" +
            strprintf("%s", FormatMoney(ASYNC_RPC_OPERATION_DEFAULT_MINERS_FEE)) +
            ") the fee amount to attach to this transaction\n"
            "5. sendChangeToSource      (boolean, optional, default = false) if true and fromaddress is a taddress return the "
            "change to it\n"

            "\nResult:\n"
            "\"operationid\"            (string) an operationid to pass to z_getoperationstatus to get the result of the "
            "operation\n"

            "\nExamples:\n" +
            HelpExampleCli("z_sendmany",
                           "\"znnwwojWQJp1ARgbi1dqYtmnNMfihmg8m1b\" '[{\"address\": "
                           "\"ztfaW34Gj9FrnGUEf833ywDVL62NWXBM81u6EQnM6VR45eYnXhwztecW1SjxA7JrmAXKJhxhj3vDNEpVCQoSvVoSpmbhtjf\""
                           " ,\"amount\": 5.0}]'") +
    ...
    if (outputs.size() == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, amounts array is empty.");
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        if (!memoValue.isNull()) {
            ...
            if (!isZaddr) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   "Memo can not be used with a taddr.  It can only be used with a zaddr.");
            } else if (!IsHex(memo)) {
    ...
    if (txsize > MAX_TX_SIZE) {
        throw JSONRPCError(
            RPC_INVALID_PARAMETER,
            strprintf("Too many outputs, size of raw transaction would be larger than limit of %d bytes", MAX_TX_SIZE));
    }
    ...
    if (params.size() > 3) {
        ...
        if (nFee > nTotalOut) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Fee %s is greater than the sum of outputs %s",
                                                                FormatMoney(nFee), FormatMoney(nTotalOut)));
        }
    }
    ...
    std::shared_ptr<AsyncRPCQueue> q = getAsyncRPCQueue();
    std::shared_ptr<AsyncRPCOperation> operation(new AsyncRPCOperation_sendmany(
        contextualTx, fromaddress, taddrRecipients, zaddrRecipients, nMinDepth, nFee, contextInfo, sendChangeToSource));
...
UniValue sc_send_certificate(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 8)
        throw runtime_error(
            "sc_send_certificate scid epochNumber quality endEpochCumScTxCommTreeRoot scProof transfers forwardTransferScFee "
            "mainchainBackwardTransferScFee [fee] [vFieldElementCertificateField] [vBitVectorCertificateField]\n"
            "\nSend cross chain backward transfers from SC to MC as a certificate."
            "\nArguments:\n"
            " 1. \"scid\"                        (string, required) The uint256 side chain ID\n"
            " 2. epochNumber                     (numeric, required) The epoch number this certificate refers to, zero-based "
            "numbering\n"
            " 3. quality                         (numeric, required) The quality of this withdrawal certificate. \n"
            " 4. \"endEpochCumScTxCommTreeRoot\" (string, required) The hex string representation of the field element "
            "corresponding to the root of the cumulative scTxCommitment tree stored at the block marking the end of the "
            "referenced epoch\n"
            " 5. \"scProof\"                     (string, required) SNARK proof whose verification key wCertVk was set at "
            "sidechain creation. Its size must be " +
            strprintf("%d", CScProof::MaxByteSize()) +
            " bytes max\n"
            " 6. transfers:                      (array, required) An array of json objects representing the amounts of the "
            "backward transfers. Can also be empty\n"
            "     [{\n"
            ...
            "     }, ... ]\n"
            " 7. forwardTransferScFee            (numeric, required) The amount of fee due to sidechain actors when creating a "
            "FT\n"
            " 8. mainchainBackwardTransferScFee  (numeric, required) The amount of fee due to sidechain actors when creating a "
            "MBTR\n"
            " 9. fee                             (numeric, optional) The fee amount of the certificate in " +
            CURRENCY_UNIT +
            ". If it is not specified or has a negative value it is automatically computed using a fixed fee rate (default is "
            "1Zat/Byte)\n"
            "10. fromAddress                     (string, optional) The taddr to send the coins from. If omitted, coins are "
            "chosen among all available UTXOs\n"
            "11. vFieldElementCertificateField   (array, optional) a list of hexadecimal strings each of them representing "
            "data used to verify the SNARK proof of the certificate\n"
            "    [\n"
            "      \"fieldElement\"             (string, required) The HEX string representing generic data\n"
            "    , ... ]\n"
            "12. vBitVectorCertificateField      (array, optional) a list of hexadecimal strings each of them representing a "
            "compressed bit vector used to verify the SNARK proof of the certificate\n"
            "    [\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("sc_send_certificate",
                           "\"054671870079a64a491ea68e08ed7579ec2e0bd148c51c6e2fe6385b597540f4\" 10 7 "
                           "\"0a85efb37d1130009f1b588dcddd26626bbb159ae4a19a703715277b51033144\" \"abcd..ef\" "
                           "'[{\"address\":\"taddr\", \"amount\":33.5}]' 0.00001") +
            HelpExampleCli("sc_send_certificate",
                           "\"ea3e7ccbfd40c4e2304c4215f76d204e4de63c578ad835510f580d529516a874\" 12 5 "
                           "\"04a1527384c67d9fce3d091ababfc1de325dbac9b3b14025a53722ff6c53d40e\" \"abcd..ef\" "
                           "'[{\"address\":\"taddr\" ,\"amount\": 5.0}]'")
    ...
    if (!Sidechain::AddScData(endCumCommTreeStr, aByteArray, CFieldElement::ByteSize(),
                              Sidechain::CheckSizeMode::CHECK_UPPER_LIMIT, errorStr)) {
    ...
    if (ret_code != CValidationState::Code::OK) {
        LogPrintf("%s():%d - ERROR: endEpochCumScTxCommTreeRoot[%s]/epochNumber[%d] are not legal, ret_code[0x%x]\n", __func__,
                  __LINE__, cert.endEpochCumScTxCommTreeRoot.GetHexRepr(), epochNumber, CValidationState::CodeToChar(ret_code));
    ...
    string inputString = params[4].get_str();
    {
        ...
        if (!cert.scProof.IsValid()) throw JSONRPCError(RPC_INVALID_PARAMETER, string("invalid cert scProof"));
    }
    ...
    if (params.size() > 9) {
        ...
        if (!inputString.empty()) {
            ...
            if (!fromaddress.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Zen address(coin to ta from)");
        }
    }
    //--------------------------------------------------------------------------
    ...
    if (params.size() > 10) {
        ...
        if (vFieldElementCertificateFieldConfig.size() != feArray.size()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               strprintf("Invalid parameter, fe array has size %d, but the expected size is %d", feArray.size(),
                                         vFieldElementCertificateFieldConfig.size()));
        }
        ...
        for (const UniValue& o : feArray.getValues()) {
            ...
            if (!Sidechain::AddCustomFieldElement(o.get_str(), fe, nBytes, strError))
                throw JSONRPCError(RPC_TYPE_ERROR,
                                   string("vFieldElementCertificateField [" + std::to_string(count) + "]: ") + strError);
    ...
    if (params.size() > 11) {
        ...
        if (cmtArray.size() != vBitVectorCertificateFieldConfig.size()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               strprintf("Invalid parameter, compr mkl tree array has size %d, but the expected size is %d",
        ...
        for (const UniValue& o : cmtArray.getValues()) {
            ...
            if (!Sidechain::AddScData(o.get_str(), cmt, cmt_size, Sidechain::CheckSizeMode::CHECK_UPPER_LIMIT, errorStr))
                throw JSONRPCError(RPC_TYPE_ERROR,
                                   string("vBitVectorCertificateField [" + std::to_string(count) + "]: ") + errorStr);
    ...
    if (nTotalOut > sidechain.balance + delta) {
        LogPrint("sc", "%s():%d - insufficent balance in scid[%s]: balance[%s], cert amount[%s]\n", __func__, __LINE__,
                 scId.ToString(), FormatMoney(sidechain.balance + delta), FormatMoney(nTotalOut));
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "\ncan be used to return a list of locked utxos.  The number of coinbase utxos selected for shielding can be "
            "limited"
            ...
            strprintf("%d bytes.", MAX_TX_SIZE) + HelpRequiringPassphrase() +
            "\n"

            "\nArguments:\n"
            "1. \"fromaddress\"         (string, required) the address is a taddr or \"*\" for all taddrs belonging to the "
            "wallet\n"
            "2. \"toaddress\"           (string, required) the address is a zaddr\n"
            "3. fee                     (numeric, optional, default=" +
            strprintf("%s", FormatMoney(SHIELD_COINBASE_DEFAULT_MINERS_FEE)) +
            ") The fee amount to attach to this transaction.\n"
            "4. limit                 (numeric, optional, default=" +
            strprintf("%d", SHIELD_COINBASE_DEFAULT_LIMIT) +
            ") Limit on the maximum number of utxos to shield.  Set to 0 to use node option -mempooltxinputlimit\n"

            "\nResult:\n"
            "{\n"
            "  \"operationid\": xxx          (string) an operationid to pass to z_getoperationstatus to get the result of the "
            "operation.\n"
    ...
    for (const COutput& out : vecOutputs) {
        ...
        if (!maxedOutFlag) {
            ...
            if (estimatedTxSize + increase >= MAX_TX_SIZE || (mempoolLimit > 0 && utxoCounter > mempoolLimit)) {
                maxedOutFlag = true;
            } else {
    ...
    if (nFee > netAmount) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Fee %s is greater than the net amount to be shielded %s",
                                                            FormatMoney(nFee), FormatMoney(netAmount)));
    }
    ...
    std::shared_ptr<AsyncRPCOperation> operation(
        new AsyncRPCOperation_shieldcoinbase(contextualTx, inputs, destaddress, nFee, contextInfo));
    q->addOperation(operation);
    AsyncRPCOperationId operationId = operation->getId();
...
#define JOINSPLIT_SIZE \
    JSDescription::getNewInstance(GROTH_TX_VERSION).GetSerializeSize(SER_NETWORK, PROTOCOL_VERSION, GROTH_TX_VERSION)
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            ...
            "3. fee                   (numeric, optional, default=" +
            strprintf("%s", FormatMoney(MERGE_TO_ADDRESS_OPERATION_DEFAULT_MINERS_FEE)) +
            ") The fee amount to attach to this transaction.\n"
            ...
            strprintf("%d", MERGE_TO_ADDRESS_DEFAULT_SHIELDED_LIMIT) +
            ") Limit on the maximum number of notes to merge.  Set to 0 to merge as many as will fit in the transaction.\n"
            "5. \"memo\"                (string, optional) Encoded as hex. When toaddress is a z-addr, this will be stored in "
            "the memo field of the new note.\n"
            "\nResult:\n"
            "{\n"
            ...
            "  \"opid\": xxx          (string) An operationid to pass to z_getoperationstatus to get the result of the "
            "operation.\n"
            "}\n"
            "\nExamples:\n" +
            HelpExampleCli("z_mergetoaddress",
    ...
    if (addresses.size() == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, fromaddresses array is empty.");
    ...
    if (params.size() > 5) {
        ...
        if (memo.length() > ZC_MEMO_SIZE * 2) {
            throw JSONRPCError(
                RPC_INVALID_PARAMETER,
                strprintf("Invalid parameter, size of memo is larger than maximum allowed %d bytes", ZC_MEMO_SIZE));
        }
    }
    ...
    if (useAny || useAnyNote || zaddrs.size() > 0) {
        ...
        for (CNotePlaintextEntry& entry : entries) {
            ...
            if (!maxedOutNotesFlag) {
                ...
                // TODO max JoinSplit per transaction is 58 since Groth16, with one JS == 1698 bytes, IIRC size estimation is
                // also broken for z_sendmany
                size_t increase = (noteInputs.empty() && !isToZaddr) || (noteInputs.size() % 2 == 0) ? JOINSPLIT_SIZE : 0;
                if (estimatedTxSize + increase >= MAX_TX_SIZE || (nNoteLimit > 0 && noteCounter > nNoteLimit)) {
    ...
    if (mergedValue < nFee) {
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                           strprintf("Insufficient funds, have %s, which is less than miners fee %s", FormatMoney(mergedValue),
                                     FormatMoney(nFee)));
    }
...
UniValue z_listoperationids(const UniValue& params, bool fHelp) {
    ...
    for (auto id : ids) {
        ...
        if (useFilter && filter.compare(state) != 0) continue;
        ret.push_back(id);
    }
