...
bool CCrypter::SetKeyFromPassphrase(const SecureString& strKeyData, const std::vector<unsigned char>& chSalt,
    ...
    if (nDerivationMethod == 0)
        i = EVP_BytesToKey(EVP_aes_256_cbc(), EVP_sha512(), &chSalt[0], (unsigned char*)&strKeyData[0], strKeyData.size(),
                           nRounds, chKey, chIV);
...
bool CCrypter::SetKey(const CKeyingMaterial& chNewKey, const std::vector<unsigned char>& chNewIV) {
    if (chNewKey.size() != WALLET_CRYPTO_KEY_SIZE || chNewIV.size() != WALLET_CRYPTO_KEY_SIZE) return false;
...
static bool EncryptSecret(const CKeyingMaterial& vMasterKey, const CKeyingMaterial& vchPlaintext, const uint256& nIV,
...
static bool DecryptSecret(const CKeyingMaterial& vMasterKey, const std::vector<unsigned char>& vchCiphertext,
...
static bool DecryptKey(const CKeyingMaterial& vMasterKey, const std::vector<unsigned char>& vchCryptedSecret,
                       const CPubKey& vchPubKey, CKey& key) {
    CKeyingMaterial vchSecret;
    if (!DecryptSecret(vMasterKey, vchCryptedSecret, vchPubKey.GetHash(), vchSecret)) return false;
...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey, const std::vector<unsigned char>& vchCryptedSecret,
                               const libzcash::PaymentAddress& address, libzcash::SpendingKey& sk) {
    CKeyingMaterial vchSecret;
    if (!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret)) return false;

    if (vchSecret.size() != libzcash::SerializedSpendingKeySize) return false;
...
bool CCryptoKeyStore::SetCrypted() {
    ...
    if (!(mapKeys.empty() && mapSpendingKeys.empty())) return false;
    fUseCrypto = true;
    return true;
}
...
bool CCryptoKeyStore::AddSpendingKey(const libzcash::SpendingKey& sk) {
    {
        ...
        if (!IsCrypted()) return CBasicKeyStore::AddSpendingKey(sk);
        ...
        if (!EncryptSecret(vMasterKey, vchSecret, address.GetHash(), vchCryptedSecret)) return false;

        if (!AddCryptedSpendingKey(address, sk.receiving_key(), vchCryptedSecret)) return false;
    }
    return true;
}

bool CCryptoKeyStore::AddCryptedSpendingKey(const libzcash::PaymentAddress& address, const libzcash::ReceivingKey& rk,
...
bool CCryptoKeyStore::GetSpendingKey(const libzcash::PaymentAddress& address, libzcash::SpendingKey& skOut) const {
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted()) return CBasicKeyStore::GetSpendingKey(address, skOut);
...
bool CCryptoKeyStore::EncryptKeys(CKeyingMaterial& vMasterKeyIn) {
    {
        ...
        for (KeyMap::value_type& mKey : mapKeys) {
            ...
            CKeyingMaterial vchSecret(key.begin(), key.end());
            std::vector<unsigned char> vchCryptedSecret;
            if (!EncryptSecret(vMasterKeyIn, vchSecret, vchPubKey.GetHash(), vchCryptedSecret)) return false;
        ...
        for (SpendingKeyMap::value_type& mSpendingKey : mapSpendingKeys) {
            ...
            std::vector<unsigned char> vchCryptedSecret;
            if (!EncryptSecret(vMasterKeyIn, vchSecret, address.GetHash(), vchCryptedSecret)) return false;
            if (!AddCryptedSpendingKey(address, sk.receiving_key(), vchCryptedSecret)) return false;
        }
