    ...
    try {
        if (obj.getTxBase()->IsCertificate()) {
            LogPrint("cert", "%s():%d - called for cert[%s], writing to db\n", __func__, __LINE__,
                     obj.getTxBase()->GetHash().ToString());
...
bool CWalletDB::EraseWalletTxBase(const uint256& hash) {
    ...
    return (Erase(std::make_pair(std::string("tx"), hash)) && Erase(std::make_pair(std::string("cert"), hash)));
}
...
bool CWalletDB::EraseSidechain(const uint256& scId) { return Erase(std::make_pair(std::string("sc"), scId)); }
...
bool CWalletDB::WriteCryptedZKey(const libzcash::PaymentAddress& addr, const libzcash::ReceivingKey& rk,
                                 const std::vector<unsigned char>& vchCryptedSecret, const CKeyMetadata& keyMeta) {
    const bool fEraseUnencryptedKey = true;
    nWalletDBUpdated++;

    if (!Write(std::make_pair(std::string("zkeymeta"), addr), keyMeta)) return false;

    if (!Write(std::make_pair(std::string("czkey"), addr), std::make_pair(rk, vchCryptedSecret), false)) return false;
    if (fEraseUnencryptedKey) {
...
bool CWalletDB::ReadPool(int64_t nPool, CKeyPool& keypool) { return Read(std::make_pair(std::string("pool"), nPool), keypool); }
...
DBErrors CWalletDB::ReorderTransactions(CWallet* pwallet) {
    ...
    for (TxItems::iterator it = txByTime.begin(); it != txByTime.end(); ++it) {
        ...
        if (nOrderPos == -1) {
            ...
            if (pwtx) {
                if (!WriteWalletTxBase(pwtx->getTxBase()->GetHash(), *pwtx)) return DB_LOAD_FAIL;
...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType,
    ...
    try {
        ...
        if (strType == "name") {
            ...
            if (!(CheckTransaction(wtx.getWrappedTx(), state, verifier) && (wtx.getWrappedTx().GetHash() == hash) &&
                  state.IsValid())) {
                LogPrintf("%s():%d - failure: tx id = %s, reject code = %d\n", __func__, __LINE__,
                          wtx.getWrappedTx().GetHash().ToString(), CValidationState::CodeToChar(state.GetRejectCode()));
                // Don't consider CValidationState::Code::REJECT_CHECKBLOCKATHEIGHT_NOT_FOUND error code as a failure. It can
                // appear because a tx is a pre-chainsplit tx, so it is perfectly fine in this case.
                if (state.GetRejectCode() != CValidationState::Code::CHECKBLOCKATHEIGHT_NOT_FOUND) return false;
            }
            ...
            ssValue >> wcert;
            CValidationState state;
            if (!(CheckCertificate(wcert.getWrappedCert(), state) && (wcert.getWrappedCert().GetHash() == hash) &&
                  state.IsValid())) {
                LogPrint("cert", "%s():%d - cert[%s] is invalid\n", __func__, __LINE__,
                         wcert.getWrappedCert().GetHash().ToString());
                return false;
            }
            ...
            char fYes;
            ssValue >> fYes;
            if (fYes == '1') pwallet->LoadViewingKey(vk);
...
static bool IsKeyType(const string& strType) {
    return (strType == "key" || strType == "wkey" || strType == "zkey" || strType == "czkey" || strType == "vkey" ||
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet) {
    ...
    LogPrintf("ZKeys: %u plaintext, %u encrypted, %u w/metadata, %u total\n", wss.nZKeys, wss.nCZKeys, wss.nZKeyMeta,
              wss.nZKeys + wss.nCZKeys);

    // nTimeFirstKey is only reliable if all keys have metadata
    if ((wss.nKeys + wss.nCKeys) != wss.nKeyMeta) pwallet->nTimeFirstKey = 1;  // 0 would be considered 'no value'

    for (const uint256& hash : wss.vWalletUpgrade) WriteWalletTxBase(hash, *(pwallet->getMapWallet().at(hash)));

    // Rewrite encrypted wallets of versions 0.4.0 and 0.5.0rc:
    if (wss.fIsEncrypted && (wss.nFileVersion == 40000 || wss.nFileVersion == 50000)) return DB_NEED_REWRITE;
...
void ThreadFlushWalletDB(const string& strFile) {
    ...
    fOneThread = true;
    if (!GetBoolArg("-flushwallet", true)) return;
...
bool CWalletDB::Recover(CDBEnv& dbenv, const std::string& filename) { return CWalletDB::Recover(dbenv, filename, false); }
