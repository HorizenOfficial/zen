...
class SidechainsCertInWalletTestSuite : public ::testing::Test {
  public:
    ...
    void SetUp() override {
        ...
        try {
            pWalletDb = new CWalletDB(pWallet->strWalletFile, "cr+");
    ...
    void TearDown() override {
        ...
        EXPECT_TRUE(DB_LOAD_OK == nZapWalletRet) << "Failed cleaning-up the wallet with return code" << nZapWalletRet
  ...
  protected:
    ...
    void SetLockingScriptFor(CTransaction& tx) {
        ...
        CScript lockingScript = CScript() << OP_DUP << OP_HASH160 << ToByteVector(coinsKey.GetPubKey().GetID())
                                          << OP_EQUALVERIFY << OP_CHECKSIG;

        for (unsigned int pos = 0; pos < mutTx.getVout().size(); ++pos) mutTx.getOut(pos).scriptPubKey = lockingScript;
    ...
    void SetLockingScriptFor(CScCertificate& cert) {
        ...
        for (unsigned int pos = 0; pos < mutCert.getVout().size(); ++pos) mutCert.getOut(pos).scriptPubKey = lockingScript;
...
TEST_F(SidechainsCertInWalletTestSuite, WalletCertSerializationOps) {
    CWallet dummyWallet;
    CScCertificate cert = txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 0, CFieldElement{SAMPLE_FIELD},
...
TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_TransparentTx_InBlockChain) {
    ...
    EXPECT_TRUE(txMaturity == CCoins::outputMaturity::MATURE) << "txMaturity is " << int(txMaturity);
}
...
TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_CoinBase_InBlockChain) {
    ...
    coinBaseMaturity = walletCoinBase.IsOutputMature(0);
    EXPECT_TRUE(coinBaseMaturity == CCoins::outputMaturity::MATURE) << "coinBaseMaturity is " << int(coinBaseMaturity);

    // Test no hysteresis
    chainSettingUtils::ExtendChainActiveToHeight(coinBaseCreationHeight + COINBASE_MATURITY - 1);
    coinBaseMaturity = walletCoinBase.IsOutputMature(0);
    EXPECT_TRUE(coinBaseMaturity == CCoins::outputMaturity::IMMATURE) << "coinBaseMaturity is " << int(coinBaseMaturity);
}

TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_Certificate_InBlockChain) {
    // Create certificate
    CScCertificate cert = txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 12, CFieldElement{SAMPLE_FIELD},
                                                             /*changeTotalAmount*/ CAmount(4), /*numChangeOut*/ 2,
                                                             /*bwTotaltAmount*/ CAmount(10), /*numBwt*/ 4,
                                                             /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    ...
    for (int height = certCreationHeight; height < certCreationHeight + walletCert.bwtMaturityDepth; ++height) {
        ...
        EXPECT_TRUE(bwtOutputMaturity == CCoins::outputMaturity::IMMATURE)
            << "bwtOutputMaturity for output" << cert.GetVout().size() - 1 << " at height " << height << " is "
            << int(bwtOutputMaturity);
    }
    ...
    changeOutputMaturity = walletCert.IsOutputMature(0);
    EXPECT_TRUE(changeOutputMaturity == CCoins::outputMaturity::MATURE)
        << "changeOutputMaturity for output 0 at height " << certCreationHeight + walletCert.bwtMaturityDepth << " is "
        << int(changeOutputMaturity);

    bwtOutputMaturity = walletCert.IsOutputMature(cert.GetVout().size() - 1);
    EXPECT_TRUE(bwtOutputMaturity == CCoins::outputMaturity::MATURE)
        << "bwtOutputMaturity for output" << cert.GetVout().size() - 1 << " at height "
        << certCreationHeight + walletCert.bwtMaturityDepth << " is " << int(bwtOutputMaturity);
...
TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_Certificate_InMemoryPool) {
    ...
    // Test
    changeOutputMaturity = walletCert.IsOutputMature(0);
    EXPECT_TRUE(changeOutputMaturity == CCoins::outputMaturity::MATURE) << "certMaturity is " << int(changeOutputMaturity);

    bwtOutputMaturity = walletCert.IsOutputMature(cert.GetVout().size() - 1);
    EXPECT_TRUE(bwtOutputMaturity == CCoins::outputMaturity::NOT_APPLICABLE) << "certMaturity is " << int(bwtOutputMaturity);
}
...
TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_TransparentTx_Conflicted) {
    ...
    // Test
    txMaturity = walletTx.IsOutputMature(0);
    EXPECT_TRUE(txMaturity == CCoins::outputMaturity::NOT_APPLICABLE) << "txMaturity is " << int(txMaturity);
}
...
TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_CoinBase_Conflicted) {
    ...
    CCoins::outputMaturity coinBaseMaturity = CCoins::outputMaturity::NOT_APPLICABLE;
    coinBaseMaturity = walletCoinBase.IsOutputMature(0);
    EXPECT_TRUE(coinBaseMaturity == CCoins::outputMaturity::NOT_APPLICABLE) << "coinBaseMaturity is " << int(coinBaseMaturity);
}
...
TEST_F(SidechainsCertInWalletTestSuite, IsOutputMature_Certificate_Conflicted) {
    ...
    bwtOutputMaturity = walletCert.IsOutputMature(cert.GetVout().size() - 1);
    EXPECT_TRUE(bwtOutputMaturity == CCoins::outputMaturity::NOT_APPLICABLE) << "txMaturity is " << int(bwtOutputMaturity);
}
...
TEST_F(SidechainsCertInWalletTestSuite, GetCredit_CoinBase) {
    ...
    for (int height = coinBaseCreationHeight + 1; height < coinBaseCreationHeight + COINBASE_MATURITY; ++height) {
        ...
        EXPECT_TRUE(CAmount(0) == coinBaseCredit) << "coinBaseCredit at height " << height << " is " << coinBaseCredit;
    }

    // Test
    chainSettingUtils::ExtendChainActiveToHeight(coinBaseCreationHeight + COINBASE_MATURITY);
    coinBaseCredit = walletCoinBase.GetCredit(ISMINE_SPENDABLE);
    EXPECT_TRUE(coinBaseAmount == coinBaseCredit) << "coinBaseCredit is " << int(coinBaseCredit);

    // Test no hysteresis
    for (int height = coinBaseCreationHeight + COINBASE_MATURITY - 1; height >= coinBaseCreationHeight; --height) {
        chainSettingUtils::ExtendChainActiveToHeight(height);
        coinBaseCredit = walletCoinBase.GetCredit(ISMINE_SPENDABLE);
        EXPECT_TRUE(CAmount(0) == coinBaseCredit) << "coinBaseCredit at height " << height << " is " << int(coinBaseCredit);
    }
}
...
TEST_F(SidechainsCertInWalletTestSuite, GetCredit_FullCertificate_NotVoided) {
    ...
    CScCertificate cert = txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 0, CFieldElement{SAMPLE_FIELD},
                                                             /*changeTotalAmount*/ changeAmount, /*numChangeOut*/ 2,
                                                             /*bwtTotalAmount*/ bwtAmount, /*numBwt*/ 4,
                                                             /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    ...
    for (int height = certCreationHeight; height < certCreationHeight + walletCert.bwtMaturityDepth; ++height) {
        ...
        EXPECT_TRUE(changeAmount == certCredit) << "certCredit at height " << height << " is " << certCredit;
    }

    // Test
    chainSettingUtils::ExtendChainActiveToHeight(certCreationHeight + walletCert.bwtMaturityDepth);
    certCredit = walletCert.GetCredit(ISMINE_SPENDABLE);
    EXPECT_TRUE((bwtAmount + changeAmount) == certCredit) << "certCredit is " << int(certCredit);

    // Test no hysteresis
    for (int height = certCreationHeight + walletCert.bwtMaturityDepth - 1; height >= certCreationHeight; --height) {
        chainSettingUtils::ExtendChainActiveToHeight(height);
        certCredit = walletCert.GetCredit(ISMINE_SPENDABLE);
        EXPECT_TRUE(changeAmount == certCredit) << "certCredit is " << int(certCredit);
    }
}
...
TEST_F(SidechainsCertInWalletTestSuite, GetCredit_BwtOnlyCertificate_NotVoided) {
    ...
    CAmount bwtAmount = 12;
    CScCertificate cert = txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 0, CFieldElement{SAMPLE_FIELD},
                                                             /*changeTotalAmount*/ changeAmount, /*numChangeOut*/ 0,
                                                             /*bwtTotalAmount*/ bwtAmount, /*numBwt*/ 4,
                                                             /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    SetLockingScriptFor(cert);
    ...
    for (int height = certCreationHeight; height < certCreationHeight + walletCert.bwtMaturityDepth; ++height) {
        chainSettingUtils::ExtendChainActiveToHeight(height);
        certCredit = walletCert.GetCredit(ISMINE_SPENDABLE);
        EXPECT_TRUE(CAmount(0) == certCredit) << "certCredit at height " << height << " is " << certCredit;
    }

    // Test
    chainSettingUtils::ExtendChainActiveToHeight(certCreationHeight + walletCert.bwtMaturityDepth);
    certCredit = walletCert.GetCredit(ISMINE_SPENDABLE);
    EXPECT_TRUE(bwtAmount == certCredit) << "certCredit is " << int(certCredit);

    // Test no hysteresis
    for (int height = certCreationHeight + walletCert.bwtMaturityDepth - 1; height >= certCreationHeight; --height) {
        chainSettingUtils::ExtendChainActiveToHeight(height);
        certCredit = walletCert.GetCredit(ISMINE_SPENDABLE);
        EXPECT_TRUE(CAmount(0) == certCredit) << "certCredit is " << int(certCredit);
    }
}
...
TEST_F(SidechainsCertInWalletTestSuite, GetCredit_NoBwtCertificate_NotVoided) {
    ...
    CScCertificate cert = txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 0, CFieldElement{SAMPLE_FIELD},
                                                             /*changeTotalAmount*/ changeAmount, /*numChangeOut*/ 2,
                                                             /*bwtTotalAmount*/ bwtAmount, /*numBwt*/ 0,
                                                             /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    SetLockingScriptFor(cert);
...
TEST_F(SidechainsCertInWalletTestSuite, GetCredit_FullCertificate_Voided) {
    ...
    CAmount bwtAmount = 12;
    CScCertificate cert =
        txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 0, CFieldElement{SAMPLE_FIELD},
...
TEST_F(SidechainsCertInWalletTestSuite, GetImmatureCredit_CoinBase) {
    ...
    chainSettingUtils::ExtendChainActiveToHeight(coinBaseCreationHeight + COINBASE_MATURITY);
    coinBaseImmatureCredit = walletCoinBase.GetImmatureCredit(/*fUseCache*/ true);
    EXPECT_TRUE(CAmount(0) == coinBaseImmatureCredit) << "coinBaseImmatureCredit is " << coinBaseImmatureCredit;
...
TEST_F(SidechainsCertInWalletTestSuite, GetImmatureCredit_FullCertificate_NotVoided) {
    ...
    chainSettingUtils::ExtendChainActiveToHeight(certCreationHeight + walletCert.bwtMaturityDepth);
    certImmatureCredit = walletCert.GetImmatureCredit(/*fUseCache*/ true);
    EXPECT_TRUE(CAmount(0) == certImmatureCredit) << "certImmatureCredit is " << int(certImmatureCredit);

    // Test no hysteresis
    for (int height = certCreationHeight + walletCert.bwtMaturityDepth - 1; height >= certCreationHeight; --height) {
        chainSettingUtils::ExtendChainActiveToHeight(height);
        certImmatureCredit = walletCert.GetImmatureCredit(/*fUseCache*/ true);
        EXPECT_TRUE(bwtAmount == certImmatureCredit) << "certImmatureCredit is " << int(certImmatureCredit);
    }
}
...
TEST_F(SidechainsCertInWalletTestSuite, SyncCertificate) {
    ...
    CScCertificate cert = txCreationUtils::createCertificate(uint256S("aaa"), /*epochNum*/ 0, CFieldElement{SAMPLE_FIELD},
                                                             /*changeTotalAmount*/ changeAmount, /*numChangeOut*/ 2,
                                                             /*bwtTotalAmount*/ bwtAmount, /*numBwt*/ 2,
                                                             /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    SetLockingScriptFor(cert);
...
TEST_F(SidechainsCertInWalletTestSuite, SyncCertStatusInfo) {
    ...
    // test
    CScCertificateStatusUpdateInfo certUpdateInfo(cert.GetScId(), cert.GetHash(), cert.epochNumber, cert.quality,
