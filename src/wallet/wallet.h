...
class JSOutPoint {
  public:
    ...
    friend bool operator<(const JSOutPoint& a, const JSOutPoint& b) {
        return (a.hash < b.hash || (a.hash == b.hash && a.js < b.js) || (a.hash == b.hash && a.js == b.js && a.n < b.n));
    }
    ...
    friend bool operator!=(const JSOutPoint& a, const JSOutPoint& b) { return !(a == b); }
...
class CNoteData {
  public:
    ...
    CNoteData(libzcash::PaymentAddress a) : address{a}, nullifier(), witnessHeight{-1} {}
    CNoteData(libzcash::PaymentAddress a, uint256 n) : address{a}, nullifier{n}, witnessHeight{-1} {}
    ...
    friend bool operator<(const CNoteData& a, const CNoteData& b) {
        return (a.address < b.address || (a.address == b.address && a.nullifier < b.nullifier));
    }
    ...
    friend bool operator!=(const CNoteData& a, const CNoteData& b) { return !(a == b); }
};
...
class CWalletTransactionBase {
  ...
  public:
    ...
    friend bool operator==(const CWalletTransactionBase& a, const CWalletTransactionBase& b) {
        if (a.getTxBase()->IsCertificate() && !b.getTxBase()->IsCertificate()) return false;
        if (!a.getTxBase()->IsCertificate() && b.getTxBase()->IsCertificate()) return false;
    ...
    void GetMatureAmountsForAccount(const std::string& strAccount, CAmount& nReceived, CAmount& nSent, CAmount& nFee,
    ...
    virtual void GetAmounts(std::list<COutputEntry>& listReceived, std::list<COutputEntry>& listSent, CAmount& nFee,
                            std::string& strSentAccount, const isminefilter& filter) const = 0;
...
class CWalletTx : public CWalletTransactionBase {
  ...
  public:
    ...
    friend bool operator==(const CWalletTx& a, const CWalletTx& b) { return a.wrappedTx == b.wrappedTx; }

    ADD_SERIALIZE_METHODS;

    template <typename Stream, typename Operation>
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        if (ser_action.ForRead()) CWalletTransactionBase::Reset(nullptr);
        char fSpent = false;
    ...
    void GetAmounts(std::list<COutputEntry>& listReceived, std::list<COutputEntry>& listSent, CAmount& nFee,
                    std::string& strSentAccount, const isminefilter& filter) const override;
...
class CWalletCert : public CWalletTransactionBase {
  ...
  public:
    ...
    friend bool operator==(const CWalletCert& a, const CWalletCert& b) { return a.wrappedCertificate == b.wrappedCertificate; }
...
class CWallet : public CCryptoKeyStore, public CValidationInterface {
  private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTransactionBase*, unsigned int>>& setCoinsRet,
  ...
  public:
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed = true, const CCoinControl* coinControl = nullptr,
                        bool fIncludeZeroValue = false, bool fIncludeCoinBase = true, bool fIncludeCommunityFund = true) const;
    ...
    bool LoadCryptedZKey(const libzcash::PaymentAddress& addr, const libzcash::ReceivingKey& rk,
                         const std::vector<unsigned char>& vchCryptedSecret);
    //! Adds an encrypted spending key to the store, and saves it to disk (virtual method, declared in crypter.h)
    bool AddCryptedSpendingKey(const libzcash::PaymentAddress& address, const libzcash::ReceivingKey& rk,
    ...
    void GetUnconfirmedData(const std::string& address, int& numbOfUnconfirmedTx, CAmount& unconfInput, CAmount& unconfOutput,
                            CAmount& bwtImmatureOutput, eZeroConfChangeUsage zconfchangeusage, bool fIncludeNonFinal) const;
    ...
    bool CreateTransaction(
        const std::vector<CRecipient>& vecSend, const std::vector<Sidechain::CRecipientScCreation>& vecScSend,
        const std::vector<Sidechain::CRecipientForwardTransfer>& vecFtSend,
        const std::vector<Sidechain::CRecipientBwtRequest>& vecBwtRequest, CWalletTx& wtxNew, CReserveKey& reservekey,
        CAmount& nFeeRet, int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl = NULL,
        bool sign = true,
    ...
    CAmount GetCredit(const CWalletTransactionBase& txWalletBase, const isminefilter& filter, bool& fCanBeCached,
    ...
    //! signify that a particular wallet feature is now used. this may change nWalletVersion and nWalletMaxVersion if those are
    //! lower
    ...
    void GetFilteredNotes(std::vector<CNotePlaintextEntry>& outEntries, std::string address, int minDepth = 1,
                          bool ignoreSpent = true, bool ignoreUnspendable = true);

    /* Find notes filtered by payment addresses, min depth, ability to spend */
    void GetFilteredNotes(std::vector<CNotePlaintextEntry>& outEntries, std::set<libzcash::PaymentAddress>& filterAddresses,
                          int minDepth = 1, bool ignoreSpent = true, bool ignoreUnspendable = true);
    ...
    void GetUnspentFilteredNotes(std::vector<CUnspentNotePlaintextEntry>& outEntries,
                                 std::set<libzcash::PaymentAddress>& filterAddresses, int minDepth = 1, int maxDepth = INT_MAX,
