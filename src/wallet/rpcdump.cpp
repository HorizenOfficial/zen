...
UniValue importaddress(const UniValue& params, bool fHelp) {
...

    {
        ...
        if (address.IsValid()) pwalletMain->SetAddressBook(address.Get(), strLabel, "receive");
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys) {
    ...
    while (pindex && pindex->pprev && pindex->GetBlockTime() > nTimeBegin - TIMESTAMP_WINDOW) pindex = pindex->pprev;
...
UniValue dumpprivkey(const UniValue& params, bool fHelp) {
    ...
    if (!address.SetString(strAddress)) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Horizen address");
    CKeyID keyID;
    if (!address.GetKeyID(keyID)) throw JSONRPCError(RPC_TYPE_ERROR, "Address does not refer to a key");
    CKey vchSecret;
...
UniValue z_exportwallet(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExports all wallet keys, for taddr and zaddr, in a human-readable format.  Overwriting an existing file is not "
            "permitted.\n"
            "\nArguments:\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("z_exportwallet", "\"filename\"") + HelpExampleRpc("z_exportwallet", "\"filename\""));
...
UniValue dumpwallet(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("dumpwallet", "\"filename\"") + HelpExampleRpc("dumpwallet", "\"filename\""));
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys) {
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the zend -exportdir option has been set");
    }
    ...
    if (fDumpZKeys) {
        ...
        for (auto addr : addresses) {
            ...
            if (pwalletMain->GetSpendingKey(addr, key)) {
                ...
                file << strprintf("%s %s # zaddr=%s\n", CZCSpendingKey(key).ToString(), strTime,
                                  CZCPaymentAddress(addr).ToString());
            }
        }
        file << "\n";
...
UniValue z_importkey(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "2. rescan             (string, optional, default=\"whenkeyisnew\") rescan the wallet for transactions - can be "
    ...
    if (params.size() > 1) {
        ...
        if (rescan.compare("whenkeyisnew") != 0) {
            ...
            if (rescan.compare("yes") == 0) {
                ...
                // Handle older API
                UniValue jVal;
                if (!jVal.read(std::string("[") + rescan + std::string("]")) || !jVal.isArray() || jVal.size() != 1 ||
                    !jVal[0].isBool()) {
    ...
    int nRescanHeight = 0;
    if (params.size() > 2) nRescanHeight = params[2].get_int();
...

    {
        ...
        if (pwalletMain->HaveSpendingKey(addr)) {
            ...
            if (!pwalletMain->AddZKey(key)) throw JSONRPCError(RPC_WALLET_ERROR, "Error adding spending key to wallet");
...
UniValue z_exportkey(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("z_exportkey", "\"myaddress\"") + HelpExampleCli("z_importkey", "\"zkey\"") +
...
UniValue z_exportviewingkey(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("z_exportviewingkey", "\"zaddr\"") + HelpExampleRpc("z_exportviewingkey", "\"zaddr\""));
