...
void AddSidechainOutsToJSON(const CTransaction& tx, UniValue& parentObj) {
    ...
    for (unsigned int i = 0; i < tx.GetVscCcOut().size(); i++) {
        ...
        if (out.constant.has_value()) o.pushKV("constant", out.constant->GetHexRepr());
        if (out.wCeasedVk.has_value()) {
            o.pushKV("cswProvingSystem", Sidechain::ProvingSystemTypeToString(out.wCeasedVk->getProvingSystemType()));
            o.pushKV("wCeasedVk", out.wCeasedVk->GetHexRepr());
        }
...
bool AddCustomFieldElement(const std::string& inputString, std::vector<unsigned char>& vBytes, unsigned int nBytes,
...
bool AddScData(const std::string& inputString, std::vector<unsigned char>& vBytes, unsigned int vSize,
...
bool AddCeasedSidechainWithdrawalInputs(UniValue& csws, CMutableTransaction& rawTx, std::string& error) {
    ...
    for (size_t i = 0; i < csws.size(); i++) {
        ...
        if (!AddScData(nullifier_v.get_str(), nullifierVec, CFieldElement::ByteSize(), CheckSizeMode::CHECK_STRICT,
                       nullifierError)) {
        ...
        if (!valActCertData.isNull() && !valActCertData.get_str().empty()) {
            std::string errStr;
            if (!AddScData(valActCertData.get_str(), vActCertData, CFieldElement::ByteSize(), CheckSizeMode::CHECK_STRICT,
                           errStr)) {
        ...
        if (!AddScData(valCumTree.get_str(), vCeasingCumScTxCommTree, CFieldElement::ByteSize(), CheckSizeMode::CHECK_STRICT,
                       errStr)) {
        ...
        CTxCeasedSidechainWithdrawalInput csw_input(amount, scId, nullifier, pubKeyHash, scProof, actCertDataHash,
                                                    ceasingCumScTxCommTree, CScript());
...
bool AddSidechainCreationOutputs(UniValue& sc_crs, CMutableTransaction& rawTx, std::string& error) {
    ...
    for (size_t i = 0; i < sc_crs.size(); i++) {
        ...
        if (!constant.isNull()) {
            ...
            if (!AddScData(inputString, scConstantByteArray, CFieldElement::ByteSize(), CheckSizeMode::CHECK_UPPER_LIMIT,
                           error)) {
        ...
        if (!CmtCfg.isNull()) {
            ...
            for (auto& pairEntry : BitVectorSizesPairArray.getValues()) {
                ...
                sc.vBitVectorCertificateFieldConfig.push_back(
        ...
        if (!uniMbtrDataLength.isNull()) {
            ...
            if (mbtrDataLength < 0 || mbtrDataLength > MAX_SC_MBTR_DATA_LEN) {
                error = strprintf("Invalid mainchainBackwardTransferRequestDataLength: out of range [%d, %d]", 0,
...
bool AddSidechainBwtRequestOutputs(UniValue& bwtreq, CMutableTransaction& rawTx, std::string& error) {
    ...
    for (size_t j = 0; j < bwtreq.size(); j++) {
        ...
        for (UniValue inputElement : vScRequestDataVal.get_array().getValues()) {
            ...
            if (!Sidechain::AddScData(inputElement.get_str(), requestDataByteArray, CFieldElement::ByteSize(),
                                      CheckSizeMode::CHECK_STRICT, error)) {
...
ScRpcCmd::ScRpcCmd(const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf, const CAmount& nFee)
    : _fromMcAddress(fromaddress),
      _changeMcAddress(changeaddress),
      _minConf(minConf),
      _fee(nFee),
      _feeNeeded(-1),
      _automaticFee(false),
...
void ScRpcCmd::init() { _totalInputAmount = 0; }
...
void ScRpcCmd::addInputs() {
    ...
    pwalletMain->AvailableCoins(vAvailableCoins, fOnlyConfirmed, NULL, fIncludeZeroValue, fIncludeCoinBase,
                                fIncludeCommunityFund);

    for (const auto& out : vAvailableCoins) {
        LogPrint("sc", "utxo %s depth: %5d, val: %12s, spendable: %s\n", out.tx->getTxBase()->GetHash().ToString(), out.nDepth,
                 FormatMoney(out.tx->getTxBase()->GetVout()[out.pos].nValue), out.fSpendable ? "Y" : "N");
    ...
    std::sort(vInputUtxo.begin(), vInputUtxo.end(),
              [](SelectedUTXO i, SelectedUTXO j) -> bool { return (std::get<2>(i) < std::get<2>(j)); });
    ...
    for (const SelectedUTXO& t : vInputUtxo) {
        ...
        LogPrint("sc", "---> added tx %s val: %12s, vout.n: %d\n", std::get<0>(t).ToString(), FormatMoney(std::get<2>(t)),
                 std::get<1>(t));
    ...
    if (_totalInputAmount < targetAmount) {
        ...
        if (_hasFromAddress) addrDetails = strprintf(" for taddr[%s]", _fromMcAddress.ToString());

        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                           strprintf("Insufficient transparent funds %s, have %s, need %s (minconf=%d)", addrDetails,
                                     FormatMoney(_totalInputAmount), FormatMoney(targetAmount), _minConf));
    }

    // If there is transparent change, is it valid or is it dust?
    if (dustChange < _dustThreshold && dustChange != 0) {
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                           strprintf("Insufficient transparent funds, have %s, need %s more to avoid creating invalid change "
                                     "output %s (dust threshold is %s)",
                                     FormatMoney(_totalInputAmount), FormatMoney(_dustThreshold - dustChange),
                                     FormatMoney(dustChange), FormatMoney(_dustThreshold)));
    }
        ...
        if (_hasChangeAddress) {
            ...
            if (!keyChange.GetReservedKey(vchPubKey))
                throw JSONRPCError(
                    ...
                    "Could not generate a taddr to use as a change address");  // should never fail, as we just unlocked
...
ScRpcCmdCert::ScRpcCmdCert(CMutableScCertificate& cert, const std::vector<sBwdParams>& bwdParams,
                           const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf,
                           const CAmount& nFee, const std::vector<FieldElementCertificateField>& vCfe,
                           const std::vector<BitVectorCertificateField>& vCmt, const CAmount& ftScFee, const CAmount& mbtrScFee)
    ...
    if (_fee < _feeNeeded) {
        if (!_automaticFee) {
            ...
            if (_fee < ::minRelayTxFee.GetFee(nSize)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("tx with size %d has too low a fee: %lld < minrelaytxfee "
                                                                    "%lld, the miner might not include it in a block",
                                                                    nSize, _fee, ::minRelayTxFee.GetFee(nSize)));
            } else {
                LogPrintf("%s():%d - Warning: using a fee(%lld) < minimum(%lld) (tx size = %d)\n", __func__, __LINE__, _fee,
                          _feeNeeded, nSize);
            }
        } else {
...
bool ScRpcCmd::send() {
    ...
    if (nSize > getMaxObjSize()) {
        ...
        throw JSONRPCError(RPC_VERIFY_ERROR, strprintf("tx/cert size %d > max size(%d)", nSize, getMaxObjSize()));
    }
...
void ScRpcCmdCert::addBackwardTransfers() {
    for (const auto& entry : _bwdParams) {
        ...
        if (txout.IsDust(::minRelayTxFee)) throw JSONRPCError(RPC_WALLET_ERROR, "backward transfer amount too small");
        _cert.addBwt(txout);
    }
}

void ScRpcCmdCert::addCustomFields() {
    if (!_vCfe.empty()) _cert.vFieldElementCertificateField = _vCfe;
    if (!_vCmt.empty()) _cert.vBitVectorCertificateField = _vCmt;
}
...
ScRpcCmdTx::ScRpcCmdTx(CMutableTransaction& tx, const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress,
...
ScRpcCreationCmdTx::ScRpcCreationCmdTx(CMutableTransaction& tx, const std::vector<sCrOutParams>& outParams,
                                       const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf,
                                       const CAmount& nFee, const CAmount& ftScFee, const CAmount& mbtrScFee,
                                       const ScFixedParameters& cd)
    : ScRpcCmdTx(tx, fromaddress, changeaddress, minConf, nFee),
      _fixedParams(cd),
...
ScRpcSendCmdTx::ScRpcSendCmdTx(CMutableTransaction& tx, const std::vector<sFtOutParams>& outParams,
                               const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf,
                               const CAmount& nFee)
...
ScRpcRetrieveCmdTx::ScRpcRetrieveCmdTx(CMutableTransaction& tx, const std::vector<sBtOutParams>& outParams,
                                       const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf,
                                       const CAmount& nFee)
    : ScRpcCmdTx(tx, fromaddress, changeaddress, minConf, nFee), _outParams(outParams) {
    for (const auto& entry : _outParams) {
