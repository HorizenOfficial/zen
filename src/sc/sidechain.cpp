...
void Sidechain::ClearSidechainsFolder() {
    ...
    for (boost::filesystem::directory_iterator it(Sidechain::GetSidechainDataDir());
        ...
        if (is_regular_file(*it)) remove(it->path());
    }
}
...
int CSidechain::GetCertSubmissionWindowLength() const { return std::max(2, fixedParams.withdrawalEpochLength / 5); }
...
int CSidechain::GetScheduledCeasingHeight() const { return GetCertSubmissionWindowEnd(lastTopQualityCertReferencedEpoch + 1); }
...
std::string CSidechain::ToString() const {
    ...
    str = strprintf(
        ...
        fixedParams.version, creationBlockHeight, creationTxHash.ToString(), pastEpochTopQualityCertView.ToString(),
        lastTopQualityCertView.ToString(), lastTopQualityCertHash.ToString(), lastTopQualityCertReferencedEpoch,
        lastTopQualityCertQuality, FormatMoney(lastTopQualityCertBwtAmount), FormatMoney(balance));
...
bool Sidechain::checkTxSemanticValidity(const CTransaction& tx, CValidationState& state) {
    ...
    if (!tx.IsScVersion()) {
        if (!tx.ccIsNull()) {
            return state.DoS(100, error("mismatch between transaction version and sidechain output presence"),
        ...
        if (tx.GetVjoinsplit().size() > 0) {
            return state.DoS(100, error("mismatch between transaction version and joinsplit presence"),
                             CValidationState::Code::INVALID, "sidechain-tx-version");
        }
    }
    ...
    for (const auto& sc : tx.GetVscCcOut()) {
        ...
        for (const auto& config : sc.vFieldElementCertificateFieldConfig) {
            if (!config.IsValid())
                return state.DoS(
                    100,
                    error(
                        "%s():%d - ERROR: Invalid tx[%s], invalid config parameters for vFieldElementCertificateFieldConfig\n",
                        __func__, __LINE__, txHash.ToString()),
                    CValidationState::Code::INVALID, "sidechain-sc-creation-invalid-custom-config");
        }
        ...
        if (!MoneyRange(sc.forwardTransferScFee)) {
            return state.DoS(100,
                             error("%s():%d - ERROR: Invalid tx[%s], forwardTransferScFee out of range [%d, %d]\n", __func__,
                                   __LINE__, txHash.ToString(), 0, MAX_MONEY),
        ...
        if (sc.mainchainBackwardTransferRequestDataLength < 0 ||
            sc.mainchainBackwardTransferRequestDataLength > MAX_SC_MBTR_DATA_LEN) {
            return state.DoS(
                100,
        ...
        if (bt.vScRequestData.size() > MAX_SC_MBTR_DATA_LEN) {
            return state.DoS(100,
                             error("%s():%d - ERROR: Invalid tx[%s], vScRequestData size out of range [%d, %d]\n", __func__,
                                   __LINE__, txHash.ToString(), 0, MAX_SC_MBTR_DATA_LEN),
                             CValidationState::Code::INVALID, "sidechain-sc-bwt-invalid-request-data");
        }
    ...
    for (const CTxCeasedSidechainWithdrawalInput& csw : tx.GetVcswCcIn()) {
        if (csw.nValue == 0 || !MoneyRange(csw.nValue)) {
            return state.DoS(100,
                             error("%s():%d - ERROR: Invalid tx[%s] : CSW value %d is non-positive or out of range\n", __func__,
                                   __LINE__, txHash.ToString(), csw.nValue),
                             CValidationState::Code::INVALID, "sidechain-cswinput-value-not-valid");
        }

        if (!csw.nullifier.IsValid()) {
            return state.DoS(
                100, error("%s():%d - ERROR: Invalid tx[%s] : invalid CSW nullifier\n", __func__, __LINE__, txHash.ToString()),
        ...
        if (!csw.scProof.IsValid()) {
            return state.DoS(
                100, error("%s():%d - ERROR: Invalid tx[%s] : invalid CSW proof\n", __func__, __LINE__, txHash.ToString()),
...
bool Sidechain::checkCertSemanticValidity(const CScCertificate& cert, CValidationState& state) {
    ...
    if (cert.quality < 0) {
        return state.DoS(
            100, error("%s():%d - ERROR: Invalid cert[%s], negative quality\n", __func__, __LINE__, certHash.ToString()),
            CValidationState::Code::INVALID, "bad-cert-quality-negative");
    }

    if (cert.epochNumber < 0) {
        return state.DoS(
            100, error("%s():%d - ERROR: Invalid cert[%s], negative epoch number\n", __func__, __LINE__, certHash.ToString()),
    ...
    if (!cert.endEpochCumScTxCommTreeRoot.IsValid()) {
        return state.DoS(100,
                         error("%s():%d - ERROR: Invalid cert[%s], invalid endEpochCumScTxCommTreeRoot [%s]\n", __func__,
                               __LINE__, certHash.ToString(), cert.endEpochCumScTxCommTreeRoot.GetHexRepr()),
...
bool Sidechain::checkCertCustomFields(const CSidechain& sidechain, const CScCertificate& cert) {
    ...
    if (vCfeCfg.size() != vCfe.size() || vCmtCfg.size() != vCmt.size()) {
        LogPrint("sc", "%s():%d - invalid custom field cfg sz: %d/%d - %d/%d\n", __func__, __LINE__, vCfeCfg.size(),
                 vCfe.size(), vCmtCfg.size(), vCmt.size());
        return false;
    }
...
int CSidechain::getNumBlocksForScFeeCheck() {
    if ((Params().NetworkIDString() == "regtest")) {
        ...
        LogPrint("sc", "%s():%d - %s: val %d is negative, using default %d\n", __func__, __LINE__, Params().NetworkIDString(),
                 val, Params().ScNumBlocksForScFeeCheck());
    }
...
void CSidechain::InitScFees() {
    ...
    if (maxSizeOfScFeesContainers == -1) {
        ...
        Sidechain::ScFeeData defaultData(lastTopQualityCertView.forwardTransferScFee,
...
void CSidechain::DumpScFees() const {
    for (const auto& entry : scFees) {
        std::cout << "[" << std::setw(2) << entry.forwardTxScFee << "/" << std::setw(2) << entry.mbtrTxScFee << "]";
    }
    std::cout << std::endl;
}
...
CAmount CSidechain::GetMinFtScFee() const {
    ...
    CAmount minScFee =
        std::min_element(scFees.begin(), scFees.end(), [](const Sidechain::ScFeeData& a, const Sidechain::ScFeeData& b) {
