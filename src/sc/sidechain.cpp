    ...
    if (tx.vft_ccout.size() )
    {
        ...
        BOOST_FOREACH(const auto& sc, tx.vft_ccout)
        {
            ...
            if (!getScInfo(sc.scId, info) )
            {
                ...
                {
                    LogPrint("sc", "%s():%d - tx[%s]: scid[%s] is being created with the same tx as fwd tr\n",
                        __func__, __LINE__, txHash.ToString(), sc.scId.ToString() );
                }
            }
        }
...
bool ScMgr::checkSidechainBackwardTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    {
        const uint256& txHash = tx.GetHash();
        const uint256& scId = tx.sc_cert.scId;
        const CAmount& totalAmount = tx.sc_cert.totalAmount;
        ...
        if (verifyingDb() )
        {
            ...
            // A workaround could be storing in scinfo the tx hash with the bw amount and check against that
        }
        else
        {
...
bool ScMgr::checkTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    {
        if (!tx.ccIsNull() || !tx.sc_cert.IsNull() )
        {
            return state.DoS(100,
...
bool ScMgr::checkCertificateInMemPool(CTxMemPool& pool, const CTransaction& tx)
{
    if (tx.IsCoinCertified() )
    {
        const uint256& txHash = tx.GetHash();
        const uint256& scId = tx.sc_cert.scId;
        const CAmount& certAmount = tx.sc_cert.totalAmount;
        ...
        for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = pool.mapTx.begin(); it != pool.mapTx.end(); ++it)
        {
            ...
            if (mpTx.nVersion == SC_TX_VERSION)
            {
                ...
                if (mpTx.IsCoinCertified() && scId == mpTx.sc_cert.scId)
                {
                    LogPrint("sc", "%s():%d - tx[%s]: bwd[%s]\n",
                        __func__, __LINE__, mpTx.GetHash().ToString(), FormatMoney(mpTx.sc_cert.totalAmount) );
                    scBalance -= mpTx.sc_cert.totalAmount;
                }
