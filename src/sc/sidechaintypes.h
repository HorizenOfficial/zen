...
static const int MAX_SC_CUSTOM_DATA_LEN =
    1024; /**< Maximum data length for custom data (optional attribute for sidechain creation) in bytes. */
static const int MAX_SC_MBTR_DATA_LEN = 16; /**< Maximum number of field elements contained in a mainchain backward transfer
                                               request (optional attribute for sidechain creation). */
...
struct CZendooBatchProofVerifierResultPtrDeleter {  // deleter
...
///////////////////////////////// CFieldElement ////////////////////////////////
struct CFieldPtrDeleter {  // deleter
...
class CFieldElement : public CZendooCctpObject {
  public:
    ...
    explicit CFieldElement(const uint256& value);  // Currently for backward compability with pre-sidechain fork blockHeader. To
                                                   // re-evaluate its necessity
...
/////////////////////////////////// CScProof ///////////////////////////////////
struct CProofPtrDeleter {  // deleter
...
//////////////////////////////////// CScVKey ///////////////////////////////////
struct CVKeyPtrDeleter {  // deleter
...
class CScVKey : public CZendooCctpObject {
  public:
    ...
    // do not check wrapped ptr
    bool operator==(const CScVKey& rhs) const {
        return isBaseEqual(rhs) && getProvingSystemType() == rhs.getProvingSystemType();
    }
...
class FieldElementCertificateFieldConfig : public CustomCertificateFieldConfig {
  ...
  public:
    ...
    bool operator==(const FieldElementCertificateFieldConfig& rhs) const { return (this->nBits == rhs.nBits); }

    bool operator!=(const FieldElementCertificateFieldConfig& rhs) const { return !(*this == rhs); }
...
class BitVectorCertificateFieldConfig : public CustomCertificateFieldConfig {
  ...
  public:
    ...
    int32_t getBitVectorSizeBits() const { return bitVectorSizeBits; }

    int32_t getMaxCompressedSizeBytes() const { return maxCompressedSizeBytes; }
    ...
    bool operator!=(const BitVectorCertificateFieldConfig& rhs) const { return !(*this == rhs); }
...
class FieldElementCertificateField : public CustomCertificateField<FieldElementCertificateFieldConfig> {
  ...
  public:
    ...
    ~FieldElementCertificateField() {
        delete pReferenceCfg;
        pReferenceCfg = nullptr;
    };
    ...
    const CFieldElement& GetFieldElement(const FieldElementCertificateFieldConfig& cfg,
                                         uint8_t sidechainVersion) const override;
...
struct ScFixedParameters {
    ...
    inline bool operator==(const ScFixedParameters& rhs) const {
        return (version == rhs.version) && (withdrawalEpochLength == rhs.withdrawalEpochLength) &&
               (customData == rhs.customData) && (constant == rhs.constant) && (wCertVk == rhs.wCertVk) &&
...
struct ScBwtRequestParameters {
    ...
    bool IsNull() const { return (scFee == 0 && vScRequestData.empty()); }
    ...
    inline bool operator==(const ScBwtRequestParameters& rhs) const {
        return (scFee == rhs.scFee) && (vScRequestData == rhs.vScRequestData);
    }
...
struct CRecipientScCreation : public CRecipientCrossChainBase {
    ...
    CRecipientScCreation() : ftScFee(-1), mbtrScFee(-1) {}
};
