...
bool AddCustomFieldElement(const std::string& inputString, std::vector<unsigned char>& vBytes, unsigned int vSize,
...
class ScRpcCmd {
  ...
  public:
    ...
    ScRpcCmd(const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf, const CAmount& nFee);
...
class ScRpcCmdTx : public ScRpcCmd {
  ...
  public:
    ScRpcCmdTx(CMutableTransaction& tx, const CBitcoinAddress& fromaddress, const CBitcoinAddress& changeaddress, int minConf,
               const CAmount& nFee);
...
class ScRpcCmdCert : public ScRpcCmd {
  ...
  public:
    struct sBwdParams {
        ...
        sBwdParams(const CScript& spk, const CAmount nAmount) : _scriptPubKey(spk), _nAmount(nAmount) {}
    };
    ...
    ScRpcCmdCert(CMutableScCertificate& cert, const std::vector<sBwdParams>& bwdParams, const CBitcoinAddress& fromaddress,
                 const CBitcoinAddress& changeaddress, int minConf, const CAmount& nFee,
...
class ScRpcCreationCmdTx : public ScRpcCmdTx {
  ...
  public:
    struct sCrOutParams {
        ...
        sCrOutParams(const uint256& toaddress, const CAmount nAmount) : _toScAddress(toaddress), _nAmount(nAmount) {}
    };
    ...
    ScRpcCreationCmdTx(CMutableTransaction& tx, const std::vector<sCrOutParams>& outParams, const CBitcoinAddress& fromaddress,
                       const CBitcoinAddress& changeaddress, int minConf, const CAmount& nFee, const CAmount& ftScFee,
                       const CAmount& mbtrScFee, const ScFixedParameters& cd);
};
...
class ScRpcSendCmdTx : public ScRpcCmdTx {
  ...
  public:
    struct sFtOutParams {
        ...
        sFtOutParams(const uint256& scId, const uint256& toaddress, const CAmount nAmount, const uint160& mcReturnAddress)
    ...
    ScRpcSendCmdTx(CMutableTransaction& tx, const std::vector<sFtOutParams>& outParams, const CBitcoinAddress& fromaddress,
                   const CBitcoinAddress& changeaddress, int minConf, const CAmount& nFee);
};
...
class ScRpcRetrieveCmdTx : public ScRpcCmdTx {
  ...
  public:
    struct sBtOutParams {
        ...
        sBtOutParams(const uint256& scId, const uint160& pkh, const ScBwtRequestParameters& params)
    ...
    ScRpcRetrieveCmdTx(CMutableTransaction& tx, const std::vector<sBtOutParams>& outParams, const CBitcoinAddress& fromaddress,
                       const CBitcoinAddress& changeaddress, int minConf, const CAmount& nFee);
};
