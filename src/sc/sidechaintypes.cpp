...
CZendooBatchProofVerifierResult::CZendooBatchProofVerifierResult(ZendooBatchProofVerifierResult* result) : resultPtr(result) {
    assert(result != nullptr);
}

bool CZendooBatchProofVerifierResult::Result() const { return resultPtr->result; }
...
void CZendooCctpLibraryChecker::CheckTypeSizes() {
    if (Sidechain::SC_FE_SIZE_IN_BYTES != zendoo_get_field_size_in_bytes()) {
        LogPrintf("%s():%d - ERROR: unexpected CCTP field element size: %d (rust lib returns %d)\n", __func__, __LINE__,
                  Sidechain::SC_FE_SIZE_IN_BYTES, zendoo_get_field_size_in_bytes());
    ...
    if (Sidechain::MAX_SC_CUSTOM_DATA_LEN != zendoo_get_sc_custom_data_size_in_bytes()) {
        LogPrintf("%s():%d - ERROR: unexpected CCTP custom data size: %d (rust lib returns %d)\n", __func__, __LINE__,
                  Sidechain::MAX_SC_CUSTOM_DATA_LEN, zendoo_get_sc_custom_data_size_in_bytes());
        assert(!"ERROR: custom data size mismatch between rust CCTP lib and c header!");
    }
}

const std::vector<unsigned char>& CZendooCctpObject::GetByteArray() const { return byteVector; }
...
const unsigned char* const CZendooCctpObject::GetDataBuffer() const {
    if (GetByteArray().empty()) return nullptr;

    return &GetByteArray()[0];
}

int CZendooCctpObject::GetDataSize() const { return GetByteArray().size(); }
...
bool CFieldElement::IsValid() const {
    if (this->GetFieldElement() == nullptr) return false;

    return true;
}

CFieldElement CFieldElement::ComputeHash(const CFieldElement& lhs, const CFieldElement& rhs) {
    if (!lhs.IsValid() || !rhs.IsValid()) throw std::runtime_error("Could not compute poseidon hash on null field elements");
...
bool CScProof::IsValid() const {
    if (this->GetProofPtr() == nullptr) return false;
...
bool CScVKey::IsValid() const {
    if (this->GetVKeyPtr() == nullptr) return false;
...
bool FieldElementCertificateFieldConfig::IsValid() const {
    ...
    */
    static_assert(std::numeric_limits<decltype(nBits)>::max() <= CFieldElement::ByteSize() * 8,
                  "Restore the IF check if ByteSize*8 is greater than the max value of a uint8_t variable");
...
uint8_t FieldElementCertificateFieldConfig::getBitSize() const { return nBits; }
...
bool BitVectorCertificateFieldConfig::IsValid() const {
    ...
    if (!isBitVectorSizeValid) {
        LogPrintf("%s():%d - Invalid or null bitVectorSizeBits=%d (MAX_BIT_VECTOR_SIZE_BITS=%d)\n", __func__, __LINE__,
                  bitVectorSizeBits, MAX_BIT_VECTOR_SIZE_BITS);
        return false;
    }
    ...
    if (merkleTreeLeaves & (merkleTreeLeaves - 1)) return false;
    ...
    if (!isMaxCompressedSizeValid) {
        LogPrintf("%s():%d - Invalid or null maxCompressedSizeBytes=%d (MAX_COMPRESSED_SIZE_BYTES=%d)\n", __func__, __LINE__,
                  maxCompressedSizeBytes, MAX_COMPRESSED_SIZE_BYTES);
        return false;
    }
...
BitVectorCertificateFieldConfig::BitVectorCertificateFieldConfig(int32_t bitVectorSizeBitsIn, int32_t maxCompressedSizeBytesIn)
    : CustomCertificateFieldConfig(), bitVectorSizeBits(bitVectorSizeBitsIn), maxCompressedSizeBytes(maxCompressedSizeBytesIn) {
...
const CFieldElement& FieldElementCertificateField::GetFieldElement(const FieldElementCertificateFieldConfig& cfg,
                                                                   uint8_t sidechainVersion) const {
    ...
    */
    static_assert(std::numeric_limits<std::result_of<decltype (&FieldElementCertificateFieldConfig::getBitSize)(
                          FieldElementCertificateFieldConfig)>::type>::max() <= CFieldElement::ByteSize() * 8,
                  "Restore the assert check if BitSize is greater than the max value of a uint8_t variable");

    int bytes = getBytesFromBits(cfg.getBitSize(), rem);

    if (vRawData.size() != bytes) {
        LogPrint("sc", "%s():%d - ERROR: wrong size: data[%d] != cfg[%d]\n", __func__, __LINE__, vRawData.size(),
                 cfg.getBitSize());
        return fieldElement;
    }
    ...
    if (rem) {
        ...
        if (numbOfZeroBits < (CHAR_BIT - rem)) {
            LogPrint("sc", "%s():%d - ERROR: wrong number of null bits in last byte[0x%x]: %d vs %d\n", __func__, __LINE__,
                     lastByte, numbOfZeroBits, (CHAR_BIT - rem));
            return fieldElement;
        }
    }
...
const CFieldElement& BitVectorCertificateField::GetFieldElement(const BitVectorCertificateFieldConfig& cfg,
                                                                uint8_t sidechainVersion) const {
    if (state != VALIDATION_STATE::NOT_INITIALIZED) {
        assert(pReferenceCfg != nullptr);
        if (*pReferenceCfg == cfg) {
    ...
    if (fe == nullptr) {
        LogPrint("sc",
                 "%s():%d - ERROR(%d): could not get merkle root field el from compr bit vector of size %d, exp uncompr size "
                 "%d (rem=%d)\n",
...
bool Sidechain::IsValidProvingSystemType(uint8_t val) { return IsValidProvingSystemType(static_cast<ProvingSystemType>(val)); }
...
Sidechain::ProvingSystemType Sidechain::StringToProvingSystemType(const std::string& str) {
    if (str == PROVING_SYS_TYPE_COBOUNDARY_MARLIN) return ProvingSystemType::CoboundaryMarlin;
    if (str == PROVING_SYS_TYPE_DARLIN) return ProvingSystemType::Darlin;
