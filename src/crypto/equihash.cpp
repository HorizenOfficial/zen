...
int Equihash<N, K>::InitialiseState(eh_HashState& base_state) {
    ...
    return crypto_generichash_blake2b_init_salt_personal(&base_state, NULL, 0,  // No key.
                                                         (512 / N) * N / 8,
                                                         NULL,  // No salt.
                                                         personalization);
}
...
void CompressArray(const unsigned char* in, size_t in_len, unsigned char* out, size_t out_len, size_t bit_len,
                   size_t byte_pad) {
    assert(bit_len >= 8);
    assert(8 * sizeof(uint32_t) >= 7 + bit_len);
...
StepRow<WIDTH>::StepRow(const unsigned char* hashIn, size_t hInLen, size_t hLen, size_t cBitLen) {
    assert(hLen <= WIDTH);
...
template <size_t WIDTH>
FullStepRow<WIDTH>::FullStepRow(const unsigned char* hashIn, size_t hInLen, size_t hLen, size_t cBitLen, eh_index i)
...
FullStepRow<WIDTH>::FullStepRow(const FullStepRow<W>& a, const FullStepRow<W>& b, size_t len, size_t lenIndices, int trim)
    ...
    for (int i = trim; i < len; i++) hash[i - trim] = a.hash[i] ^ b.hash[i];
...
template <size_t WIDTH>
TruncatedStepRow<WIDTH>::TruncatedStepRow(const unsigned char* hashIn, size_t hInLen, size_t hLen, size_t cBitLen, eh_index i,
                                          unsigned int ilen)
    : StepRow<WIDTH>{hashIn, hInLen, hLen, cBitLen} {
...
template <size_t WIDTH>
template <size_t W>
TruncatedStepRow<WIDTH>::TruncatedStepRow(const TruncatedStepRow<W>& a, const TruncatedStepRow<W>& b, size_t len,
                                          size_t lenIndices, int trim)
    : StepRow<WIDTH>{a} {
    assert(len + lenIndices <= W);
    assert(len - trim + (2 * lenIndices) <= WIDTH);
    for (int i = trim; i < len; i++) hash[i - trim] = a.hash[i] ^ b.hash[i];
...
bool Equihash<N, K>::BasicSolve(const eh_HashState& base_state,
    ...
    for (eh_index g = 0; X.size() < init_size; g++) {
        ...
        for (eh_index i = 0; i < IndicesPerHashOutput && X.size() < init_size; i++) {
            X.emplace_back(tmpHash + (i * N / 8), N / 8, HashLength, CollisionBitLength, (g * IndicesPerHashOutput) + i);
        }
    ...
    for (int r = 1; r < K && X.size() > 0; r++) {
        ...
        while (i < X.size() - 1) {
            ...
            int j = 1;
            while (i + j < X.size() && HasCollision(X[i], X[i + j], CollisionByteLength)) {
                j++;
            }
    ...
    if (X.size() > 1) {
        ...
        while (i < X.size() - 1) {
            int j = 1;
            while (i + j < X.size() && HasCollision(X[i], X[i + j], hashLen)) {
                j++;
            }
...
template <size_t WIDTH>
void CollideBranches(std::vector<FullStepRow<WIDTH>>& X, const size_t hlen, const size_t lenIndices, const unsigned int clen,
                     const unsigned int ilen, const eh_trunc lt, const eh_trunc rt) {
    int i = 0;
    int posFree = 0;
    ...
    while (i < X.size() - 1) {
        // 2b) Find next set of unordered pairs with collisions on the next n/(k+1) bits
        int j = 1;
        while (i + j < X.size() && HasCollision(X[i], X[i + j], clen)) {
            j++;
        }
...
bool Equihash<N, K>::OptimisedSolve(const eh_HashState& base_state,
    ...
    int invalidCount = 0;
    {
        ...
        for (eh_index g = 0; Xt.size() < init_size; g++) {
            ...
            for (eh_index i = 0; i < IndicesPerHashOutput && Xt.size() < init_size; i++) {
                Xt.emplace_back(tmpHash + (i * N / 8), N / 8, HashLength, CollisionBitLength, (g * IndicesPerHashOutput) + i,
        ...
        for (int r = 1; r < K && Xt.size() > 0; r++) {
            ...
            while (i < Xt.size() - 1) {
                // 2b) Find next set of unordered pairs with collisions on the next n/(k+1) bits
                int j = 1;
                while (i + j < Xt.size() && HasCollision(Xt[i], Xt[i + j], CollisionByteLength)) {
                    j++;
                }
                ...
                for (int l = 0; l < j - 1; l++) {
                    for (int m = l + 1; m < j; m++) {
                        ...
                        if (!(Xi.IsZero(hashLen - CollisionByteLength) &&
                              IsProbablyDuplicate<soln_size>(
                                  Xi.GetTruncatedIndices(hashLen - CollisionByteLength, 2 * lenIndices), 2 * lenIndices))) {
                            Xc.emplace_back(Xi);
                        }
                    }
                }
        ...
        if (Xt.size() > 1) {
            ...
            while (i < Xt.size() - 1) {
                int j = 1;
                while (i + j < Xt.size() && HasCollision(Xt[i], Xt[i + j], hashLen)) {
                    j++;
                }
    ...
    for (std::shared_ptr<eh_trunc> partialSoln : partialSolns) {
        ...
        for (eh_index i = 0; i < soln_size; i++) {
            ...
            for (eh_index j = 0; j < recreate_size; j++) {
                ...
                if (j == 0 || newIndex % IndicesPerHashOutput == 0) {
                    GenerateHash(base_state, newIndex / IndicesPerHashOutput, tmpHash, HashOutput);
                }
                icv.emplace_back(tmpHash + ((newIndex % IndicesPerHashOutput) * N / 8), N / 8, HashLength, CollisionBitLength,
                                 newIndex);
            ...
            for (size_t r = 0; r <= K; r++) {
                ...
                if (r < X.size()) {
                    if (X[r]) {
                        ...
                        size_t lti = rti - (1 << r);
                        CollideBranches(*ic, hashLen, lenIndices, CollisionByteLength, CollisionBitLength + 1,
        ...
        for (auto soln : solns) {
            if (validBlock(soln)) return true;
        }
