 ...
 * deque
 */
template<typename T, typename A> unsigned int GetSerializeSize(const std::deque<T, A>& m, int nType, int nVersion);
template<typename Stream, typename T, typename A> void Serialize(Stream& os, const std::deque<T, A>& m, int nType, int nVersion);
template<typename Stream, typename T, typename A> void Unserialize(Stream& is, std::deque<T, A>& m, int nType, int nVersion);
 ...
 */
template<typename T, typename A>
unsigned int GetSerializeSize(const std::deque<T, A>& l, int nType, int nVersion)
{
    unsigned int nSize = GetSizeOfCompactSize(l.size());
    for (typename std::deque<T, A>::const_iterator it = l.begin(); it != l.end(); ++it)
        nSize += GetSerializeSize((*it), nType, nVersion);
    return nSize;
}

template<typename Stream, typename T, typename A>
void Serialize(Stream& os, const std::deque<T, A>& l, int nType, int nVersion)
{
    WriteCompactSize(os, l.size());
    for (typename std::deque<T, A>::const_iterator it = l.begin(); it != l.end(); ++it)
        Serialize(os, (*it), nType, nVersion);
...
template<typename Stream, typename T, typename A>
void Unserialize(Stream& is, std::deque<T, A>& l, int nType, int nVersion)
{
    l.clear();
    unsigned int nSize = ReadCompactSize(is);
