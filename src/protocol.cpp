...
bool CMessageHeader::IsValid(const MessageStartChars& pchMessageStartIn) const {
    // Check start string
    if (memcmp(pchMessageStart, pchMessageStartIn, MESSAGE_START_SIZE) != 0) return false;
    ...
    // Message size
    if (nMessageSize > MAX_SERIALIZED_COMPACT_SIZE) {
        LogPrintf("CMessageHeader::IsValid(): (%s, %u bytes) nMessageSize > MAX_SERIALIZED_COMPACT_SIZE\n", GetCommand(),
                  nMessageSize);
        return false;
    }
...
const char* CInv::GetCommand() const {
    if (!IsKnownType()) throw std::out_of_range(strprintf("CInv::GetCommand(): type=%d unknown type", type));
