...
void static BatchWriteAnchor(CLevelDBBatch& batch, const uint256& croot, const ZCIncrementalMerkleTree& tree,
                             const bool& entered) {
    if (!entered)
...
void static BatchWriteHashBestChain(CLevelDBBatch& batch, const uint256& hash) { batch.Write(DB_BEST_BLOCK, hash); }

void static BatchWriteHashBestAnchor(CLevelDBBatch& batch, const uint256& hash) { batch.Write(DB_BEST_ANCHOR, hash); }

void static BatchWriteCswNullifier(CLevelDBBatch& batch, const uint256& scId, const CFieldElement& nullifier,
                                   CCswNullifiersCacheEntry state) {
...
CCoinsViewDB::CCoinsViewDB(std::string dbName, size_t nCacheSize, bool fMemory, bool fWipe)
    : db(GetDataDir() / dbName, nCacheSize, fMemory, fWipe, false, 64) {}

CCoinsViewDB::CCoinsViewDB(size_t nCacheSize, bool fMemory, bool fWipe)
    : db(GetDataDir() / "chainstate", nCacheSize, fMemory, fWipe, false, 64) {}
...
bool CCoinsViewDB::GetCoins(const uint256& txid, CCoins& coins) const { return db.Read(make_pair(DB_COINS, txid), coins); }

bool CCoinsViewDB::HaveCoins(const uint256& txid) const { return db.Exists(make_pair(DB_COINS, txid)); }
...
bool CCoinsViewDB::HaveSidechain(const uint256& scId) const { return db.Exists(std::make_pair(DB_SIDECHAINS, scId)); }

bool CCoinsViewDB::HaveSidechainEvents(int height) const { return db.Exists(std::make_pair(DB_CEASEDSCS, height)); }
...
uint256 CCoinsViewDB::GetBestAnchor() const {
    ...
    if (!db.Read(DB_BEST_ANCHOR, hashBestAnchor)) return ZCIncrementalMerkleTree::empty_root();
...
bool CCoinsViewDB::BatchWrite(CCoinsMap& mapCoins, const uint256& hashBlock, const uint256& hashAnchor, CAnchorsMap& mapAnchors,
                              CNullifiersMap& mapNullifiers, CSidechainsMap& mapSidechains,
                              CSidechainEventsMap& mapSidechainEvents, CCswNullifiersMap& cswNullifies) {
    CLevelDBBatch batch;
    size_t count = 0;
    size_t changed = 0;
    ...
    if (!hashAnchor.IsNull()) BatchWriteHashBestAnchor(batch, hashAnchor);
...
CBlockTreeDB::CBlockTreeDB(size_t nCacheSize, bool fMemory, bool fWipe, bool compression, int maxOpenFiles)
    : CLevelDBWrapper(GetDataDir() / "blocks" / "index", nCacheSize, fMemory, fWipe, compression, maxOpenFiles) {}

bool CBlockTreeDB::ReadBlockFileInfo(int nFile, CBlockFileInfo& info) { return Read(make_pair(DB_BLOCK_FILES, nFile), info); }
...
bool CBlockTreeDB::ReadTxIndex(const uint256& txid, CTxIndexValue& val) { return Read(make_pair(DB_TXINDEX, txid), val); }
...
bool CBlockTreeDB::UpdateMaturityHeightIndex(const std::vector<std::pair<CMaturityHeightKey, CMaturityHeightValue>>& vect) {
    CLevelDBBatch batch;
    for (std::vector<std::pair<CMaturityHeightKey, CMaturityHeightValue>>::const_iterator it = vect.begin(); it != vect.end();
         it++)
...
bool CBlockTreeDB::UpdateAddressUnspentIndex(const std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>& vect) {
    CLevelDBBatch batch;
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>::const_iterator it = vect.begin(); it != vect.end();
         it++) {
        if (it->second.IsNull()) {
...
bool CBlockTreeDB::UpdateAddressIndex(const std::vector<std::pair<CAddressIndexKey, CAddressIndexValue>>& vect) {
    ...
    for (std::vector<std::pair<CAddressIndexKey, CAddressIndexValue>>::const_iterator it = vect.begin(); it != vect.end();
         it++) {
        if (it->second.IsNull()) {
...
bool CBlockTreeDB::ReadAddressIndex(uint160 addressHash, int type,
                                    std::vector<std::pair<CAddressIndexKey, CAddressIndexValue>>& addressIndex, int start,
                                    int end) {
    boost::scoped_ptr<leveldb::Iterator> pcursor(NewIterator());
...
bool CBlockTreeDB::ReadTimestampBlockIndex(const uint256& hash, unsigned int& ltimestamp) {
    ...
    if (!Read(std::make_pair(DB_BLOCKHASHINDEX, hash), lts)) return false;
...
bool CBlockTreeDB::ReadString(const std::string& name, std::string& sValue) {
    if (!Read(std::make_pair(DB_FLAG, name), sValue)) return false;
    return true;
}
