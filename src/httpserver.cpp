...
static bool HTTPBindAddresses(struct evhttp* http) {
    ...
    if (!mapArgs.count("-rpcallowip")) {  // Default to loopback if not allowing external IPs
        ...
        if (mapArgs.count("-rpcbind")) {
            LogPrintf(
                "WARNING: option -rpcbind was ignored because -rpcallowip was not specified, refusing to allow everyone to "
                "connect\n");
        }
...
bool InitHTTPServer() {
    ...
    evhttp_set_max_body_size(http, MAX_SERIALIZED_COMPACT_SIZE);
...
void InterruptHTTPServer() {
    ...
    if (eventHTTP) {
        // Unlisten sockets
        BOOST_FOREACH (evhttp_bound_socket* socket, boundSockets) { evhttp_del_accept_socket(eventHTTP, socket); }
...
void HTTPRequest::WriteReply(int nStatus, const std::string& strReply) {
    ...
    HTTPEvent* ev =
        new HTTPEvent(eventBase, true, boost::bind(evhttp_send_reply, req, nStatus, (const char*)NULL, (struct evbuffer*)NULL));
    ev->trigger(0);
    replySent = true;
