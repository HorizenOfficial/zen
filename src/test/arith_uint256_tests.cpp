...
void shiftArrayLeft(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int bitsToShift) {
    for (unsigned int T = 0; T < arrayLength; ++T) {
        if (T >= bitsToShift / 8) {
            unsigned int F = T - bitsToShift / 8;
            to[T] = from[F] << (bitsToShift % 8);
            if (T >= bitsToShift / 8 + 1) to[T] |= from[F - 1] >> (8 - bitsToShift % 8);
        } else {
            to[T] = 0;
        }
    }
...
BOOST_AUTO_TEST_CASE(unaryOperators)  // !    ~    -
{
    BOOST_CHECK(!ZeroL);
    BOOST_CHECK(!(!OneL));
    for (unsigned int i = 0; i < 256; ++i) BOOST_CHECK(!(!(OneL << i)));
    BOOST_CHECK(!(!R1L));
    BOOST_CHECK(!(!MaxL));
    ...
    for (unsigned int i = 0; i < 256; ++i) BOOST_CHECK(-(OneL << i) == (MaxL << i));
}
...
bool almostEqual(double d1, double d2) { return fabs(d1 - d2) <= 4 * fabs(d1) * std::numeric_limits<double>::epsilon(); }
...
BOOST_AUTO_TEST_CASE(methods)  // GetHex SetHex size() GetLow64 GetSerializeSize, Serialize, Unserialize
{
    ...
    for (int i = 256; i > 53; --i) BOOST_CHECK(almostEqual((R1L >> (256 - i)).getdouble(), ldexp(R1Ldouble, i)));
