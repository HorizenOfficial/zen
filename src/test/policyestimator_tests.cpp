...
BOOST_AUTO_TEST_CASE(BlockPolicyEstimates) {
    ...
    CScript garbage;
    for (unsigned int i = 0; i < 128; i++) garbage.push_back('X');
    ...
    while (blocknum < 200) {
        for (int j = 0; j < 10; j++) {                                 // For each fee/pri multiple
            for (int k = 0; k < 5; k++) {                              // add 4 fee txs for every priority tx
                ...
                mpool.addUnchecked(
                    hash, CTxMemPoolEntry(tx, feeV[k / 4][j], GetTime(), priV[k / 4][j], blocknum, mpool.HasNoInputsOf(tx)));
        ...
        for (int h = 0; h <= blocknum % 10; h++) {
            ...
            while (txHashes[9 - h].size()) {
                CTransaction btx;
                if (mpool.lookup(txHashes[9 - h].back(), btx)) block.push_back(btx);
    ...
    while (blocknum < 250) mpool.removeForBlock(block, ++blocknum, dummyTxs, dummyCerts);
    ...
    for (int j = 0; j < 10; j++) {
        while (txHashes[j].size()) {
            CTransaction btx;
            if (mpool.lookup(txHashes[j].back(), btx)) block.push_back(btx);
            txHashes[j].pop_back();
        }
    }
        ...
        for (int j = 0; j < 10; j++) {     // For each fee/pri multiple
            for (int k = 0; k < 5; k++) {  // add 4 fee txs for every priority tx
                ...
                mpool.addUnchecked(
                    hash, CTxMemPoolEntry(tx, feeV[k / 4][j], GetTime(), priV[k / 4][j], blocknum, mpool.HasNoInputsOf(tx)));
                CTransaction btx;
                if (mpool.lookup(hash, btx)) block.push_back(btx);
            }
        }
        mpool.removeForBlock(block, ++blocknum, dummyTxs, dummyCerts);
        block.clear();
