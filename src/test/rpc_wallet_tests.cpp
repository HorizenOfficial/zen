...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_parameters)
{
    ...
    try {
        ...
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_sendmany(mtx, "INVALID", std::move(recipients), {}, 1) );
    } catch (const UniValue& objError) {
    ...
    try {
        std::vector<SendManyRecipient> recipients = { SendManyRecipient("dummy",1.0, "") };
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_sendmany(mtx, "zcMuhvq8sEkHALuSU2i4NbNQxshSAYrpCExec45ZjtivYPbuiFPwk6WHy4SvsbeZ4siy1WheuRGjtaJmoD1J8bFqNXhsG6U", std::move(recipients), {}, 1) );
    } catch (const UniValue& objError) {
        BOOST_CHECK( find_error(objError, "Invalid from address"));
    }
    ...
    try {
        std::vector<SendManyRecipient> recipients = { SendManyRecipient("dummy",1.0, "") };
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_sendmany(mtx, "ztjiDe569DPNbyTE6TSdJTaSDhoXEHLGvYoUnBU1wfVNU52TEyT6berYtySkd21njAeEoh8fFJUT42kua9r8EnhBaEKqCpP", std::move(recipients), {}, 1) );
    } catch (const UniValue& objError) {
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_internals)
{
    ...
    // minconf cannot be zero when sending from zaddr
    {
        try {
            ...
            std::shared_ptr<AsyncRPCOperation> operation(new AsyncRPCOperation_sendmany(mtx, zaddr1, std::move(recipients), {}, 0));
...
BOOST_AUTO_TEST_CASE(rpc_z_shieldcoinbase_parameters)
{
    ...
    // Testnet payment addresses begin with 'zt'.  This test detects an incorrect prefix.
    try {
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_shieldcoinbase(mtx, { ShieldCoinbaseUTXO{uint256(), 0, 0} }, mainnetzaddr, 1) );
    } catch (const UniValue& objError) {
...
BOOST_AUTO_TEST_CASE(rpc_z_shieldcoinbase_internals)
{
    ...
    {
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_shieldcoinbase(mtx, { ShieldCoinbaseUTXO{uint256(), 0, 0}, ShieldCoinbaseUTXO{uint256(), 0, 0} }, zaddr) );
        operation->main();
        BOOST_CHECK(operation->isFailed());
        std::string msg = operation->getErrorMessage();
    ...
    // Insufficient funds
    {
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_shieldcoinbase(mtx, { ShieldCoinbaseUTXO{uint256(), 0, 0} }, zaddr) );
        operation->main();
        BOOST_CHECK(operation->isFailed());
        std::string msg = operation->getErrorMessage();
    ...
    {
        // Dummy input so the operation object can be instantiated.
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_shieldcoinbase(mtx, { ShieldCoinbaseUTXO{uint256(), 0, 100000} }, zaddr) );
