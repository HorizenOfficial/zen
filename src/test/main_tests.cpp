...
static void TestBlockSubsidyHalvings(const Consensus::Params& consensusParams) {
    ...
    BOOST_CHECK_EQUAL(
        GetBlockSubsidy((maxHalvings * consensusParams.nSubsidyHalvingInterval) + consensusParams.SubsidySlowStartShift(),
                        consensusParams),
        0);
}
...
BOOST_AUTO_TEST_CASE(block_subsidy_test) {
    ...
    TestBlockSubsidyHalvings(1, 2000);                                        // As in regtest
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test) {
    ...
    for (int nHeight = consensusParams.nSubsidySlowStartInterval;
         nHeight < consensusParams.nSubsidyHalvingInterval + consensusParams.SubsidySlowStartShift(); nHeight++) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
        BOOST_CHECK(nSubsidy <= 12.5 * COIN);
        nSum += nSubsidy;
        BOOST_CHECK(MoneyRange(nSum));
    ...
    // Regular mining
    for (int nHeight = consensusParams.nSubsidyHalvingInterval + consensusParams.SubsidySlowStartShift(); nHeight < 56000000;
         nHeight += 1000) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
        BOOST_CHECK(nSubsidy <= 12.5 * COIN);
        nSum += nSubsidy * 1000;
