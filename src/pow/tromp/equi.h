...
int verifyrec(const crypto_generichash_blake2b_state* ctx, u32* indices, uchar* hash, int r) {
    ...
    if (*indices >= *indices1) return POW_OUT_OF_ORDER;
    ...
    for (int i = 0; i < WN / 8; i++) hash[i] = hash0[i] ^ hash1[i];
    int i, b = r * DIGITBITS;
    for (i = 0; i < b / 8; i++)
        if (hash[i]) return POW_NONZERO_XOR;
    if ((b % 8) && hash[i] >> (8 - (b % 8))) return POW_NONZERO_XOR;
    return POW_OK;
}
