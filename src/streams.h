...
class CBaseDataStream {
  ...
  public:
    ...
    explicit CBaseDataStream(int nTypeIn, int nVersionIn) { Init(nTypeIn, nVersionIn); }
    ...
    CBaseDataStream& ignore(int nSize) {
        ...
        unsigned int nReadPosNext = nReadPos + nSize;
        if (nReadPosNext >= vch.size()) {
            if (nReadPosNext > vch.size()) throw std::ios_base::failure("CBaseDataStream::ignore(): end of data");
            nReadPos = 0;
            vch.clear();
            return (*this);
        }
...
class CDataStream : public CBaseDataStream<CSerializeData> {
  public:
    ...
    CDataStream(const vector_type& vchIn, int nTypeIn, int nVersionIn) : CBaseDataStream(vchIn, nTypeIn, nVersionIn) {}

    CDataStream(const std::vector<char>& vchIn, int nTypeIn, int nVersionIn) : CBaseDataStream(vchIn, nTypeIn, nVersionIn) {}
...
class CAutoFile {
  ...
  public:
    ...
    CAutoFile& read(char* pch, size_t nSize) {
        if (!file) throw std::ios_base::failure("CAutoFile::read: file handle is NULL");
    ...
    CAutoFile& ignore(size_t nSize) {
        if (!file) throw std::ios_base::failure("CAutoFile::ignore: file handle is NULL");
    ...
    CAutoFile& write(const char* pch, size_t nSize) {
        if (!file) throw std::ios_base::failure("CAutoFile::write: file handle is NULL");
        if (fwrite(pch, 1, nSize, file) != nSize) throw std::ios_base::failure("CAutoFile::write: write failed");
        return (*this);
    }
    ...
    CAutoFile& operator<<(const T& obj) {
        // Serialize to this stream
        if (!file) throw std::ios_base::failure("CAutoFile::operator<<: file handle is NULL");
        ::Serialize(*this, obj, nType, nVersion);
        return (*this);
    }
...
class CBufferedFile {
  ...
  public:
    ...
    CBufferedFile& read(char* pch, size_t nSize) {
        if (nSize + nReadPos > nReadLimit) throw std::ios_base::failure("Read attempted past buffer limit");
