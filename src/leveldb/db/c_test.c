...
int main(int argc, char** argv) {
    ...
    StartPhase("approximate_sizes");
    {
        ...
        for (i = 0; i < n; i++) {
            ...
            leveldb_put(db, woptions, keybuf, strlen(keybuf), valbuf, strlen(valbuf), &err);
            CheckNoError(err);
        }
    ...
    for (run = 0; run < 2; run++) {
        ...
        if (run == 0) {
            policy = leveldb_filterpolicy_create(NULL, FilterDestroy, FilterCreate, FilterKeyMatch, FilterName);
        } else {
