...
static bool json_isdigit(int ch) { return ((ch >= '0') && (ch <= '9')); }
...
enum jtokentype getJsonToken(std::string& tokenVal, unsigned int& consumed, const char* raw, const char* end) {
    ...
    switch (*raw) {
        ...
        case '9': {
            ...
            if ((*firstDigit == '0') && json_isdigit(firstDigit[1])) return JTOK_ERR;

            numStr += *raw;  // copy first char
            raw++;

            if ((*first == '-') && (raw < end) && (!json_isdigit(*raw))) return JTOK_ERR;
                ...
                if (raw >= end || (unsigned char)*raw < 0x20)
                    ...
                    switch (*raw) {
                        ...
                        case 'u': {
                            ...
                            if (raw + 1 + 4 >= end || hatoui(raw + 1, raw + 1 + 4, codepoint) != raw + 1 + 4) return JTOK_ERR;
...
bool UniValue::read(const char* raw, size_t size) {
    ...
    do {
        ...
        bool isValueOpen = jsonTokenIsValue(tok) || tok == JTOK_OBJ_OPEN || tok == JTOK_ARR_OPEN;
        ...
        switch (tok) {
            ...
            case JTOK_COMMA: {
                if (!stack.size() || (last_tok == JTOK_COMMA) || (last_tok == JTOK_ARR_OPEN)) goto return_fail;
