...
static DH* get_dh2048(void) {
    static unsigned char dhp_2048[] = {
        };
...
static void dumpCertificateDebugInfo(int preverify_ok, X509_STORE_CTX* chainContext) {
    if (!LogAcceptCategory("tls")) return;
    ...
    if (err != X509_V_OK) {
        LogPrintf("TLS: %s: %s():%d - Certificate Verification ERROR=%d: [%s] at chain depth=%d\n", __FILE__, __func__,
                  __LINE__, err, X509_verify_cert_error_string(err), depth);
...
/**
 * @brief If verify_callback always returns 1, the TLS/SSL handshake will not be terminated with respect to verification
...
int TLSManager::waitFor(SSLConnectionRoutine eRoutine, SOCKET hSocket, SSL* ssl, int timeoutSec, unsigned long& err_code) {
    ...
    while (true) {
        ...
        if (eRoutine == SSL_SHUTDOWN) {
            if (retOp == 0) {
                LogPrint("tls",
                         "TLS: WARNING: %s: %s():%d - SSL_SHUTDOWN: The close_notify was sent but the peer did not send it "
                         "back yet.\n",
                         __FILE__, __func__, __LINE__);
        ...
        if (sslErr != SSL_ERROR_WANT_READ && sslErr != SSL_ERROR_WANT_WRITE) {
            ...
            const char* error_str = ERR_error_string(err_code, NULL);
            LogPrint("tls",
                     "TLS: WARNING: %s: %s():%d - routine(%d), sslErr[0x%x], retOp[%d], errno[0x%x], lib[0x%x], func[0x%x], "
                     "reas[0x%x]-> err: %s\n",
                     __FILE__, __func__, __LINE__, eRoutine, sslErr, retOp, errno, ERR_GET_LIB(err_code),
                     ERR_GET_FUNC(err_code), ERR_GET_REASON(err_code), error_str);
            retOp = -1;
            break;
        }
        ...
        if (sslErr == SSL_ERROR_WANT_READ) {
            ...
            if (result == 0) {
                LogPrint("tls", "TLS: ERROR: %s: %s():%d - WANT_READ timeout on %s\n", __FILE__, __func__, __LINE__,
                         (eRoutine == SSL_CONNECT ? "SSL_CONNECT" : (eRoutine == SSL_ACCEPT ? "SSL_ACCEPT" : "SSL_SHUTDOWN")));
                err_code = SELECT_TIMEDOUT;
                retOp = -1;
                break;
            } else if (result == -1) {
                LogPrint("tls", "TLS: ERROR: %s: %s: WANT_READ ssl_err_code: 0x%x; errno: %s\n", __FILE__, __func__, sslErr,
                         strerror(errno));
                retOp = -1;
                break;
            }
...
SSL* TLSManager::connect(SOCKET hSocket, const CAddress& addrConnect, unsigned long& err_code) {
    ...
    if (bConnectedTLS) {
        LogPrintf("TLS: connection to %s has been established (tlsv = %s 0x%04x / ssl = %s 0x%x ). Using cipher: %s\n",
                  addrConnect.ToString(), SSL_get_version(ssl), SSL_version(ssl), OpenSSL_version(OPENSSL_VERSION),
                  OpenSSL_version_num(), SSL_get_cipher(ssl));
    } else {
        LogPrintf("TLS: %s: %s():%d - TLS connection to %s failed (err_code 0x%X)\n", __FILE__, __func__, __LINE__,
                  addrConnect.ToString(), err_code);
 ...
 */
SSL_CTX* TLSManager::initCtx(TLSContextType ctxType, const boost::filesystem::path& privateKeyFile,
    ...
    LogPrintf("TLS: %s: %s():%d - Initializing %s context\n", __FILE__, __func__, __LINE__,
              ctxType == SERVER_CONTEXT ? "server" : "client");

    if (!boost::filesystem::exists(privateKeyFile) || !boost::filesystem::exists(certificateFile)) return NULL;
    ...
    if ((tlsCtx = SSL_CTX_new(ctxType == SERVER_CONTEXT ? TLS_server_method() : TLS_client_method()))) {
        ...
        for (boost::filesystem::path trustedDir : trustedDirs) {
            if (SSL_CTX_load_verify_locations(tlsCtx, NULL, trustedDir.string().c_str()) == 1) trustedPathsNum++;
        }

        if (rootCertsNum == 0 && trustedPathsNum == 0)
            LogPrintf(
                "TLS: WARNING: %s: %s: failed to set up verified certificates. It will be impossible to verify peer "
                "certificates. \n",
                __FILE__, __func__);
...
bool TLSManager::prepareCredentials() {
    boost::filesystem::path defaultKeyPath(GetDataDir() / TLS_KEY_FILE_NAME),
        defaultCertPath(GetDataDir() / TLS_CERT_FILE_NAME);

    CredentialsStatus credStatus =
        VerifyCredentials(boost::filesystem::path(GetArg("-tlskeypath", defaultKeyPath.string())),
                          boost::filesystem::path(GetArg("-tlscertpath", defaultCertPath.string())), GetArg("-tlskeypwd", ""));
    ...
    if (!bPrepared) {
        if (!mapArgs.count("-tlskeypath") && !mapArgs.count("-tlscertpath")) {
            ...
            if (credStatus == credAbsent) {
                ...
                //
                bPrepared = GenerateCredentials(defaultKeyPath, defaultCertPath, GetArg("-tlskeypwd", ""));
            }
        }
    }
...
SSL* TLSManager::accept(SOCKET hSocket, const CAddress& addr, unsigned long& err_code) {
    ...
    if (bAcceptedTLS) {
        LogPrintf("TLS: connection from %s has been accepted (tlsv = %s 0x%04x / ssl = %s 0x%x ). Using cipher: %s\n",
                  addr.ToString(), SSL_get_version(ssl), SSL_version(ssl), OpenSSL_version(OPENSSL_VERSION),
                  OpenSSL_version_num(), SSL_get_cipher(ssl));
...
void TLSManager::cleanNonTLSPool(std::vector<NODE_ADDR>& vPool, CCriticalSection& cs) {
    ...
    BOOST_FOREACH (NODE_ADDR nodeAddrDeleted, vDeleted) {
        vPool.erase(remove(vPool.begin(), vPool.end(), nodeAddrDeleted), vPool.end());
    }
}
...
int TLSManager::threadSocketHandler(CNode* pnode, fd_set& fdsetRecv, fd_set& fdsetSend, fd_set& fdsetError) {
...

    {
        ...
        if (pnode->hSocket == INVALID_SOCKET) return -1;
        ...
        if (lockRecv) {
            {
...

                {
                    ...
                    if (bIsSSL) {
                        ERR_clear_error();  // clear the error queue, otherwise we may be reading an old error that occurred
                ...
                if (nBytes > 0) {
                    ...
                    //
                    if (!pnode->fDisconnect) LogPrint("tls", "socket closed (%s)\n", pnode->addr.ToString());
                    ...
                    if (bIsSSL) {
                        if (nRet != SSL_ERROR_WANT_READ &&
                            nRet != SSL_ERROR_WANT_WRITE)  // SSL_read() operation has to be repeated because of
                                                           // SSL_ERROR_WANT_READ or SSL_ERROR_WANT_WRITE
                                                           // (https://wiki.openssl.org/index.php/Manual:SSL_read(3)#NOTES)
                        {
                            if (!pnode->fDisconnect) LogPrintf("TLS: ERROR: SSL_read %s\n", ERR_error_string(nRet, NULL));
                            pnode->CloseSocketDisconnect();

                            unsigned long error = ERR_get_error();
                            const char* error_str = ERR_error_string(error, NULL);
                            LogPrint("tls", "TLS: WARNING: %s: %s():%d - SSL_read - code[0x%x], err: %s\n", __FILE__, __func__,
                                     __LINE__, nRet, error_str);
                        ...
                        if (nRet != WSAEWOULDBLOCK && nRet != WSAEMSGSIZE && nRet != WSAEINTR && nRet != WSAEINPROGRESS) {
                            if (!pnode->fDisconnect) LogPrintf("TLS: ERROR: socket recv %s\n", NetworkErrorString(nRet));
                            pnode->CloseSocketDisconnect();
                        }
                    }
                }
...
bool TLSManager::initialize() {
    ...
    OpenSSL_add_ssl_algorithms();  // OpenSSL_add_ssl_algorithms() always returns "1", so it is safe to discard the return
                                   // value.

    namespace fs = boost::filesystem;
    fs::path certFile = GetArg("-tlscertpath", "");
    if (!fs::exists(certFile)) certFile = (GetDataDir() / TLS_CERT_FILE_NAME);

    fs::path privKeyFile = GetArg("-tlskeypath", "");
    if (!fs::exists(privKeyFile)) privKeyFile = (GetDataDir() / TLS_KEY_FILE_NAME);
    ...
    for (fs::path dir : trustedDirs) LogPrintf("TLS: trusted directory '%s' will be used\n", dir.string().c_str());
