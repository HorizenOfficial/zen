...
#else  // Linux build
    "/etc/ssl/certs",     "/usr/local/ssl/certs",    "/usr/lib/ssl/certs", "/usr/share/ssl/certs",
    "/etc/pki/tls/certs", "/var/lib/ca-certificates"
#endif
};
...
// Generates certificate for a specified public key using a corresponding private key (both of them should be specified in the
// 'keypair').
//
static X509* GenerateCertificate(EVP_PKEY* keypair) {
    ...
    if (cert) {
        ...
        if (RAND_bytes((unsigned char*)&sn, sizeof sn) && ASN1_INTEGER_set(X509_get_serialNumber(cert), sn)) {
...
static EVP_PKEY* LoadKey(const boost::filesystem::path& filePath, const std::string& passphrase) {
    if (!boost::filesystem::exists(filePath)) return NULL;
...
static bool IsMatching(EVP_PKEY* key, X509* cert) {
    ...
    if (ctxSign) {
        if (EVP_PKEY_sign_init(ctxSign) == 1 && EVP_PKEY_CTX_set_signature_md(ctxSign, EVP_sha512()) > 0) {
                        ...
                        if (pubkey) {
                            ...
                            if (ctxVerif) {
                                if (EVP_PKEY_verify_init(ctxVerif) == 1 &&
                                    ...
                                    bIsMatching =
                                        (EVP_PKEY_verify(ctxVerif, signature, signatureSize, digest, digestSize) == 1);
                                }
...
bool GenerateCredentials(const boost::filesystem::path& keyPath, const boost::filesystem::path& certPath,
    ...
    if (key) {
        ...
        if (cert) {
            if (StoreKey(key, keyPath, passphrase) && StoreCertificate(cert, certPath)) {
                bGenerated = true;
...
std::vector<boost::filesystem::path> GetDefaultTrustedDirectories() {
    ...
    if (fs::exists(libDefaultDir)) defaultDirectoriesList.push_back(libDefaultDir);

    // Check and set all possible standard default directories
    for (const char* dir : defaultTrustedDirs) {
        fs::path defaultDir(dir);

        if (defaultDir != libDefaultDir && fs::exists(defaultDir)) defaultDirectoriesList.push_back(defaultDir);
    }
...
int LoadDefaultRootCertificates(SSL_CTX* ctx) {
    ...
    if (memBuf) {
        ...
        while ((cert = PEM_read_bio_X509(memBuf, NULL, 0, NULL))) {
            if (X509_STORE_add_cert(SSL_CTX_get_cert_store(ctx), cert) > 0) certsLoaded++;
