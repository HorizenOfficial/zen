...
public:
	 ...
	 */
	inline virtual int getNewBlockVersion() const { return BLOCK_VERSION_OLD; }

    /**
     * @brief returns true if the nVersion is valid at input block height
     */
    inline virtual bool isValidBlockVersion(int nVersion) const { return (nVersion >= BLOCK_VERSION_OLD); }
