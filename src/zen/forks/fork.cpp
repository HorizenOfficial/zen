 ...
 */
int Fork::getMinimumTime(CBaseChainParams::Network network) const { return minimumTimeMap.at(network); }
 ...
 */
int Fork::getHeight(CBaseChainParams::Network network) const { return heightMap.at(network); }
 ...
 */
void Fork::setCommunityFundAddressMap(
    ...
    if (communityFundAddressMap.size() != CBaseChainParams::Network::MAX_NETWORK_TYPES) {
        printf(
            "Fork attempting to set communityFundAddress map of the wrong size! communityFundAddressMap.size()=%ld "
            "MAX_NETWORK_TYPES=%d\n",
 ...
 */
const std::vector<std::string>& Fork::getCommunityFundAddresses(CBaseChainParams::Network network,
