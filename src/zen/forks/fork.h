...
class Fork {
  public:
     ...
     */
    const std::vector<std::string>& getCommunityFundAddresses(CBaseChainParams::Network network,
    ...
    virtual const std::string& getCommunityFundAddress(CBaseChainParams::Network network, int height, int maxHeight,
                                                       CommunityFundType cfType) const = 0;
  ...
  protected:
     ...
     */
    void setCommunityFundAddressMap(
        const std::map<CBaseChainParams::Network, std::vector<std::string>>& communityFundAddressMap, CommunityFundType cfType);
  ...
  private:
    // Note: there variables are not consolidated into one map with a structure because some get changed in certain forks and
    // not others and therefore they inherit the parameters from the parent fork
    std::map<CBaseChainParams::Network, int> heightMap; /**< the height at which this fork takes effect depending on network */
    std::map<CBaseChainParams::Network, int>
        minimumTimeMap; /**< the time at which this fork takes effects depending on network */
    std::map<CBaseChainParams::Network, std::vector<std::string>>
        communityFundAddressMap; /**< the community fund addresses to use depending on network */

    std::map<CBaseChainParams::Network, std::vector<std::string>>
        secureNodeFundAddressMap; /**< the securenode fund addresses to use depending on network */
    std::map<CBaseChainParams::Network, std::vector<std::string>>
        superNodeFundAddressMap; /**< the supernode fund addresses to use depending on network */
};
}  // namespace zen
#endif  // FORK_H
