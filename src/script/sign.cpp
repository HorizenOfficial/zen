...
TransactionSignatureCreator::TransactionSignatureCreator(const CKeyStore* keystoreIn, const CTransaction* txToIn, unsigned int nInIn, const CAmount& amountIn, int nHashTypeIn) : BaseSignatureCreator(keystoreIn), txTo(txToIn), nIn(nInIn), nHashType(nHashTypeIn), amount(amountIn), checker(txTo, nIn, nullptr, amountIn) {}
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    if (solved && (whichType == TX_SCRIPTHASH || whichType == TX_SCRIPTHASH_REPLAY))
    {
        ...
        solved = solved && SignStep(creator, script, result, whichType) && whichType != TX_SCRIPTHASH && whichType != TX_SCRIPTHASH_REPLAY;
    ...
    // Test solution
    return solved && VerifyScript(sigdata.scriptSig, fromPubKey, STANDARD_NONCONTEXTUAL_SCRIPT_VERIFY_FLAGS, creator.Checker());
}
