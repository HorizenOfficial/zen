...
bool TransactionSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& address,
...
CertificateSignatureCreator::CertificateSignatureCreator(const CKeyStore& keystoreIn, const CScCertificate& certToIn,
                                                         unsigned int nInIn, int nHashTypeIn)
    : BaseSignatureCreator(keystoreIn), certTo(certToIn), nIn(nInIn), nHashType(nHashTypeIn), checker(&certTo, nIn, nullptr) {}

bool CertificateSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& address,
                                            const CScript& scriptCode) const {
    CKey key;
    if (!keystore.GetKey(address, key)) return false;
...
static bool Sign1(const CKeyID& address, const BaseSignatureCreator& creator, const CScript& scriptCode,
...
static bool SignN(const vector<valtype>& multisigdata, const BaseSignatureCreator& creator, const CScript& scriptCode,
                  CScript& scriptSigRet) {
    int nSigned = 0;
    ...
    for (unsigned int i = 1; i < multisigdata.size() - 1 && nSigned < nRequired; i++) {
        ...
        if (Sign1(keyID, creator, scriptCode, scriptSigRet)) ++nSigned;
    }
 ...
 */
static bool SignStep(const BaseSignatureCreator& creator, const CScript& scriptPubKey, CScript& scriptSigRet,
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, CScript& scriptSig) {
    ...
    if (!SignStep(creator, fromPubKey, scriptSig, whichType)) return false;
    ...
    if (whichType == TX_SCRIPTHASH || whichType == TX_SCRIPTHASH_REPLAY) {
        ...
        txnouttype subType;
        bool fSolved =
            SignStep(creator, subscript, scriptSig, subType) && subType != TX_SCRIPTHASH && subType != TX_SCRIPTHASH_REPLAY;
...
bool SignSignature(const CKeyStore& keystore, const CScCertificate& certFrom, CMutableTransaction& txTo, unsigned int nIn,
                   int nHashType) {
...
bool SignSignature(const CKeyStore& keystore, const CScript& fromPubKey, CMutableScCertificate& certTo, unsigned int nIn,
                   int nHashType) {
...
bool SignSignature(const CKeyStore& keystore, const CScCertificate& certFrom, CMutableScCertificate& certTo, unsigned int nIn,
                   int nHashType) {
    assert(nIn < certTo.vin.size());
    CTxIn& txin = certTo.vin[nIn];
    assert(txin.prevout.n < certFrom.GetVout().size());
...
static CScript CombineMultisig(const CScript& scriptPubKey, const BaseSignatureChecker& checker,
    ...
    for (unsigned int i = nSigsHave; i < nSigsRequired; i++) result << OP_0;

    return result;
}

static CScript CombineSignatures(const CScript& scriptPubKey, const BaseSignatureChecker& checker, const txnouttype txType,
                                 const vector<valtype>& vSolutions, vector<valtype>& sigs1, vector<valtype>& sigs2) {
    switch (txType) {
        case TX_NONSTANDARD:
        case TX_NULL_DATA:
        case TX_NULL_DATA_REPLAY:
        ...
        case TX_PUBKEYHASH_REPLAY:
            ...
            if (sigs1.empty() || sigs1[0].empty()) return PushAll(sigs2);
...
CScript CombineSignatures(const CScript& scriptPubKey, const CTransaction& txTo, unsigned int nIn, const CScript& scriptSig1,
...
CScript CombineSignatures(const CScript& scriptPubKey, const BaseSignatureChecker& checker, const CScript& scriptSig1,
                          const CScript& scriptSig2) {
    txnouttype txType;
...
bool DummySignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& keyid,
                                      const CScript& scriptCode) const {
