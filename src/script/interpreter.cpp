...
bool EvalScript(vector<vector<unsigned char> >& stack, const CScript& script, unsigned int flags,
    ...
    if (script.size() > MAX_SCRIPT_SIZE) return set_error(serror, SCRIPT_ERR_SCRIPT_SIZE);
    int nOpCount = 0;
    ...
    try {
        while (pc < pend) {
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
                ...
                switch (opcode) {
                    ...
                    case OP_CHECKLOCKTIMEVERIFY: {
                        ...
                        if (!checker.CheckLockTime(nLockTime)) return set_error(serror, SCRIPT_ERR_UNSATISFIED_LOCKTIME);
                    ...
                    case OP_NOTIF: {
                        ...
                        bool fValue = false;
                        if (fExec) {
                            if (stack.size() < 1) return set_error(serror, SCRIPT_ERR_UNBALANCED_CONDITIONAL);
                    ...
                    case OP_CHECKMULTISIGVERIFY: {
                        ...
                        int i = 1;
                        if ((int)stack.size() < i) return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);

                        int nKeysCount = CScriptNum(stacktop(-i), fRequireMinimal).getint();
                        if (nKeysCount < 0 || nKeysCount > 20) return set_error(serror, SCRIPT_ERR_PUBKEY_COUNT);
                        nOpCount += nKeysCount;
                        if (nOpCount > 201) return set_error(serror, SCRIPT_ERR_OP_COUNT);
                        int ikey = ++i;
                        i += nKeysCount;
                        if ((int)stack.size() < i) return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);

                        int nSigsCount = CScriptNum(stacktop(-i), fRequireMinimal).getint();
                        if (nSigsCount < 0 || nSigsCount > nKeysCount) return set_error(serror, SCRIPT_ERR_SIG_COUNT);
                        int isig = ++i;
                        i += nSigsCount;
                        if ((int)stack.size() < i) return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
            ...
            // Size limits
            if (stack.size() + altstack.size() > 1000) return set_error(serror, SCRIPT_ERR_STACK_SIZE);
        }
    } catch (...) {
...
class CTransactionSignatureSerializer {
  ...
  public:
    CTransactionSignatureSerializer(const CTransactionBase& txToIn, const CScript& scriptCodeIn, unsigned int nInIn,
                                    int nHashTypeIn)
        : txBaseTo(txToIn),
    ...
    void SerializeCswInput(S& s, unsigned int nCswInput, const CTransaction& txTo, int nType, int nVersion) const {
        ...
        if (fAnyoneCanPay) nCswInput = nIn - txTo.GetVin().size();
    ...
    void Serialize(S& s, int nType, int nVersion) const {
        ...
        if (!txBaseTo.IsCertificate()) {
            ...
            unsigned int nInputs =
                fAnyoneCanPay ? (txTo.IsScVersion() && nIn >= txTo.GetVin().size() ? 0 : 1) : txTo.GetVin().size();
            ::WriteCompactSize(s, nInputs);
            for (unsigned int nInput = 0; nInput < nInputs; nInput++) SerializeInput(s, nInput, nType, nVersion);
            // Serialize vout
            unsigned int nOutputs = fHashNone ? 0 : (fHashSingle ? nIn + 1 : txTo.GetVout().size());
            ::WriteCompactSize(s, nOutputs);
            for (unsigned int nOutput = 0; nOutput < nOutputs; nOutput++) SerializeOutput(s, nOutput, nType, nVersion);

            if (txTo.IsScVersion()) {
                // Serialize CSW inputs
                // In case of SIGHASH_ANYONECANPAY:
                // * if Tx has Sc support version and nIn belongs to the CSW inputs - only the CSW input being signed is
                // serialized
                // * otherwise skip CSW inputs
                // Otherwise - serialize all CSW inputs
                unsigned int nCswInputs = fAnyoneCanPay ? (nIn < txTo.GetVin().size() ? 0 : 1) : txTo.GetVcswCcIn().size();
...
TransactionSignatureChecker::TransactionSignatureChecker(const CTransaction* txToIn, unsigned int nInIn, const CChain* chainIn)
    : txTo(txToIn), nIn(nInIn), chain(chainIn) {}
...
bool TransactionSignatureChecker::CheckLockTime(const CScriptNum& nLockTime) const {
    ...
    if (!((txTo->GetLockTime() < LOCKTIME_THRESHOLD && nLockTime < LOCKTIME_THRESHOLD) ||
          (txTo->GetLockTime() >= LOCKTIME_THRESHOLD && nLockTime >= LOCKTIME_THRESHOLD)))
        return false;

    // Now that we know we're comparing apples-to-apples, the
    // comparison is a simple numeric one.
    if (nLockTime > (int64_t)txTo->GetLockTime()) return false;
    ...
    if (nIn < txTo->GetVin().size()) {
        if (txTo->GetVin()[nIn].IsFinal()) return false;
    }
...
bool CheckReplayProtectionData(const CChain* chain, int nHeight, const std::vector<unsigned char>& vchCompareTo) {
    ...
    if (nHeight <= chain->Height() - getCheckBlockAtHeightSafeDepth()) {
        LogPrint("cbh", "%s: %s():%d - Old block: dont even check [h=%d, chain h=%d, safe depth = %d]\n", __FILE__, __func__,
                 __LINE__, nHeight, chain->Height(), getCheckBlockAtHeightSafeDepth());
        return true;
    }
#else
...
CertificateSignatureChecker::CertificateSignatureChecker(const CScCertificate* certToIn, unsigned int nInIn,
                                                         const CChain* chainIn)
    : certTo(certToIn), nIn(nInIn), chain(chainIn) {}

bool CertificateSignatureChecker::VerifySignature(const std::vector<unsigned char>& vchSig, const CPubKey& pubkey,
                                                  const uint256& sighash) const {
    return pubkey.Verify(sighash, vchSig);
}

bool CertificateSignatureChecker::CheckSig(const vector<unsigned char>& vchSigIn, const vector<unsigned char>& vchPubKey,
                                           const CScript& scriptCode) const {
    CPubKey pubkey(vchPubKey);
    if (!pubkey.IsValid()) return false;
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags,
    ...
    if (CastToBool(stack.back()) == false) return set_error(serror, SCRIPT_ERR_EVAL_FALSE);
