...
const char* GetTxnOutputType(txnouttype t) {
    switch (t) {
        ...
        case TX_PUBKEYHASH_REPLAY:
            return "pubkeyhashreplay";
        case TX_MULTISIG_REPLAY:
            return "multisigreplay";
        case TX_NULL_DATA_REPLAY:
            return "nulldatareplay";
    }
    return NULL;
}

static std::multimap<txnouttype, CScript> init_templates() {
    multimap<txnouttype, CScript> ret{
        // Standard tx, sender provides pubkey, receiver adds signature
        {TX_PUBKEY, CScript() << OP_PUBKEY << OP_CHECKSIG},
        {TX_PUBKEY_REPLAY, CScript() << OP_PUBKEY << OP_CHECKSIG << OP_SMALLDATA << OP_SMALLDATA << OP_CHECKBLOCKATHEIGHT},
        // Bitcoin address tx, sender provides hash of pubkey, receiver provides signature and pubkey
        {TX_PUBKEYHASH, CScript() << OP_DUP << OP_HASH160 << OP_PUBKEYHASH << OP_EQUALVERIFY << OP_CHECKSIG},
        {TX_PUBKEYHASH_REPLAY, CScript() << OP_DUP << OP_HASH160 << OP_PUBKEYHASH << OP_EQUALVERIFY << OP_CHECKSIG
                                         << OP_SMALLDATA << OP_SMALLDATA << OP_CHECKBLOCKATHEIGHT},
        // Sender provides N pubkeys, receivers provides M signatures
        {TX_MULTISIG, CScript() << OP_SMALLINTEGER << OP_PUBKEYS << OP_SMALLINTEGER << OP_CHECKMULTISIG},
        {TX_MULTISIG_REPLAY, CScript() << OP_SMALLINTEGER << OP_PUBKEYS << OP_SMALLINTEGER << OP_CHECKMULTISIG << OP_SMALLDATA
                                       << OP_SMALLDATA << OP_CHECKBLOCKATHEIGHT},
        // P2SH, sender provides script hash
        {TX_SCRIPTHASH, CScript() << OP_HASH160 << OP_PUBKEYHASH << OP_EQUAL},
        {TX_SCRIPTHASH_REPLAY,
         CScript() << OP_HASH160 << OP_PUBKEYHASH << OP_EQUAL << OP_SMALLDATA << OP_SMALLDATA << OP_CHECKBLOCKATHEIGHT},

        {TX_NULL_DATA, CScript() << OP_RETURN},
        {TX_NULL_DATA_REPLAY, CScript() << OP_RETURN << OP_SMALLDATA << OP_SMALLDATA << OP_CHECKBLOCKATHEIGHT}};

    // Empty, provably prunable, data-carrying output
    if (GetBoolArg("-datacarrier", true)) {
        ret.insert(make_pair(TX_NULL_DATA, CScript() << OP_RETURN << OP_SMALLDATA));
        ret.insert(make_pair(TX_NULL_DATA_REPLAY,
                             CScript() << OP_RETURN << OP_SMALLDATA << OP_SMALLDATA << OP_SMALLDATA << OP_CHECKBLOCKATHEIGHT));
    }
    return ret;
}
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char>>& vSolutionsRet,
    ...
    // Templates
    static const multimap<txnouttype, CScript> mTemplates{init_templates()};
        ...
        while (true) {
            ...
            if (opcode2 == OP_PUBKEY) {
                ...
                if (rpLevel < RPLEVEL_FIXED_2) {
                    ...
                    if (vch1.size() == 0) {
                        if ((opcode1 >= OP_1 && opcode1 <= OP_16) || opcode1 == OP_1NEGATE) {
                            ...
                            LogPrintf(
                                "%s: %s:%d - OP_CHECKBLOCKATHEIGHT verification failed. Bad height param (opcode=0x%X not "
                                "legal in setting height).\n",
                ...
                if (rpLevel < RPLEVEL_FIXED_2) {
                    ...
                    // According to BIP115, sufficiently old blocks are always valid, so reject only blocks of depth less than
                    // 52596. Skip check if referenced block is further than chainActive. It means that we are not fully
                    // synchronized.
                    ...
                    if ((vchBlockHeight.size() > sizeof(int)) || (vchBlockHash.size() != 32)) {
                        LogPrintf(
                            "%s: %s():%d - OP_CHECKBLOCKATHEIGHT verification failed. Bad params: vh size = %d, vhash size = "
                            "%d\n",
                    ...
                    try {
                        ...
                        LogPrintf("%s: %s():%d - OP_CHECKBLOCKATHEIGHT nHeight 0x%s not minimally encoded (err=%s)\n", __FILE__,
                                  __func__, __LINE__, HexStr(vchBlockHeight.begin(), vchBlockHeight.end()), e.what());
                        break;
                    } catch (...) {
                    ...
                    if (!CheckMinimalPush(vchBlockHeight, hopcode)) {
                        LogPrintf("%s: %s():%d - OP_CHECKBLOCKATHEIGHT value 0x%s not minimally pushed\n", __FILE__, __func__,
                                  __LINE__, HexStr(vchBlockHeight.begin(), vchBlockHeight.end()));
                        break;
                    }
                    ...
                    if (!CheckReplayProtectionData(&chainActive, nHeight, vchBlockHash)) {
                        LogPrintf(
                            "%s: %s():%d OP_CHECKBLOCKATHEIGHT verification failed. Referenced height %d invalid or not "
                            "corresponding to hash %s\n",
    ...
    if (!rpAttributes.foundOpCode) {
        if (!solverResult) {
            LogPrint("cbh", "%s: %s():%d solver failed but no rp attributes found for script %s\n", __FILE__, __func__,
                     __LINE__, scriptPubKey.ToString());
        }
...
bool IsStandard(const CScript& scriptPubKey, txnouttype& whichType, ReplayProtectionAttributes& rpAttributes) {
    vector<valtype> vSolutions;
    if (!Solver(scriptPubKey, whichType, vSolutions, rpAttributes)) return false;
...
class CScriptVisitor : public boost::static_visitor<bool> {
  ...
  public:
    ...
    bool operator()(const CKeyID& keyID) const {
        ...
        *script << OP_DUP << OP_HASH160 << ToByteVector(keyID) << OP_EQUALVERIFY << OP_CHECKSIG
                << ToByteVector(chainActive[blockIndex]->GetBlockHash()) << chainActive[blockIndex]->nHeight
    ...
    bool operator()(const CScriptID& scriptID) const {
        ...
        int blockIndex = currentBlock->nHeight - CBH_DELTA_HEIGHT;
        if (blockIndex < 0) blockIndex = 0;
        *script << OP_HASH160 << ToByteVector(scriptID) << OP_EQUAL << ToByteVector(chainActive[blockIndex]->GetBlockHash())
                << chainActive[blockIndex]->nHeight << OP_CHECKBLOCKATHEIGHT;
        return true;
    }
#endif  // BITCOIN_TX
};
...
ReplayProtectionAttributes::ReplayProtectionAttributes() : referencedHeight(UNDEF), referencedHash(), foundOpCode(false) {}
...
bool ReplayProtectionAttributes::GotValues() const { return (referencedHeight != UNDEF && !referencedHash.empty()); }
