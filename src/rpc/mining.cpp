...
int64_t GetNetworkHashPS(int lookup, int height) {
    ...
    if (lookup <= 0) lookup = Params().GetConsensus().nPowAveragingWindow;
...
UniValue getnetworksolps(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. blocks     (numeric, optional, default=120) the number of blocks, or -1 for blocks over difficulty averaging "
            "window\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("getnetworksolps", "") + HelpExampleRpc("getnetworksolps", ""));
...
UniValue generate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nGenerate 11 blocks\n" +
            HelpExampleCli("generate", "11") + HelpExampleRpc("generate", "11"));
    ...
    if (!Params().MineBlocksOnDemand()) throw JSONRPCError(RPC_METHOD_NOT_FOUND, "This method can only be used on regtest");
...
#endif
        if (!pblocktemplate.get()) throw JSONRPCError(RPC_INTERNAL_ERROR, "Wallet keypool empty");
...
UniValue setgenerate(const UniValue& params, bool fHelp) {
    ...
    bool fGenerate = true;
    if (params.size() > 0) fGenerate = params[0].get_bool();

    int nGenProcLimit = -1;
    if (params.size() > 1) {
        nGenProcLimit = params[1].get_int();
        if (nGenProcLimit == 0) fGenerate = false;
    }
...
UniValue getmininginfo(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"localsolps\": xxxxxxxx,         (numeric) the average local solution rate in Sol/s since this node was "
            "started\n"
...
static UniValue BIP22ValidationResult(const CValidationState& state) {
    ...
    if (state.IsError()) throw JSONRPCError(RPC_VERIFY_ERROR, strRejectReason);
    if (state.IsInvalid()) {
...
UniValue getblocktemplate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"jsonrequestobject\"                   (string, optional) a json object (it can also be empty) in the "
            "following spec\n"
            "     {\n"
            ...
            "     }\n"
            "\n"
            "2. \"includeMerkleRoots\"                  (boolean, optional, default=false) If true, include \"merkleTree\" and "
            "\"scTxsCommitment\" fields in the result object. \n"

            "\nResult:\n"
            "{\n"
            "  \"version\" : n,                         (numeric) the block version\n"
            "  \"merkleTree\" : \"xxxx\",               (string) Merkleroot calculated on transactions and certificates. "
            "(Note: If you construct coinbasetxn outside of zend please use \"getblockmerkleroots\" RPC to calculate "
            "\"merkleTree\" and \"scTxsCommitment\".)\n"
            "  \"scTxsCommitment\" : \"xxxx\",          (string) scTxsCommitment calculated on certificates.\n"
            "  \"previousblockhash\" : \"xxxx\",        (string) the hash of current highest block\n"
            "  \"transactions\" : [                     (array) contents of non-coinbase transactions that should be included "
            "in the next block\n"
            "      {\n"
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                       (numeric) difference in value between transaction inputs and outputs "
            "(in Satoshis); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not "
            "including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "         \"required\" : true|false         (boolean) if provided and true, this transaction must be in the final "
            "block\n"
            "      }\n"
            "      ,...\n"
            "  ],\n"
    ...
    if (params.size() > 0) {
        ...
        if (strMode == "proposal") {
            ...
            if (!dataval.isStr()) throw JSONRPCError(RPC_TYPE_ERROR, "Missing data String key for proposal");

            CBlock block;
            if (!DecodeHexBlk(block, dataval.get_str())) throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block decode failed");
            ...
            if (block.hashPrevBlock != pindexPrev->GetBlockHash()) return "inconclusive-not-best-prevblk";
            CValidationState state;
    ...
    if (vNodes.empty()) throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Horizen is not connected!");

    if (IsInitialBlockDownload()) throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Horizen is downloading blocks...");
    ...
    for (const CTransaction& tx : pblock->vtx) {
        ...
        if (tx.IsCoinBase() && !coinbasetxn) continue;
    ...
    if (pblock->nVersion != BLOCK_VERSION_SC_SUPPORT) block_size_limit = MAX_BLOCK_SIZE_BEFORE_SC;
...
UniValue submitblock(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "For more information on submitblock parameters and results, see: "
    ...
    CBlock block;
    if (!DecodeHexBlk(block, params[0].get_str())) throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block decode failed");
...
UniValue estimatefee(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExample:\n" +
            HelpExampleCli("estimatefee", "6") + HelpExampleRpc("estimatefee", "6"));
...
UniValue estimatepriority(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nExample:\n" +
            HelpExampleCli("estimatepriority", "6") + HelpExampleRpc("estimatepriority", "6"));
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns block subsidy reward, taking into account the mining slow start and the community fund, of block at "
            "index provided.\n"
            ...
            "\nResult:\n"
            "{\n"
            "  \"miner\": xxxx,           (numeric) the mining reward amount in " +
            CURRENCY_UNIT +
            "\n"
            "  \"community\": xxxx,       (numeric) the community fund amount in " +
            CURRENCY_UNIT +
            "\n"
            "  \"securenodes\": xxxx,     (numeric) the securenodes fund amount in " +
            CURRENCY_UNIT +
            "\n"
            "  \"supernodes\": xxxx       (numeric) the supernodes fund amount in " +
            CURRENCY_UNIT +
            "\n"
            "}\n"
    ...
    if (nHeight < 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Block height out of range");

    CAmount nReward = GetBlockSubsidy(nHeight, Params().GetConsensus());
    CAmount minerReward = nReward;

    CAmount nCommunityFund =
        ForkManager::getInstance().getCommunityFundReward(nHeight, nReward, Fork::CommunityFundType::FOUNDATION);
    minerReward -= nCommunityFund;

    CAmount secureNodeFund =
        ForkManager::getInstance().getCommunityFundReward(nHeight, nReward, Fork::CommunityFundType::SECURENODE);
    if (secureNodeFund > 0) {
        minerReward -= secureNodeFund;
    }
    CAmount superNodeFund =
        ForkManager::getInstance().getCommunityFundReward(nHeight, nReward, Fork::CommunityFundType::SUPERNODE);
    if (superNodeFund > 0) {
...
UniValue getblockmerkleroots(const UniValue& params, bool fHelp) {
    ...
    for (const UniValue& tx : txsStr.getValues()) {
        ...
        if (!DecodeHexTx(transaction, tx.get_str())) throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed");
