...
UniValue getpeerinfo(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"tls_verified\": true|false,           (boolean) status of peer certificate. True if the chain of trust of a "
            "peer certificate can be verified using the OS certificate store\n"
    ...
    for (const CNodeStats& stats : vstats) {
        ...
        if (!(stats.addrLocal.empty())) obj.pushKV("addrlocal", stats.addrLocal);
        ...
        if (stats.dPingWait > 0.0) obj.pushKV("pingwait", stats.dPingWait);
...
UniValue addnode(const UniValue& params, bool fHelp) {
    string strCommand;
    if (params.size() == 2) strCommand = params[1].get_str();
    ...
    if (strCommand == "add") {
        if (it != vAddedNodes.end()) throw JSONRPCError(RPC_CLIENT_NODE_ALREADY_ADDED, "Error: Node already added");
        vAddedNodes.push_back(strNode);
    } else if (strCommand == "remove") {
        if (it == vAddedNodes.end()) throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: Node has not been added.");
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. dns                                          (boolean, required) if false, only a list of added nodes will be "
            "provided, otherwise connected information will also be available\n"
            "2. \"node\"                                     (string, optional) if provided, return information about this "
    ...
    if (params.size() == 1) {
        ...
        if (laddedNodes.size() == 0) throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: Node has not been added.");
    }
    ...
    for (list<pair<string, vector<CService> > >::iterator it = laddedAddreses.begin(); it != laddedAddreses.end(); it++) {
        ...
        for (const CService& addrNode : it->second) {
            ...
            if (!fFound) node.pushKV("connected", "false");
...
UniValue getnetworkinfo(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"localservices\": \"xxxxxxx\",                (string) the services supported by the node, as advertised in "
            "its version message\n"
            ...
            "  \"tls_cert_verified\": true|flase,             (boolean) true if the certificate of the current node is "
            "verified\n"
            "  \"networks\": [                                (array) an array of objects describing IPV4, IPV6 and Onion "
            "network status\n"
            "  {\n"
            ...
            "  ,...\n"
            "  ],\n"
            "  \"relayfee\": xxxxxx,                          (numeric) minimum relay fee for non-free transactions in " +
            CURRENCY_UNIT +
            "/kB\n"
            "  \"localaddresses\": [                          (array) an array of objects describing local addresses being "
            "listened on by the node\n"
            "   {\n"
    ...
    obj.pushKV("subversion", FormatSubVersion(CLIENT_NAME, CLIENT_VERSION, std::vector<string>()));
...
UniValue setban(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || (strCommand != "add" && strCommand != "remove"))
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"ip(/netmask)\" (string, required) the IP/Subnet (see getpeerinfo for nodes ip) with an optional netmask "
            "(default is /32 = single ip)\n"
            "2. \"command\"      (string, required) 'add' to add an IP/Subnet to the list, 'remove' to remove an IP/Subnet "
            "from the list\n"
            "3. \"bantime\"      (numeric, optional) time in seconds how long (or until when if [absolute] is set) the ip is "
            "banned (0 or empty means using the default time of 24h which can also be overwritten by the -bantime startup "
            "argument)\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("setban", "\"192.168.0.6\" \"add\" 86400") + HelpExampleCli("setban", "\"192.168.0.0/24\" \"add\"") +
    ...
    if (params[0].get_str().find("/") != string::npos) isSubnet = true;
    ...
    if (strCommand == "add") {
        ...
        if (params.size() >= 3 && !params[2].isNull()) banTime = params[2].get_int64();

        bool absolute = false;
        if (params.size() == 4 && params[3].isTrue()) absolute = true;

        isSubnet ? CNode::Ban(subNet, banTime, absolute) : CNode::Ban(netAddr, banTime, absolute);

        // disconnect possible nodes
        while (CNode* bannedNode = (isSubnet ? FindNode(subNet) : FindNode(netAddr))) bannedNode->fDisconnect = true;
    } else if (strCommand == "remove") {
