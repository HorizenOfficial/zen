...
static bool AppInitRawTx(int argc, char* argv[]) {
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help")) {
        ...
        strUsage += HelpMessageOpt(
            "-regtest", _("Enter regression test mode, which uses a special chain in which blocks can be solved instantly."));
        ...
        strUsage +=
            HelpMessageOpt("sign=SIGHASH-FLAGS",
...
static void MutateTxVersion(CMutableTransaction& tx, const string& cmdVal) {
    ...
    if ((newVersion < CTransaction::MIN_OLD_VERSION &&
         newVersion != GROTH_TX_VERSION))  // || newVersion > CTransaction::MAX_CURRENT_VERSION)
...
static void MutateTxAddInput(CMutableTransaction& tx, const string& strInput) {
    ...
    if ((vout < 0) || (vout > (int)maxVout)) throw runtime_error("invalid TX input vout");
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const string& strInput) {
    ...
    CAmount value;
    if (!ParseMoney(strValue, value)) throw runtime_error("invalid TX output value");

    // extract and validate ADDRESS
    string strAddr = strInput.substr(pos + 1, string::npos);
    CBitcoinAddress addr(strAddr);
    if (!addr.IsValid()) throw runtime_error("invalid TX output address");
...
static void MutateTxAddOutScript(CMutableTransaction& tx, const string& strInput) {
    ...
    size_t pos = strInput.find(':');
    if ((pos == string::npos) || (pos == 0)) throw runtime_error("TX output missing separator");
...
uint256 ParseHashUO(map<string, UniValue>& o, string strKey) {
    if (!o.count(strKey)) return uint256();
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr) {
    ...
    if (flagStr.size() > 0)
        if (!findSighashFlags(nHashType, flagStr)) throw runtime_error("unknown sighash flag/sign option");
    ...
    if (!registers.count("privatekeys")) throw runtime_error("privatekeys register variable must be set.");
    ...
    //             csw input script.
    //             * Later the signature will be checked, so in case no origin signature and no new one exist -> verification
    //             will fail.
    //             */
...
static int CommandLineRawTx(int argc, char* argv[]) {
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            if (!DecodeHexTx(txDecodeTmp, strHexTx)) throw runtime_error("invalid transaction encoding");
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (!AppInitRawTx(argc, argv)) return EXIT_FAILURE;
    } catch (const std::exception& e) {
