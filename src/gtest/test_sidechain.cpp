...
TEST_F(SidechainTestSuite, RevertCertOutputsRestoresLastCertHash) {
    ...
    ASSERT_TRUE(sidechainsView->HandleSidechainEvents(scCreationHeight + Params().ScCoinsMaturity(), dummyBlockUndo, &voidedCerts));
...
TEST_F(SidechainTestSuite, CertificateUpdatesLastCertificateHash) {
    ...
    CBlockUndo dummyBlockUndo;
    std::vector<uint256> voidedCerts;
    ASSERT_TRUE(sidechainsView->HandleSidechainEvents(coinMaturityHeight, dummyBlockUndo, voidedCerts));
