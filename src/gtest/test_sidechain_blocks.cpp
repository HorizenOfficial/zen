...
class CInMemorySidechainDb final : public CCoinsView {
  public:
    ...
    bool GetSidechain(const uint256& scId, CSidechain& info) const override {
        if (!HaveSidechain(scId)) return false;
...
class SidechainsConnectCertsBlockTestSuite : public ::testing::Test {
  public:
    SidechainsConnectCertsBlockTestSuite()
        ...
        dummyScriptPubKey = GetScriptForDestination(CKeyID(uint160(ParseHex("816115944e077fe7c803cfa57f29b36bf87c1d35"))),
    ...
    void SetUp() override {
        ...
        dummyCoinbaseScript = CScript() << OP_DUP << OP_HASH160 << ToByteVector(uint160()) << OP_EQUALVERIFY << OP_CHECKSIG;
    };
...
TEST_F(SidechainsConnectCertsBlockTestSuite, ConnectBlock_SingleCert_SameEpoch_CertCoinHasBwt) {
    ...
    bool res = ConnectBlock(certBlock, dummyState, certBlockIndex, *sidechainsView, dummyChain,
                            flagBlockProcessingType::CHECK_ONLY, flagScRelatedChecks::OFF, flagScProofVerification::ON,
                            flagLevelDBIndexesWrite::OFF, &dummyCertStatusUpdateInfo);
...
TEST_F(SidechainsConnectCertsBlockTestSuite, ConnectBlock_MultipleCerts_SameEpoch_LowQualityCertCoinHasNotBwt) {
    ...
    // create coinbase to finance certificate submission (just in view)
    uint256 inputLowQCertHash =
        txCreationUtils::CreateSpendableCoinAtHeight(*sidechainsView, certBlockHeight - COINBASE_MATURITY);
    uint256 inputHighQCertHash =
        txCreationUtils::CreateSpendableCoinAtHeight(*sidechainsView, certBlockHeight - COINBASE_MATURITY - 1);
...
TEST_F(SidechainsConnectCertsBlockTestSuite, ConnectBlock_ScCreation_then_Mbtr_InSameBlock) {
    ...
    uint256 inputScCreationHash =
        txCreationUtils::CreateSpendableCoinAtHeight(*sidechainsView, certBlockHeight - COINBASE_MATURITY);
    uint256 inputMbtrHash =
        txCreationUtils::CreateSpendableCoinAtHeight(*sidechainsView, certBlockHeight - COINBASE_MATURITY - 1);
    ...
    // test
    bool res = ConnectBlock(block, dummyState, blockIndex, *sidechainsView, dummyChain, flagBlockProcessingType::CHECK_ONLY,
                            flagScRelatedChecks::OFF, flagScProofVerification::ON, flagLevelDBIndexesWrite::OFF,
                            &dummyCertStatusUpdateInfo);
...
void SidechainsConnectCertsBlockTestSuite::storeSidechain(const uint256& scId, const CSidechain& sidechain) {
    ...
    uint256 dummyAnchor =
        uint256S("59d2cde5e65c1414c32ba54f0fe4bdb3d67618125286e6a191317917c812c6d7");  // anchor for empty block!?
    ...
    sidechainsView->BatchWrite(dummyCoins, dummyHash, dummyAnchor, dummyAnchors, dummyNullifiers, dummySidechains,
                               dummySidechainsEvents, dummyCswNullifiers);
  ...
  public:
    SidechainsBlockFormationTestSuite()
        : fakeChainStateDb(nullptr),
          ...
          mapDependers(),
          dummyHeight(1987),
          dummyLockTimeCutoff(0),
          dummyAmount(10),
          dummyScript(),
          dummyOut(dummyAmount, dummyScript) {}
...
TEST_F(SidechainsBlockFormationTestSuite, SingleTxes_MempoolOrdering) {
    ...
    CTxMemPoolEntry tx_highPriority_entry(tx_highPriority, /*fee*/ CAmount(1), /*time*/ 1000, /*priority*/ 100.0,
                                          /*height*/ dummyHeight);
...
TEST_F(SidechainsBlockFormationTestSuite, DifferentScIdCerts_FeesAndPriorityOnlyContributeToMempoolOrdering) {
    ...
    CCertificateMemPoolEntry cert_highFee_entry(cert_highFee, /*fee*/ CAmount(100), /*time*/ 1000, /*priority*/ 1.0,
                                                /*height*/ dummyHeight);
    ...
    CCertificateMemPoolEntry cert_highPriority_entry(cert_highPriority, /*fee*/ CAmount(1), /*time*/ 1000, /*priority*/ 100.0,
                                                     /*height*/ dummyHeight);
...
TEST_F(SidechainsBlockFormationTestSuite, SameScIdCerts_HighwQualityCertsSpedingLowQualityOnesAreAccepted) {
    ...
    CCertificateMemPoolEntry cert_lowQuality_entry(cert_lowQuality, /*fee*/ CAmount(1), /*time*/ 1000, /*priority*/ 1.0,
                                                   /*height*/ dummyHeight);
    ...
    CCertificateMemPoolEntry cert_highQuality_entry(cert_highQuality, /*fee*/ CAmount(1), /*time*/ 1000, /*priority*/ 1.0,
                                                    /*height*/ dummyHeight);
