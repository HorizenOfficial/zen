...
CTxCeasedSidechainWithdrawalInput txCreationUtils::CreateCSWInput(const uint256& scId, const std::string& nullifierHex,
                                                                  const std::string& actCertDataHex,
                                                                  const std::string& ceasingCumScTxCommTreeHex,
                                                                  CAmount amount) {
...
CScCertificate txCreationUtils::createCertificate(const uint256& scId, int epochNum,
                                                  const CFieldElement& endEpochCumScTxCommTreeRoot, CAmount changeTotalAmount,
                                                  unsigned int numChangeOut, CAmount bwtTotalAmount, unsigned int numBwt,
...
uint256 txCreationUtils::CreateSpendableCoinAtHeight(CCoinsViewCache& targetView, unsigned int coinHeight) {
    ...
    CScript dummyCoinbaseScript = CScript() << OP_DUP << OP_HASH160 << ToByteVector(uint160()) << OP_EQUALVERIFY << OP_CHECKSIG;
...
void txCreationUtils::storeSidechainEvent(CSidechainEventsMap& mapToWriteInto, int eventHeight,
                                          const CSidechainEvents& scEvent) {
...
void chainSettingUtils::ExtendChainActiveWithBlock(const CBlock& block) {
    ...
    pNewBlockIdx->nBits = 0x1e7fffff;
    pNewBlockIdx->nChainWork = (chainActive.Height() + 1) == 0 ? arith_uint256(0)
                                                               : mapBlockIndex.at(prevBlockHash)->nChainWork +
                                                                     GetBlockProof(*(mapBlockIndex.at(prevBlockHash)));
    pNewBlockIdx->hashAnchor = dummyTree.root();
 ...
 */
std::shared_ptr<CInMemorySidechainDb> BlockchainTestManager::CoinsView() const { return view; }
 ...
 */
std::shared_ptr<CNakedCCoinsViewCache> BlockchainTestManager::CoinsViewCache() const { return viewCache; }
 ...
 */
std::string BlockchainTestManager::TempFolderPath() const { return tempFolderPath.string(); }
...
CTxCeasedSidechainWithdrawalInput BlockchainTestManager::CreateCswInput(uint256 scId, CAmount nValue,
                                                                        ProvingSystem provingSystem) const {
    ...
    CCswProofVerifierInput verifierInput =
        CScProofVerifier::CswInputToVerifierItem(input, nullptr, sidechain.fixedParams, nullptr);
...
MempoolReturnValue BlockchainTestManager::TestAcceptTxToMemoryPool(CValidationState& state, const CTransaction& tx) const {
    ...
    LOCK(cs_main);
    MempoolReturnValue val =
        AcceptTxToMemoryPool(pool, state, tx, LimitFreeFlag::OFF, RejectAbsurdFeeFlag::OFF, MempoolProofVerificationFlag::SYNC);
 ...
 */
CScCertificate BlockchainTestManager::GenerateCertificate(uint256 scId, int epochNumber, int64_t quality,
                                                          ProvingSystem provingSystem, CTransactionBase* inputTxBase) const {
...
void BlockchainTestManager::GenerateSidechainTestParameters(ProvingSystem provingSystem, TestCircuitType circuitType) const {
    ...
    zendoo_generate_mc_test_params(circuitType, provingSystem, 1 << 10, (path_char_t*)tempFolderPath.string().c_str(),
                                   strlen(tempFolderPath.string().c_str()), &errorCode);
}
 ...
 */
CScProof BlockchainTestManager::GenerateTestCertificateProof(CCertProofVerifierInput certificate, ProvingSystem provingSystem,
                                                             TestCircuitType circuitType) const {
    ...
    zendoo_create_cert_test_proof(false /*zk*/, sptrConst.get(), scidFe, certificate.epochNumber, certificate.quality, btList,
                                  certificate.bt_list.size(), custom_fields.get(), custom_fields_len, sptrCum.get(),
                                  certificate.mainchainBackwardTransferRequestScFee, certificate.forwardTransferScFee,
                                  provingKey, (path_char_t*)certProofPath.c_str(), strlen(certProofPath.c_str()), 1 << 10,
                                  &errorCode);
 ...
 * @return CScProof The generated proof.
 */
CScProof BlockchainTestManager::GenerateTestCswProof(CCswProofVerifierInput csw, ProvingSystem provingSystem,
                                                     TestCircuitType circuitType) const {
    ...
    bool ret = zendoo_create_csw_test_proof(false, /*zk*/
                                            csw.nValue, constantFe, scidFe, sptrNullifier.get(), &bwsCswPkHash, sptrCdh.get(),
                                            sptrCum.get(), provingKey, (path_char_t*)cswProofPath.c_str(),
                                            strlen(cswProofPath.c_str()), 1 << 10, &code);
 ...
 */
void BlockchainTestManager::StoreSidechainWithCurrentHeight(const uint256& scId, const CSidechain& sidechain,
                                                            int chainActiveHeight) const {
...
bool BlockchainTestManager::VerifyCertificateProof(CCertProofVerifierInput certificate) const {
    ...
    return zendoo_verify_certificate_proof(sptrConst.get(), scidFe, certificate.epochNumber, certificate.quality, btList,
                                           certificate.bt_list.size(), customFields.get(), customFieldsLen, sptrCum.get(),
                                           certificate.mainchainBackwardTransferRequestScFee, certificate.forwardTransferScFee,
                                           sptrProof.get(), sptrCertVk.get(), &errorCode);
}
...
bool BlockchainTestManager::VerifyCswProof(CCswProofVerifierInput csw) const {
    ...
    return zendoo_verify_csw_proof(csw.nValue, sptrConst.get(), scidFe, sptrNullifier.get(), &bwsCswPkHash, sptrCdh.get(),
                                   sptrCum.get(), sptrProof.get(), sptrCeasedVk.get(), &code);
}
 ...
 */
void BlockchainTestManager::ResetAsyncProofVerifier() const { TEST_FRIEND_CScAsyncProofVerifier::GetInstance().Reset(); }
...
sc_pk_t* BlockchainTestManager::GetTestProvingKey(ProvingSystem provingSystem, TestCircuitType circuitType) const {
    ...
    return zendoo_deserialize_sc_pk_from_file((path_char_t*)provingKeyPath.c_str(), provingKeyPath.length(),
