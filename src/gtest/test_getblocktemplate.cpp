...
class TestReserveKey : public CReserveKey {
  public:
    TestReserveKey() : CReserveKey(nullptr) { m_key.MakeNewKey(true); }
...
class TestCCoinsViewDB : public CCoinsViewDB {
  public:
    TestCCoinsViewDB(size_t nCacheSize, bool fWipe = false) : CCoinsViewDB(nCacheSize, false, fWipe) {}
...
class TxFactory {
  public:
    TxFactory(unsigned int numFakeCoins, unsigned int numSpendTx, unsigned int numBlocks)
        : m_numFakeCoins(numFakeCoins), m_numSpendTx(numSpendTx), m_numBlocks(numBlocks) {
        InitKeys();
        InitScriptPubKey();
    }
  ...
  private:
    ...
    void GenerateSpendTxSet() {
        ...
        ASSERT_LE((INPUTS_IN_TX_0 / 2 + INPUTS_IN_TX_1 / 2) * m_numSpendTx, m_numFakeCoins)
            << "Inputs number must be less than coins number.";
    ...
    const CTxOut& GetCacheEntryTxOut(const CCoinsCacheEntry& entry, unsigned int num) { return entry.coins.vout[num]; }
    ...
    CScript& GetScriptPubKey() { return m_script; }
...
class GetBlockTemplateTest : public ::testing::Test {
  protected:
    ...
    CBlockIndex& lastBlock() { return blocks.back(); }

    void InitBlockTreeDB() { pblocktree = new CBlockTreeDB(1 << 20, true); }
    ...
    void FillMempool(const std::vector<CMutableTransaction>& transactions) {
        for (const CMutableTransaction& tx : transactions) {
            // To be able to control the order of adding transactions in the blocktemplate based on the number of inputs we
            // provide different fees.
...
TEST_F(GetBlockTemplateTest, TxWith100Inputs) {
    ...
    try {
        ...
        TestMaxWeight(INPUTS_IN_TX_0 * INPUTS_IN_TX_0 * 5 + INPUTS_IN_TX_1 * INPUTS_IN_TX_1 * 4 + 1,
                      {{INPUTS_IN_TX_0, 5}, {INPUTS_IN_TX_1, 4}});
        TestMaxWeight(INPUTS_IN_TX_0 * INPUTS_IN_TX_0 * 10 + INPUTS_IN_TX_1 * INPUTS_IN_TX_1 * 9 + 1,
                      {{INPUTS_IN_TX_0, 10}, {INPUTS_IN_TX_1, 9}});
        TestMaxWeight(
            INPUTS_IN_TX_0 * INPUTS_IN_TX_0 * NUM_SPEND_TX / 2 + INPUTS_IN_TX_1 * INPUTS_IN_TX_1 * NUM_SPEND_TX / 2 + 1,
            {{INPUTS_IN_TX_0, NUM_SPEND_TX / 2}, {INPUTS_IN_TX_1, NUM_SPEND_TX / 2}});
