 ...
 */
const uint8_t TEST_CUSTOM_FIELD[] = {0xbe, 0x61, 0x16, 0xab, 0x27, 0xee, 0xab, 0xbc, 0x09, 0x35, 0xb3,
                                     0xe2, 0x1b, 0xc3, 0xcf, 0xcd, 0x3f, 0x06, 0xac, 0xb3, 0x8a, 0x5c,
                                     0xeb, 0xd4, 0x42, 0xf4, 0x96, 0xd8, 0xbf, 0xd3, 0x8e, 0x7d};
 ...
 */
const uint8_t TWEEDLE_FR_MODULUS[] = {0x01, 0x00, 0x00, 0x00, 0xe2, 0x64, 0x40, 0xa1, 0xb9, 0x59, 0x3f,
                                      0x6c, 0x27, 0xa1, 0x8a, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static CMutableScCertificate CreateDefaultCert() {
    ...
    for (unsigned int idx = 0; idx < 2; ++idx) mcert.addOut(CTxOut(1, dummyScriptPubKey));

    for (unsigned int idx = 0; idx < 3; ++idx) mcert.addBwt(CTxOut(1000 * idx + 456, dummyScriptPubKey));
...
TEST(SidechainsField, Serialization) {
    ...
    ////////////////////
    std::vector<unsigned char> overModuleByteArray = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
TEST(SidechainsField, ComputeHash_EmptyField) {
    std::vector<unsigned char> lhs{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                   0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
TEST(SidechainsField, ComputeHash_ValidField) {
    ...
    std::vector<unsigned char> rhs{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                   0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                   0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x3f};
    CFieldElement rhsField{rhs};
    ASSERT_TRUE(rhsField.IsValid());

    std::vector<unsigned char> expectedHash{0xee, 0x63, 0x04, 0xc4, 0x92, 0xac, 0x10, 0x22, 0xbe, 0xfd, 0x4d,
                                            0x88, 0x5d, 0x4a, 0x13, 0x8b, 0x12, 0x99, 0x55, 0xa0, 0xff, 0x20,
                                            0x1a, 0x92, 0x41, 0xf8, 0xc1, 0x2a, 0x03, 0x21, 0xc7, 0x24};
...
TEST(SidechainsField, NakedZendooFeatures_PoseidonHashTest) {
    unsigned char lhs[CFieldElement::ByteSize()] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x3f};

    unsigned char rhs[CFieldElement::ByteSize()] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x3f};

    unsigned char hash[CFieldElement::ByteSize()] = {0x42, 0xff, 0xd4, 0x94, 0x7f, 0x76, 0xf7, 0xc1, 0xba, 0x0a, 0xcf,
                                                     0x73, 0xf3, 0x0a, 0xa3, 0x7b, 0x5a, 0xe8, 0xeb, 0xde, 0x5d, 0x61,
                                                     0xc3, 0x19, 0x70, 0xc2, 0xf6, 0x45, 0x7b, 0x83, 0x2a, 0x39};
...
TEST(SidechainsField, NakedZendooFeatures_PoseidonMerkleTreeTest) {
    ...
    // Deserialize root
    std::vector<unsigned char> expected_root_bytes{113, 174, 41, 1,  227, 14,  47,  27,  44, 172, 21,  18,  63,  182, 174, 162,
                                                   239, 251, 93, 88, 43,  221, 235, 253, 30, 110, 180, 114, 134, 192, 15,  20};
...
TEST(SidechainsField, NakedZendooFeatures_TreeCommitmentCalculation) {
    ...
    CScCertificate cert = txCreationUtils::createCertificate(
        scId,
...
TEST(SidechainsField, CommitmentComputationFromSerializedBlock) {
    ...
    boost::filesystem::path blockPath =
        boost::dll::program_location().parent_path() / "/gtest/test-data/block_with_v0_and_v1_certificates.hex";
...
TEST(CctpLibrary, BitVectorMerkleTree) {
    ...
    //    unsigned char buffer[5] = {0xad, 0xde, 0xef, 0xbe, 0x00};
    unsigned char buffer[65] = {0xad, 0xde, 0xef, 0xbe, 0x00, 0xaa, 0xdd, 0xff, 0xad, 0xde, 0xef, 0xbe, 0x00,
                                0xaa, 0xdd, 0xff, 0xad, 0xde, 0xef, 0xbe, 0x00, 0xaa, 0xdd, 0xff, 0xad, 0xde,
                                0xef, 0xbe, 0x00, 0xaa, 0xdd, 0xff, 0xad, 0xde, 0xef, 0xbe, 0x00, 0xaa, 0xdd,
                                0xff, 0xad, 0xde, 0xef, 0xbe, 0x00, 0xaa, 0xdd, 0xff, 0xad, 0xde, 0xef, 0xbe,
                                0x00, 0xaa, 0xdd, 0xff, 0xad, 0xde, 0xef, 0xbe, 0x00, 0xaa, 0xdd, 0xff, 0x33};
...
TEST(CctpLibrary, BitVectorMerkleTreeData) {
    ...
    unsigned char buffer[] = {0x44, 0xc7, 0xe2, 0x1b, 0xa1, 0xc7, 0xc0, 0xa2, 0x9d, 0xe0, 0x06, 0xcb, 0x80, 0x74, 0xe2, 0xba,
    ...
    unsigned char solution[] = {0x8a, 0x7d, 0x52, 0x29, 0xf4, 0x40, 0xd4, 0x70, 0x0d, 0x8b, 0x03, 0x43, 0xde, 0x4e, 0x14, 0x40,
...
TEST(CctpLibrary, BitVectorCertificateFieldFullGzip) {
    ...
    for (size_t i = 0; i < SC_BV_SIZE_IN_BYTES; i++) buffer[i] = rand() % 256;
...
TEST(CctpLibrary, CommitmentTreeBuilding) {
    for (uint8_t sidechainVersion = 0;
        ...
        ASSERT_TRUE(ret_code == CctpErrorCode::OK);
        printf("ct = [");
        for (int i = 0; i < sizeof(field_bytes); i++) printf("%02x", ((unsigned char*)field_bytes)[i]);
        printf("]\n");
        ...
        for (const CTxScCreationOut& ccout : tx.GetVscCcOut()) {
            ...
            if (!ccout.vFieldElementCertificateFieldConfig.empty()) {
                bws_fe_cfg.reset(new BufferWithSize((const unsigned char*)&ccout.vFieldElementCertificateFieldConfig[0],
            ...
            bool ret = zendoo_commitment_tree_add_scc(
                ct, scid_fe, ccout.nValue, &bws_pk, &bws_tx_hash, out_idx, ccout.withdrawalEpochLength,
                ccout.mainchainBackwardTransferRequestDataLength, bws_fe_cfg.get(), bvcfg.get(), bvcfg_size,
                ccout.mainchainBackwardTransferRequestScFee, ccout.forwardTransferScFee, bws_custom_data.get(), constant_fe,
                &bws_cert_vk, &bws_csw_vk, &ret_code);
        ...
        for (const CBwtRequestOut& ccout : tx.GetVBwtRequestOut()) {
            ...
            bool ret = zendoo_commitment_tree_add_bwtr(ct, scid_fe, ccout.scFee, sc_req_data.get(), sc_req_data_len,
                                                       &bws_tx_hash,       // swapped
            ...
            ret = zendoo_commitment_tree_add_bwtr(ct, scid_fe, ccout.scFee, sc_req_data.get(), sc_req_data_len,
                                                  &bws_bwtr_pk_hash, &bws_tx_hash, out_idx, &ret_code);
            ASSERT_TRUE(ret == true);
            ASSERT_TRUE(ret_code == CctpErrorCode::OK);
        ...
        for (const CTxCeasedSidechainWithdrawalInput& ccin : tx.GetVcswCcIn()) {
            ...
            bool ret =
                zendoo_commitment_tree_add_csw(ct, scid_fe, ccin.nValue, sptrNullifier.get(), &bws_csw_pk_hash, &ret_code);
            ASSERT_TRUE(ret == true);
            ASSERT_TRUE(ret_code == CctpErrorCode::OK);
        ...
        bool ret = zendoo_commitment_tree_add_cert(
            ct, scid_fe, epoch_number, quality, bt_list, bt_list_len, custom_fields.get(), custom_fields_len, sptrCum.get(),
    ...
    for (uint8_t sidechainVersion = 0;
        ...
        for (const CTxScCreationOut& ccout : tx.GetVscCcOut()) {
            ...
            ret = zendoo_commitment_tree_add_scc(nullptr,  // null ptr obj
                                                 scid_fe, crAmount, &bws_pk, &bws_tx_hash, out_idx, epoch_len, mbtr_len,
                                                 bws_fe_cfg.get(), bvcfg.get(), bvcfg_size,
                                                 ccout.mainchainBackwardTransferRequestScFee, ccout.forwardTransferScFee,
                                                 bws_custom_data.get(), constant_fe, &bws_cert_vk, bws_csw_vk.get(), &ret_code);
            ...
            ret = zendoo_commitment_tree_add_scc(ct, scid_fe, crAmount, &bws_pk, &bws_tx_hash, out_idx, epoch_len, mbtr_len,
                                                 &bws_bad,  // bad params
                                                 bvcfg.get(), bvcfg_size, ccout.mainchainBackwardTransferRequestScFee,
                                                 ccout.forwardTransferScFee, bws_custom_data.get(), constant_fe, &bws_cert_vk,
                                                 bws_csw_vk.get(), &ret_code);
            ASSERT_TRUE(ret == false);
            ASSERT_TRUE(ret_code == CctpErrorCode::InvalidBufferData);

            printf("Adding a sc creation to the commitment tree - using null ptr buff as a param ...\n");
            ret = zendoo_commitment_tree_add_scc(ct, scid_fe, crAmount,
                                                 nullptr,  // null ptr
                                                 &bws_tx_hash, out_idx, epoch_len, mbtr_len, bws_fe_cfg.get(), bvcfg.get(),
                                                 bvcfg_size, ccout.mainchainBackwardTransferRequestScFee,
                                                 ccout.forwardTransferScFee, bws_custom_data.get(), constant_fe, &bws_cert_vk,
                                                 bws_csw_vk.get(), &ret_code);
            ASSERT_TRUE(ret == false);
            ASSERT_TRUE(ret_code == CctpErrorCode::NullPtr);
...
static unsigned char genericArr[37] = {0x3e, 0x61, 0xea, 0xe3, 0x11, 0xa5, 0xe1, 0x1a, 0x52, 0xdf, 0xb5, 0xe1, 0xc0,
                                       0x06, 0xe1, 0x77, 0x8a, 0xb8, 0x8d, 0xd3, 0x32, 0x8f, 0xff, 0xe8, 0x9d, 0xdf,
                                       0xa6, 0xc2, 0x1a, 0xff, 0xe4, 0x33, 0x6a, 0xf1, 0x36, 0xb2, 0x1b};
...
TEST(CctpLibrary, GetScIdFromNullInputs) {
    unsigned char expected_res[CFieldElement::ByteSize()] = {0xe5, 0x89, 0x89, 0x23, 0xc5, 0x50, 0x1d, 0xbe, 0xcd, 0x48, 0x45,
                                                             0x65, 0x55, 0xcf, 0x92, 0x25, 0xaa, 0x44, 0xbf, 0x3a, 0x4e, 0x84,
                                                             0xbc, 0x20, 0xec, 0x06, 0x9b, 0x4a, 0x4d, 0xcf, 0x97, 0x2a};
    ...
    printf("           scid_fe = [");
    for (int i = 0; i < CFieldElement::ByteSize(); i++) printf("%02x", *ptr++);
    printf("]\n");
...
TEST(CctpLibrary, ReadWriteCmtObj) {
    ...
    mtx.vsc_ccout[0].customData = ParseHex(
    ...
    mtx.vsc_ccout[0].wCertVk = CScVKey{ParseHex(
