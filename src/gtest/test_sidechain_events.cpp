...
TEST_F(SidechainsEventsTestSuite, SidechainInItsFirstEpochIsNotCeased) {
    ...
    view->UpdateScInfo(scCreationTx, dummyBlock, creationHeight);
...
TEST_F(SidechainsEventsTestSuite, CeasingHeightUpdateForScCreation) {
    ...
    int ceasingHeight = scInfo.StartHeightForEpoch(1)+scInfo.SafeguardMargin();
...
TEST_F(SidechainsEventsTestSuite, CeasingHeightUpdateForFullCert) {
    ...
    ASSERT_TRUE(view->GetSidechain(scId, scInfo));
    int currentEpoch = scInfo.EpochFor(creationHeight);
    int initialCeasingHeight = scInfo.StartHeightForEpoch(currentEpoch+1)+scInfo.SafeguardMargin();
    ...
    //Checks
    ASSERT_TRUE(view->GetSidechain(scId, scInfo));
    int newCeasingHeight = scInfo.StartHeightForEpoch(cert.epochNumber+2)+scInfo.SafeguardMargin();
...
TEST_F(SidechainsEventsTestSuite, FullCertCoinsHaveBwtStrippedOutWhenSidechainCeases) {
    ...
    view->UpdateScInfo(scCreationTx, dummyBlock, scCreationHeight);
    view->ScheduleSidechainEvent(scCreationTx.GetVscCcOut().at(0), scCreationHeight);

    //Generate certificate
    CScCertificate cert = txCreationUtils::createCertificate(scId, /*epochNumber*/0, dummyBlock.GetHash(),
...
TEST_F(SidechainsEventsTestSuite, PureBwtCoinsAreRemovedWhenSidechainCeases) {
    ...
    ASSERT_TRUE(view->UpdateScInfo(cert, dummyUndo));
