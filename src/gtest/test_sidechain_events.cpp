...
class SidechainsEventsTestSuite : public ::testing::Test {
  public:
    SidechainsEventsTestSuite()
        : dummyBackingView(nullptr),
          view(nullptr),
          dummyBlock(),
          dummyUndo(IncludeScAttributes::ON),
          dummyTxundo(),
          dummyInfo(),
          dummyHeight{1957} {};
  ...
  protected:
    ...
    void storeSidechainWithCurrentHeight(txCreationUtils::CNakedCCoinsViewCache& view, const uint256& scId,
                                         const CSidechain& sidechain, int chainActiveHeight);
};
...
TEST_F(SidechainsEventsTestSuite, CertDataHash_EndWindowToEndWindows_WithoutCert) {
    ...
    initialScState.pastEpochTopQualityCertView.certDataHash =
        CFieldElement{std::vector<unsigned char>(CFieldElement::ByteSize(), 'a')};
    initialScState.lastTopQualityCertView.certDataHash =
        CFieldElement{std::vector<unsigned char>(CFieldElement::ByteSize(), 'b')};
    initialScState.InitScFees();

    storeSidechainWithCurrentHeight(*view, scId, initialScState,
                                    initialScState.GetCertSubmissionWindowStart(initialEpochReferencedByCert));
    ...
    for (int heightToInspect = currWindowEnd + 1; heightToInspect <= nextWindowEnd; ++heightToInspect) {
        ...
        EXPECT_TRUE(view->GetActiveCertView(scId).certDataHash == expectedActiveCertDataHash)
            ...
            << view->GetActiveCertView(scId).certDataHash.GetHexRepr() << "\ninstead of "
            << expectedActiveCertDataHash.GetHexRepr();
    }
}
...
TEST_F(SidechainsEventsTestSuite, CertDataHash_EndWindowToEndWindows_WithCert) {
    ...
    sidechain.pastEpochTopQualityCertView.certDataHash =
        CFieldElement{std::vector<unsigned char>(CFieldElement::ByteSize(), 'a')};
    sidechain.lastTopQualityCertView.certDataHash = CFieldElement{std::vector<unsigned char>(CFieldElement::ByteSize(), 'b')};

    storeSidechainWithCurrentHeight(*view, scId, sidechain,
                                    sidechain.GetCertSubmissionWindowStart(initialEpochReferencedByCert));
    ASSERT_TRUE(view->GetSidechainState(scId) == CSidechain::State::ALIVE);
    ...
    for (int heightToInspect = currWindowEnd + 1; heightToInspect <= nextWindowEnd; ++heightToInspect) {
        if (heightToInspect < firstBlockWhereCertIsIncluded) {
            ...
            sidechain.lastTopQualityCertView.certDataHash =
                CFieldElement{std::vector<unsigned char>(CFieldElement::ByteSize(), 'c')};
...
TEST_F(SidechainsEventsTestSuite, UnknownSidechainIsNeitherAliveNorCeased) {
    ...
    EXPECT_TRUE(state == CSidechain::State::NOT_APPLICABLE) << "sc is in state " << int(state);
}
...
TEST_F(SidechainsEventsTestSuite, FullCertMovesSidechainTerminationToNextEpochSafeguard) {
    ...
    CScCertificate cert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(4),
                                           /*numChangeOut*/ 2, /*bwtAmount*/ CAmount(0),
...
TEST_F(SidechainsEventsTestSuite, PureBwtCertificateMovesSidechainTerminationToNextEpochSafeguard) {
    ...
    int certEpoch = initialScState.lastTopQualityCertReferencedEpoch + 1;
    CScCertificate cert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(0),
                                           /*numChangeOut*/ 0, /*bwtAmount*/ CAmount(10), /*numBwt*/ 1,
...
TEST_F(SidechainsEventsTestSuite, NoBwtCertificateMovesSidechainTerminationToNextEpochSafeguard) {
    ...
    int certEpoch = initialScState.lastTopQualityCertReferencedEpoch + 1;
    CScCertificate cert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(10),
                                           /*numChangeOut*/ 1, /*bwtAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    ASSERT_TRUE(view->UpdateSidechain(cert, dummyUndo));
...
TEST_F(SidechainsEventsTestSuite, EmptyCertificateMovesSidechainTerminationToNextEpochSafeguard) {
    ...
    CScCertificate cert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(0),
                                           /*numChangeOut*/ 0, /*bwtAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    ASSERT_TRUE(view->UpdateSidechain(cert, dummyUndo));
...
TEST_F(SidechainsEventsTestSuite, CeasingHeightUpdateForPureBwtCert) {
    ...
    CScCertificate cert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(0),
                                           /*numChangeOut*/ 0, /*bwtAmount*/ CAmount(0), /*numBwt*/ 4,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    ASSERT_TRUE(view->UpdateSidechain(cert, dummyUndo));
...
TEST_F(SidechainsEventsTestSuite, CeasingHeightUpdateForNoBwtCert) {
    ...
    int certEpoch = initialScState.lastTopQualityCertReferencedEpoch + 1;
    CScCertificate cert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(3),
                                           /*numChangeOut*/ 3, /*bwtAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    ASSERT_TRUE(view->UpdateSidechain(cert, dummyUndo));
...
TEST_F(SidechainsEventsTestSuite, FullCertCoinsHaveBwtStrippedOutWhenSidechainCeases) {
    ...
    uint256 inputCoinHash =
        txCreationUtils::CreateSpendableCoinAtHeight(*view, initialScState.creationBlockHeight - COINBASE_MATURITY);
    int certEpoch = initialScState.lastTopQualityCertReferencedEpoch + 1;
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(4),
                                           /*numChangeOut*/ 2, /*bwtTotalAmount*/ CAmount(2), /*numBwt*/ 1,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
...
TEST_F(SidechainsEventsTestSuite, PureBwtCoinsAreRemovedWhenSidechainCeases) {
    ...
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(0),
                                           /*numChangeOut*/ 0, /*bwtTotalAmount*/ CAmount(1), /*numBwt*/ 1,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
    ...
    for (int pos = cert.nFirstBwtPos; pos < cert.GetVout().size(); ++pos) {
        ...
        EXPECT_TRUE((coinsBlockUndo.scUndoDatabyScId.at(scId).ceasedBwts.at(bwtCounter).nVersion & 0x7f) ==
                    (SC_CERT_VERSION & 0x7f))
            << coinsBlockUndo.scUndoDatabyScId.at(scId).ceasedBwts.at(bwtCounter).nVersion;
        EXPECT_TRUE(coinsBlockUndo.scUndoDatabyScId.at(scId).ceasedBwts.at(bwtCounter).nBwtMaturityHeight ==
                    coinFromCert.nBwtMaturityHeight);
...
TEST_F(SidechainsEventsTestSuite, NoBwtCertificatesCoinsAreNotAffectedByCeasedSidechainHandling) {
    ...
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(4),
                                           /*numChangeOut*/ 2, /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, EmptyCertificatesCoinsAreNotAffectedByCeasedSidechainHandling) {
    ...
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(0),
                                           /*numChangeOut*/ 0, /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, RestoreNoBwtCeasedCoins) {
    ...
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(4),
                                           /*numChangeOut*/ 1, /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, UndoPureBwtCertUpdatesToCeasingScs) {
    ...
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(0),
                                           /*numChangeOut*/ 0, /*bwtTotalAmount*/ CAmount(3), /*numBwt*/ 3,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, UndoNoBwtCertUpdatesToCeasingScs) {
    ...
    CMutableScCertificate mutCert =
        txCreationUtils::createCertificate(scId, certEpoch, CFieldElement{SAMPLE_FIELD}, /*changeTotalAmount*/ CAmount(4),
                                           /*numChangeOut*/ 4, /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 0,
                                           /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, Cert_CoinReconstructionFromBlockUndo_SpendChangeOutput) {
    ...
    CBlock dummyBlock;
    CMutableScCertificate mutCert = txCreationUtils::createCertificate(scId, /*epochNumber*/ 0, CFieldElement{SAMPLE_FIELD},
                                                                       /*changeTotalAmount*/ CAmount(4), /*numChangeOut*/ 1,
                                                                       /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 1,
                                                                       /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, Cert_CoinReconstructionFromBlockUndo_SpendFullCoinsByChangeOutput) {
    ...
    CMutableScCertificate mutCert = txCreationUtils::createCertificate(scId, /*epochNumber*/ 0, CFieldElement{SAMPLE_FIELD},
                                                                       /*changeTotalAmount*/ CAmount(4), /*numChangeOut*/ 1,
                                                                       /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 0,
                                                                       /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
    CScCertificate cert(mutCert);
...
TEST_F(SidechainsEventsTestSuite, Cert_CoinReconstructionFromBlockUndo_SpendFullCoinsByBwt) {
    ...
    uint256 inputCoinHash = txCreationUtils::CreateSpendableCoinAtHeight(*view, 100);
    CMutableScCertificate mutCert = txCreationUtils::createCertificate(scId, /*epochNumber*/ 0, CFieldElement{SAMPLE_FIELD},
                                                                       /*changeTotalAmount*/ CAmount(4), /*numChangeOut*/ 0,
                                                                       /*bwtTotalAmount*/ CAmount(0), /*numBwt*/ 1,
                                                                       /*ftScFee*/ 0, /*mbtrScFee*/ 0);
    mutCert.vin.clear();
    mutCert.vin.push_back(CTxIn(inputCoinHash, 0));
...
TEST_F(SidechainsEventsTestSuite, DoubleFwdsMatureAtHeight) {
    ...
    EXPECT_TRUE(blockUndo.scUndoDatabyScId.at(scId).appliedMaturedAmount ==
                fwdTx.GetVftCcOut()[0].nValue + fwdTx.GetVftCcOut()[1].nValue);
}
...
TEST_F(SidechainsEventsTestSuite, DoubleFwdsDoNotMatureUponRevertSidechainEvents) {
    ...
    EXPECT_TRUE(sidechain.balance == scCreationTx.GetVscCcOut()[0].nValue);
    EXPECT_TRUE(sidechain.mImmatureAmounts.count(fwdMaturityHeight) != 0);
    EXPECT_TRUE(sidechain.mImmatureAmounts.at(fwdMaturityHeight) ==
                fwdTx.GetVftCcOut()[0].nValue + fwdTx.GetVftCcOut()[1].nValue);
}
