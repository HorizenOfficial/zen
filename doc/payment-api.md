...
sc_send <br>|outputs [params]|Send funds to a sidechain<br><br>**outputs** is an array of lists of key/value pairs representing the amounts to send:<br><br>"scid" : id &emsp;- _Sidechain ID_<br>"toaddress" : scaddr &emsp; - _The receiver PublicKey25519Proposition in the SC_<br>"amount" : id &emsp;- _The amount to transfer_<br>"mcReturnAddress" : taddr &emsp;- _The Horizen address where to send the backward transfer in case Forward Transfer is rejected by sidechain_<br><br>**params** is an optionaly of lists of optional key/value pairs :<br><br>["fromaddress" : taddr]&emsp;- _The taddr to send the funds from. If omitted funds are taken from all available UTXO_<br>["changeaddress" : taddr] &emsp;- _The taddr to send the change to, if any. If not set, "fromaddress" is used. If the latter is not set too, a new generated address will be used_<br>["minconf" : conf] &emsp;- _Only use funds confirmed at least this many times. Default=1._<br>["fee" : fee] &emsp;- _The fee amount to attach to this transaction._<br>
