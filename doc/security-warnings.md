...
This implementation of Zen is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `zend` process is running on will
be able to:
...
unprivileged) on the hardware your `zend` process runs on until these
...
In addition to potential mistakes in code we added to Zcash, and
potential mistakes in our modifications to Zcash, it is also possible
that there were potential changes we were supposed to make to Zcash but
